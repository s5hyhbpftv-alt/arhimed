...
LESSONS_MAP = {
    # 603–606, 608, 610 — 16 кадров: список/правило, ловушки, тренажёр
    601: 16, 602: 9, 603: 16, 604: 16, 605: 16, 606: 16, 607: 9, 608: 16, 609: 9, 610: 16,
    ...
    622: 13,
    # 623 — логика: семь приёмов (кто есть кто, истина и ложь, взвешивания,
    # переливания, число встреч, чётность, Дирихле) и памятка
    623: 16,
    # 624 — среднее, оценка и прикидка: среднее и обратный ход к сумме,
    # границы среднего, сторона округления, проверка чужого ответа
    624: 16,
    # 625 — доли и проценты: три вида вопроса (часть, целое, проценты),
    # прирост от прежнего числа и две ловушки со скидками
    625: 16,
}
ok, bad = [], []
...
# (все листы LESSONS_MAP на двух ширинах). Если раскладка уже проверена отдельно,
# его можно выключить: QA_RU_NO_LAYOUT=1 python3 qa_ru.py
if os.environ.get('QA_RU_NO_LAYOUT'):
    print(f'  (пропущено по QA_RU_NO_LAYOUT — раскладку проверяйте отдельно: qa_layout.py {min(LESSONS_MAP)} … {max(LESSONS_MAP)})')
try:
    # Список уроков берём из LESSONS_MAP: иначе новые листы молча не попадают
    # в раскладку, а гейт при этом остаётся зелёным.
    ...
    if not os.environ.get('QA_RU_NO_LAYOUT'):
        # Сколько строк ждём, считаем от LESSONS_MAP, а не числом: по две
        # строки на урок (ширины 390 и 320). Зашитое «36» означало, что после
        # каждого нового листа гейт краснеет сам по себе — на 21 уроке он и
        # покраснел, хотя раскладка была в порядке.
        ждём = 2 * len(LESSONS_MAP)
        check(f'гейт раскладки прогнан по всем {len(LESSONS_MAP)} урокам',
              len(probs) == ждём, f'{len(probs)} строк, ждали {ждём}')
