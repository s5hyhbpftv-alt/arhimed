...
(function(){
  ...
  L603.explain.unshift('Словарные слова нельзя проверить правилом — их просто запоминают. Ниже список слов этого урока по группам: видно, какая буква в каком слове опасна. Толкование каждого слова ждёт в тренажёре — там к нему подсказка и разбор.');
  function scene(step, st){
    if(step===0){
      return SV(`
...
(function(){
  ...
  L604.explain.unshift('Здесь список словарных слов этого урока. Одни запоминают из-за удвоенной согласной, другие — из-за парной или непроизносимой, которую проверка не подскажет. Толкование каждого слова есть в тренажёре.');
  function scene(step, st){
    if(step===0){
      return SV(`
