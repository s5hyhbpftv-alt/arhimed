...
(function(){
  ...
  const L601 = {
    ...
    explain: [
      'Смотри: «Рыжий кот быстро прыгнул на стол». Слова стоят рядом, а работы у них разные. Кликни слово — увидишь вопрос и должность. Должность слова и есть часть речи.',
      'Морфология — наука о частях речи. Часть речи узнают не по буквам, а по трём делам: что слово называет, на какой вопрос отвечает и чем работает в предложении.',
      'Части речи живут двумя семьями. Самостоятельные называют что-то и отвечают на вопрос — из них собирается предложение. Служебные ничего не называют: они только служат. Из «и на но» предложения не выйдет.',
      'Имя существительное отвечает на кто? или что? и называет предмет. Кот, дом, мама, река. Кто? — если живое. Что? — всё остальное.',
      'Предмет — не только то, что можно потрогать. Смех, бег, доброта, радость — тоже существительные: они отвечают на что? Не путай: бег — это предмет, бежит — уже действие.',
      'Одушевлённые отвечают на кто?: кот, друг, мама. Неодушевлённые — на что?: дом, книга, радость. Кукла похожа на человека, но для языка это что? — неодушевлённое.',
      'Имя прилагательное отвечает на какой? какая? какое? какие? и называет признак. Оно всегда цепляется к существительному и меняет окончание вместе с ним: рыжий кот, рыжая лиса, рыжее солнце.',
      'Глагол отвечает на что делает? что делать? что сделать? и называет действие или состояние. Бежит — действие. Спит, болеет, радуется — тоже глаголы, только про состояние.',
      'Местоимение не называет, а указывает: я, ты, он, она, мы, вы, они. «Маша читает» и «Она читает» — об одном человеке. Имя не повторяем — речь становится легче.',
      'Имя числительное называет число или порядок. Сколько? — пять, двое, сто. Который? — третий, пятый. «Пятый» — это всё ещё число, а не признак вроде «рыжий».',
      'Наречие — признак действия: как? где? когда? куда? Главный секрет: наречие не изменяется, у него нет окончания. Быстрый кот — прилагательное. Бежит быстро — наречие.',
      'Одно гнездо — разные должности. Бег, бежит, быстрый, быстро: смысл рядом, части речи разные. Кликни каждое — и вопрос сам покажет, кто есть кто.',
      'Предлог — мостик: он ставит существительное рядом с другим словом. Кот НА столе, БЕЗ стола, К столу. Это отдельное слово, его пишут раздельно. Это не приставка.',
      'Союз связывает слова и предложения: и, а, но. Частица даёт оттенок: не, бы, же, ли. «Не спит» — два слова. Из одних служебных «и не бы же» предложения не собрать.',
      'Четыре шага — и часть речи найдена. Задай вопрос. Скажи, что слово называет. Посмотри, изменяется ли. Реши: член предложения или служит. Дальше потренируемся.',
      'Теперь сам. Задай вопрос к слову и положи его в нужный ящик. Здесь шесть самостоятельных частей речи, которые мы разобрали.'
    ],
    check: { q: 'Чем наречие отличается от прилагательного?', choices: ['наречие не изменяется', 'наречие называет предмет', 'наречие отвечает кто? что?'], ans: 0,
      exp: 'Наречие — признак действия и не изменяется: быстро всегда быстро. Прилагательное — признак предмета и меняет окончание: быстрый, быстрая, быстрое.' },
    tasks: [
      { q: 'Какое слово — имя существительное?', kind: 'choice', choices: ['бег', 'бежит', 'быстрый'], ans: 0,
        hints: ['Задай вопрос: кто? что? — это существительное.', '«Бежит» — что делает? Это глагол.', '«Бег» отвечает на вопрос что?'], sol: 'бег — предмет, отвечает на что?' },
      { q: 'Сколько самостоятельных слов в предложении «Кот спит на столе»?', kind: 'unit', ans: 3, tol: 0,
        hints: ['Самостоятельные называют и отвечают на вопрос.', '«На» — предлог, служебное слово.', 'Кот, спит, столе — три самостоятельных.'], sol: '3: кот, спит, столе. «На» — предлог.' }
    ]
  };
...
/* ============ СЛОВАРНЫЕ СЛОВА: кадр-список в начале урока ============
   В уроке про словарные слова первым кадром идёт список слов — только слова,
   без толкований. Толкование у каждого слова уже есть в задаче-тренажёре
   (там подсказка и разбор), второй раз писать его в кадре незачем.
   Слова идут группами по трудности, поэтому у 603 и 604 кадры разные:
   у 603 группы по гласной, у 604 — удвоенная согласная против одиночной. */
function ruDictTasks(lesson){
  try{ return (window.ARH_TASKS||[]).filter(t=>t.dict && t.lesson===lesson); }catch(e){ return []; }
}
function ruDictTasksByClass(a,b){
  try{ return (window.ARH_TASKS||[]).filter(t=>t.dict && t.cls>=a && t.cls<=b); }catch(e){ return []; }
}
/* Берём слова вперемешку из разных групп, чтобы в кадре были все трудности,
   а не двадцать слов на одну букву. */
function ruMixWords(groups,limit){
  const out=[], pools=groups.map(g=>g.words.slice());
  let more=true;
  while(out.length<limit && more){
    more=false;
    for(let i=0;i<pools.length && out.length<limit;i++){
      if(pools[i].length){ out.push(pools[i].shift()); more=true; }
    }
  }
  return out;
}
function ruWordsFrame(title, groups, total, note){
  const shown=groups.map(g=>({label:g.label, words:g.words})).filter(g=>g.words.length);
  const body=shown.map(g=>
    `<div class="rw-grp"><span class="rw-lab">${esc(g.label)}</span>`+
    `<span class="rw-list">${g.words.map(w=>`<span class="rw-w">${esc(w)}</span>`).join('')}</span></div>`).join('');
  const rest=total>0? ' Всего в уроке '+total+' '+ruWordForm(total)+' — каждый есть в тренажёре ниже, там к нему подсказка.' : '';
  return `<div class="rw"><div class="rw-h">${esc(title)}</div>${body}`+
    `<div class="rw-note">${esc(note||'')}${rest}</div></div>`;
}
function ruWordForm(n){
...
/* Кадр списка для 603: группы по трудной гласной. */
const RU_LETTER_NAME={о:'гласная «о»',а:'гласная «а»',е:'гласная «е»',и:'гласная «и»',я:'гласная «я»',э:'гласная «э»'};
function ruWords603(){
  const all=ruDictTasks(603), byLetter={};
  all.forEach(t=>{ const L=String(t.ans||''); (byLetter[L]=byLetter[L]||[]).push(t.word); });
  const groups=['о','а','е','и','я','э'].filter(L=>byLetter[L]).map(L=>({label:RU_LETTER_NAME[L], words:byLetter[L]}));
  const shown=ruMixWords(groups,24), set={}; shown.forEach(w=>set[w]=1);
  const out=groups.map(g=>({label:g.label, words:g.words.filter(w=>set[w])})).filter(g=>g.words.length);
  return ruWordsFrame('Слова этого урока по группам', out, all.length,
    'В каждом слове есть буква, которую правило не подскажет.');
}
/* Списки слов для 619 и 620: группы по классу. */
function ruWords619(){
  const all=ruDictTasksByClass(1,2);
  const groups=[{label:'1 класс',words:all.filter(t=>t.cls===1).map(t=>t.word)},
                {label:'2 класс',words:all.filter(t=>t.cls===2).map(t=>t.word)}];
  const shown=ruMixWords(groups,24), set={}; shown.forEach(w=>set[w]=1);
  const out=groups.map(g=>({label:g.label,words:g.words.filter(w=>set[w])})).filter(g=>g.words.length);
  return ruWordsFrame('Слова 1 и 2 класса', out, all.length, 'Слова не проверяются правилом: их запоминают.');
}
function ruWords620(){
  const all=ruDictTasksByClass(3,4);
  const groups=[{label:'3 класс',words:all.filter(t=>t.cls===3).map(t=>t.word)},
                {label:'4 класс',words:all.filter(t=>t.cls===4).map(t=>t.word)}];
  const shown=ruMixWords(groups,24), set={}; shown.forEach(w=>set[w]=1);
  const out=groups.map(g=>({label:g.label,words:g.words.filter(w=>set[w])})).filter(g=>g.words.length);
  return ruWordsFrame('Слова 3 и 4 класса', out, all.length, 'Слова не проверяются правилом: их запоминают.');
}
/* наружу — чтобы проверка могла посчитать слова в первом кадре */
window.RUWORDS={603:ruWords603,604:ruWords604,619:ruWords619,620:ruWords620};

/* Кадр списка для 604: группы другие — удвоенная согласная против одиночной. */
function ruWords604(){
  const all=ruDictTasks(604), dbl=[], one=[];
  all.forEach(t=>{ (String(t.ans||'').length===2? dbl : one).push(t.word); });
  const groups=[{label:'удвоенная согласная',words:dbl},{label:'парная или непроизносимая',words:one}];
  const shown=ruMixWords(groups,24), set={}; shown.forEach(w=>set[w]=1);
  const out=groups.map(g=>({label:g.label, words:g.words.filter(w=>set[w])})).filter(g=>g.words.length);
  return ruWordsFrame('Слова этого урока по группам', out, all.length,
    'Удвоенную согласную нельзя проверить — только запомнить.');
}
...
(function(){
  ...
  const L603 = {
    ...
    src: 'Русский язык · 5–6 класс · Орфография корня', subj: 'rus',
        explain: [
      'Сначала список словарных слов этого урока. Правило их не проверит — только память. Посмотри, какая гласная в каком слове опасна. Толкование каждого слова ждёт в тренажёре.',
      'Скажи вслух «лиса» и «леса». Разницы нет. Без ударения гласная теряет ясность: на слух букву не выбрать.',
      'Поставь ударение. «Лес» — гласная ясна, как на ладони. «Лесной» — та же буква, уже сомнительная. Ударение — наш фонарик.',
      'Правило короткое: безударную гласную в корне проверяй ударением. Подбери слово, где эта же гласная стоит под ударением, и напиши ту букву, которую услышал.',
      'Два пути к проверке. Измени форму: гора — горы. Или найди родственника: гора — горный. Корень один, буква одна.',
      'Возьми «письмо». Какое слово его проверит? Письма: и под ударением. «Писать» не годится — там гласная снова безударная.',
      'Ловушка. «Вода» про реку, «водитель» про руль. Звучат похоже, смысл разный. Проверять одно другим нельзя.',
      'Ещё одна ловушка: ложка и ложь, гора и горе. Буквы рядом, смысл чужой. Проверочное слово обязано быть роднёй по значению.',
      'Есть слова без фонарика. Собака, корова, ворона, работа. Проверочного нет — это словарные. Их запоминают или смотрят в словаре.',
      'Разберём. Голова — головы, пишем о. Земля — земли, пишем е. Пятак — пять, пишем я. Каждый раз ищем слово, где гласная станет ударной.',
      'Частая ошибка: «а» вместо «о», «и» вместо «е». Спасение одно — произнести проверочное вслух. Под ударением буква не врёт.',
      'Проба: молодой. Проверка — молод, молодость. Пишем о. «Малый» — другое слово, это не проверка.',
      'Алгоритм. Найди корень. Падает ли ударение на гласную? Нет — подбери форму или родственника. Напиши ту букву, которую услышал под ударением. Нет проверки — словарь.',
      'Если проверки нет, слово запоминают целиком. Проговори по слогам, запомни опасную букву, сомневаешься — открой словарь. Не угадывай.',
      'Три фразы с собой. Ударение проверяет гласную. Родственник — по смыслу, не по первым буквам. Нет проверки — словарь. Дальше короткий тренажёр.',
      'Теперь сам. Подбери проверочное слово и вставь букву. Она прилетит на место пропуска.'
    ],
  ...
  /* Первым кадром идёт список словарных слов урока (ruWordsFrame), он живёт в
     RULESSON-описании ниже. Здесь сдвинуты прежние кадры: тренажёр теперь
     девятый, а не восьмой. */
  function scene(step, st){
    if(step===0){
      return SV(`
  ...
  const GAME603=[['м_лодой','о','мОлод — молодость'],['л_сной','е','лЕс'],['г_ра','о','гОры'],
  ...
  const PRED603=[
    ...
    ['q6','«Собака» — как пишем?',[{k:'a',t:'запомнить: словарное',ok:true,fb:'верно: проверки нет'},{k:'b',t:'проверить ударением',ok:false,fb:'у словарных нет проверки'}]],
  ...
  const NOTES603=[
    ...
    ['Примеры','Голова — гОловы, земля — зЕмли, пятак — пЯть.'],
  ...
  function visB603(el){
    try{
      ...
      const st=CHS[lk];
      let extra='';
      /* кадры сдвинуты на один: нулевой теперь отдан списку слов */
      if(step===9){
        const done=st.gRes!=null;
        const it=GAME603[(st.gIdx||0)%GAME603.length];
        /* отклик — компонент RUFEED, а не голая строка: так же, как в 601–610 */
        const feed=done? (st.gRes? window.RUFEED.note('ok','верно',it[2]) : window.RUFEED.note('no','исправить','Правильно: '+it[2])) : '';
      ...
      const p=(step===0||step===9)?null:PRED603[step-1];
      const predHtml=p?PRED(st,p[0],p[1],p[2]):'';
      const nt=(step===0)?null:NOTES603[step-1];
      const noteHtml=nt?NOTE(nt[0],nt[1]):'';
      const stage=(step===0)? words603() : scene(step-1,st);
      el.innerHTML=`<div class="wv"><div class="wv-col">${stage}${extra}${predHtml}${noteHtml}</div></div>`;
    }catch(e){ el.innerHTML=''; }
  }
...
(function(){
  ...
  const L604 = {
    ...
    src: 'Русский язык · 5–6 класс · Орфография корня', subj: 'rus',
        explain: [
      'Сначала список словарных слов этого урока. Одни держат удвоенную согласную, другие — ту, которую слух не выдаст. Толкование каждого слова есть в тренажёре.',
      'Скажи «зуб». На конце слышится [зуп]. Слышим один звук, а буква может быть другой. Это слабая позиция.',
      'Парных согласных шесть: б—п, в—ф, г—к, д—т, ж—ш, з—с. В паре один звонкий, другой глухой. В слабой позиции они путаются.',
      'Опасно в двух местах: на конце слова и перед глухой. Зуб звучит как [зуп], ложка — как [лошка]. Слуху здесь верить нельзя.',
      'Проверка простая: поставь после согласной гласный. Зуб — зубы, снег — снега, глаз — глаза. Гласный вытягивает настоящий звук.',
      'Помогают и согласные л, м, н, р. Зуб — зубной, снег — снежный. После них согласная тоже ясна. Ищем родственника так же, как для гласной.',
      'Разберём. Снег — снега, пишем г. Ложка — ложечка, пишем ж. Глаз — глаза, пишем з. Мороз — морозы, пишем з.',
      'Отдельная история — непроизносимые. Пишем «солнце», слышим [сонцэ]. Буква есть, звука нет.',
      'Такие согласные прячутся в сочетаниях стн, здн, лнц, вств, рдц: местный, праздник, солнце, чувство, сердце.',
      'Проверяют их тем же способом: подбери слово, где согласная зазвучит. Солнце — солнышко, сердце — сердечко, местный — место, здравствуй — здравый.',
      'Ловушка: иногда согласная только кажется. Вкусный — вкусен: буквы т нет. Опасный — опасен. Чудесный — чудеса. Проверка спасает и от лишней буквы.',
      'Удвоенные согласные проверить нельзя: класс, суббота, аллея, хоккей. Это словарные слова, их запоминают по словарю.',
      'Алгоритм. Найди сомнительную согласную. Подбери проверку с гласным после неё. Послушай: зазвучала — буква нужна, не зазвучала — буквы нет. Нет проверки — словарь.',
      'Два слова рядом: местный и вкусный. Оба на «-сный». В одном т есть — место. В другом нет — вкусен. Не копируй одно на другое.',
      'Три фразы с собой. Гласный после согласной. Непроизносимую заставь зазвучать. Удвоенные — словарь. Дальше короткий тренажёр.',
      'Теперь сам. Подбери проверочное слово и вставь букву. Она прилетит на место пропуска.'
    ],
  ...
  /* Первым кадром — список словарных слов урока (ruWordsFrame), он живёт в
     RULESSON-описании ниже; здесь прежние кадры сдвинуты на один. */
  function scene(step, st){
    if(step===0){
      return SV(`
  ...
  function visB604(el){
    try{
      ...
      const st=CHS[lk];
      let extra='';
      /* кадры сдвинуты на один: нулевой отдан списку слов (см. RULESSON ниже) */
      if(step===9){
        const done=st.gRes!=null;
        const it=GAME604[(st.gIdx||0)%GAME604.length];
        const feed=done? (st.gRes? window.RUFEED.note('ok','верно',it[2]) : window.RUFEED.note('no','исправить','Правильно: '+it[2])) : '';
      ...
      const p=(step===0||step===9)?null:PRED604[step-1];
      const predHtml=p?PRED(st,p[0],p[1],p[2]):'';
      const nt=(step===0)?null:NOTES604[step-1];
      const noteHtml=nt?NOTE(nt[0],nt[1]):'';
      el.innerHTML=`<div class="wv"><div class="wv-col">${scene(step-1,st)}${extra}${predHtml}${noteHtml}</div></div>`;
    }catch(e){ el.innerHTML=''; }
  }
...
(function(){
  ...
  const GAME605=[['(за)шёл','слитно','зашёл — приставка приросла к корню'],['(на)столе','раздельно','на столе — предлог, можно вставить «новом»'],
    ...
    ['(при)ехал','слитно','приехал — приставка, слитно'],['(в)доме','раздельно','в доме — предлог, в (этом) доме'],
    ['(об)явил','слитно','объявил — об- на согласную, дальше я, нужен ъ'],['(на)реке','раздельно','на реке — предлог']];
...
(function(){
  ...
  const GAME606=[['ночь','она','женский род: она ночь, поэтому ь'],['стол','он','мужской род'],['окно','оно','средний род'],
    ...
    ['день','он','он день — мужской, хотя есть ь'],['земля','она','она земля — женский'],
    ['молоко','оно','оно молоко — средний, только единственное'],['врач','он','он врач — после шипящей без ь']];
...
(function(){
  ...
  const GAME608=[['читает','наст','настоящее: действие идёт сейчас'],['читал','прош','прошедшее: суффикс -л'],
    ...
    ['бежит','наст','настоящее: сейчас бежит'],['читала','прош','прошедшее, женский род'],
    ['буду писать','буд','будущее сложное: два слова'],['светит','наст','настоящее: светит сейчас']];
...
(function(){
  ...
  const L610 = {
    ...
    explain: [
      'Смотри: «В саду росли яблони груши сливы». Язык спотыкается. Три дерева стоят в ряд, а между ними тишина. Кликни слова — запятые встанут сами, как вдох между именами.',
      'Однородные — это слова-соседи. Один вопрос на всех и одно общее слово. «Росли» — что? Яблони. Груши. Сливы. Три ответа, один вопрос. Поэтому они однородные.',
      'Если соседей просто перечисляют, между ними ставят запятую. Как пауза в списке: яблони, груши, сливы. Союзов нет — значит, запятые работают за них.',
      'Но вот пришёл одиночный союз «и». Он сам разделяет слова. Яблони и груши. Запятая больше не нужна: союз уже сделал её работу.',
      'Если «и» повторяется, запятая возвращается. И яблони, и груши, и сливы. Каждый повтор — это новый вдох. Перед каждым «и», кроме первого, стоит запятая.',
      'Союзы «а» и «но» всегда спорят. Не груши, а сливы. Маленький, но смелый. Перед ними запятая ставится всегда — без разговоров.',
      'Однородными бывают не только деревья в саду. Подлежащие: книга, тетрадь, ручка лежат. Сказуемые: дети читают, рисуют и поют. Дополнения: люблю яблоки, груши, сливы. Один вопрос — и они соседи.',
      'Теперь другой гость. «Маша, помоги мне». Слово «Маша» не про то, кто помогает. Это оклик: эй, Маша! Так называют того, к кому говорят. Это обращение.',
      'Обращение можно поставить в трёх местах. В начале — одна запятая после него. В середине — две, как обнять имя. В конце — одна запятая перед ним. Кликни, куда поставить «Маша».',
      'Обращение не член предложения. Его нельзя подчеркнуть как подлежащее. В «Маша, помоги» подлежащее — скрытое «ты». Маша — просто имя за дверью.',
      'Ловушка. «Маша помогает» — Маша подлежащее, запятая не нужна. «Маша, помоги» — Маша обращение, запятая нужна. Смотри на глагол: рассказ или просьба?',
      'Ещё одна большая запятая. «Светит солнце, и поют птицы». Здесь уже две основы: солнце светит и птицы поют. Это сложное предложение. Между частями — запятая.',
      'Один и тот же «и» — два разных дела. Яблони и груши: одна основа, запятой нет. Светит солнце, и поют птицы: две основы, запятая есть. Сначала посчитай основы, потом ставь знак.',
      'Алгоритм в карман. 1) Найди однородных — проверь союзы. 2) Найди обращение — выдели запятыми. 3) Посчитай основы. 4) Если основ две — запятая между частями.',
      'Три фразы с собой. Перечисление без союза — запятые. Одиночное «и» — без запятой. Обращение всегда выделяют. Дальше короткий тренажёр.',
      'Теперь сам. Прочитай фразу, найди соседей или оклик и скажи, где нужны запятые.'
    ],
    check: { q: 'Нужна ли запятая в «яблони и груши»?', choices: ['нет: союз одиночный', 'да: перед «и» всегда', 'только в конце'], ans: 0,
      exp: 'Одиночный союз «и» сам разделяет однородные. Запятая не нужна: яблони и груши.' },
    tasks: [
      { q: 'Где нужны запятые: «В саду росли яблони груши сливы»?', kind: 'choice', choices: ['после «яблони» и после «груши»', 'только после «яблони»', 'запятые не нужны'], ans: 0,
        hints: ['Это перечисление без союзов.', 'Между однородными при перечислении ставят запятые.', 'Три соседа — две запятые между ними.'], sol: 'после «яблони» и после «груши»' },
      { q: 'Сколько запятых в «Маша помоги мне»?', kind: 'unit', ans: 1, tol: 0,
        hints: ['Маша — это обращение, оклик.', 'Обращение в начале выделяется одной запятой.', 'Маша, помоги мне.'], sol: '1 — обращение в начале.' }
    ]
  };
  function scene(step, st){
    if(step===0){
  ...
  const PRED610=[
    ['q1','Почему фраза «яблони груши сливы» спотыкается?',[{k:'a',t:'нет запятых между соседями',ok:true,fb:'верно: три имени — две паузы'},{k:'b',t:'мало слов',ok:false,fb:'слов хватает, не хватает знаков'}]],
    ['q2','Однородные члены — это…',[{k:'a',t:'один вопрос, одно общее слово',ok:true,fb:'верно'},{k:'b',t:'любые слова рядом',ok:false,fb:'нужен общий вопрос и общее слово'}]],
    ['q3','«Яблони, груши, сливы» — запятые?',[{k:'a',t:'да, это перечисление',ok:true,fb:'верно'},{k:'b',t:'нет, союз же есть',ok:false,fb:'союза тут нет'}]],
    ['q4','«Яблони и груши» — запятая?',[{k:'a',t:'нет: союз одиночный',ok:true,fb:'верно'},{k:'b',t:'да: перед и всегда',ok:false,fb:'перед одиночным «и» не ставят'}]],
    ['q5','«И яблони, и груши» — запятая?',[{k:'a',t:'да: союз повторяется',ok:true,fb:'верно'},{k:'b',t:'нет',ok:false,fb:'повтор союза требует запятых'}]],
    ['q6','Перед «а» и «но» запятая…',[{k:'a',t:'ставится всегда',ok:true,fb:'верно'},{k:'b',t:'не ставится',ok:false,fb:'ставится всегда'}]],
    ['q7','Однородными могут быть…',[{k:'a',t:'подлежащие, сказуемые, дополнения',ok:true,fb:'верно: любой член, если вопрос один'},{k:'b',t:'только подлежащие',ok:false,fb:'соседями бывают разные члены'}]],
    ['q8','«Маша, помоги мне» — что такое Маша?',[{k:'a',t:'обращение',ok:true,fb:'верно: выделяется запятой'},{k:'b',t:'подлежащее',ok:false,fb:'подлежащее — скрытое «ты»'}]],
    ['q9','Обращение в середине выделяют…',[{k:'a',t:'двумя запятыми',ok:true,fb:'верно: с двух сторон'},{k:'b',t:'одной запятой',ok:false,fb:'в середине имя обнимают две запятые'}]],
    ['q10','Обращение — член предложения?',[{k:'a',t:'нет',ok:true,fb:'верно: его только выделяют запятыми'},{k:'b',t:'да, подлежащее',ok:false,fb:'не член предложения'}]],
    ['q11','В «Маша помогает» слово Маша — это…',[{k:'a',t:'подлежащее',ok:true,fb:'верно: рассказ, не оклик'},{k:'b',t:'обращение',ok:false,fb:'здесь нет просьбы — это подлежащее'}]],
    ['q12','Как найти сложное предложение?',[{k:'a',t:'посчитать основы',ok:true,fb:'верно: две и больше — сложное'},{k:'b',t:'посчитать слова',ok:false,fb:'считают основы'}]],
    ['q13','«Яблони и груши» и «светит солнце, и поют птицы» — в чём разница?',[{k:'a',t:'число основ',ok:true,fb:'верно: одна основа против двух'},{k:'b',t:'длина фразы',ok:false,fb:'дело не в длине, а в основах'}]],
    ['q14','Первый шаг алгоритма — …',[{k:'a',t:'найти однородных и проверить союзы',ok:true,fb:'верно'},{k:'b',t:'сразу ставить запятую перед «и»',ok:false,fb:'сначала смотри, одиночный ли союз'}]],
    ['q15','Одиночное «и» между однородными…',[{k:'a',t:'запятую не просит',ok:true,fb:'верно'},{k:'b',t:'всегда с запятой',ok:false,fb:'только повтор «и» просит запятые'}]]
  ];
  const NOTES610=[
    ['Сад без знаков','Три имени подряд без паузы — глаз спотыкается. Запятая — вдох.'],
    ...
    ['Любые члены','Соседями бывают подлежащие, сказуемые, дополнения.'],
    ['Обращение','Оклик: Маша, помоги мне.'],
    ['Три места','Начало — одна запятая, середина — две, конец — одна.'],
    ['Не член','Обращение не подлежащее и не дополнение.'],
    ['Ловушка','Маша помогает — подлежащее. Маша, помоги — обращение.'],
    ['Сложное','Две и более основ — запятая между частями.'],
    ['Один «и», два дела','Сначала посчитай основы, потом ставь знак.'],
    ['Алгоритм','Однородные → союзы; обращение → запятые; основы → сложное.'],
    ['В карман','Перечисление — запятые. Одиночное «и» — нет. Обращение — всегда.'],
    ['Тренажёр','Найди соседей или оклик и скажи, где знак.']
  ];
...
window.RUTRAIN = (function(){
  ...
  window.RUTRAIN_DATA = {
    ...
    603:{title:'Вставь букву', tip:'подбери проверочное слово', buttons:[['о','о'],['е','е'],['и','и']],
      items:[['м_лодой','о','мОлод — молодость'],['л_сной','е','лЕс'],['г_ра','о','гОры'],['п_сьмо','и','пИсьма'],['з_мля','е','зЕмли'],['х_лодный','о','хОлод']]},
...
window.RULETTER = (function(){
  ...
  const L={603:[['м_лодой','о','мОлод — молодость'],['л_сной','е','лЕс'],['г_ра','о','гОры'],['п_сьмо','и','пИсьма'],['з_мля','е','зЕмли'],['х_лодный','о','хОлод']],
...
/* ================= 601 заново: 16 кадров по Ладыженской §15 =================
   Повторение частей речи в 5 классе: вопрос как ключ, самостоятельные и
   служебные, ловушки «бег/бежит» и «быстрый/быстро», алгоритм из четырёх шагов.
   Живой рисовальщик — этот блок: visB601 и RU601 ниже перекрыты. */
window.RU601V2 = (function(){
  const GOLD='#ffd76a', GREEN='#8fd1a8', BLUE='#7fd1ff', MUTED='#d8c9a6';
  const CSS=`
  #lvis .s6.l601{gap:14px}
  #lvis .s6.l601 .sent{display:flex;flex-wrap:wrap;gap:8px;justify-content:center}
  #lvis .s6.l601 .sent button{min-height:48px;min-width:48px;padding:10px 14px;font-size:20px;font-weight:600}
  #lvis .s6.l601 .job{display:flex;flex-direction:column;align-items:center;gap:4px;padding:14px 16px;border-radius:16px;
    background:linear-gradient(180deg,#22362c,#17261e);border:1.5px solid var(--line);text-align:center}
  #lvis .s6.l601 .job b{font-size:24px;line-height:1.15;color:var(--gold);letter-spacing:-.02em}
  #lvis .s6.l601 .job i{font-style:normal;font-size:16px;line-height:1.35;color:var(--mut)}
  #lvis .s6.l601 .fam{display:grid;grid-template-columns:1fr 1fr;gap:10px;width:100%}
  #lvis .s6.l601 .fam button{display:flex;flex-direction:column;align-items:flex-start;gap:2px;min-height:56px;
    padding:12px 14px;text-align:left}
  #lvis .s6.l601 .fam button b{font-size:24px;line-height:1.1;color:var(--gold)}
  #lvis .s6.l601 .fam button i{font-style:normal;font-size:14px;color:var(--mut)}
  #lvis .s6.l601 .bridge{display:flex;flex-direction:column;align-items:center;gap:8px;width:100%}
  #lvis .s6.l601 .bridge .word{max-width:100%}
  #lvis .s6.l601 .bridge .prep{min-width:64px;min-height:48px;padding:10px 16px;font-size:20px;font-weight:600;color:#20180a;
    border-color:var(--gold);background:linear-gradient(180deg,#ffd76a,#e2b23f)}
  #lvis .s6.l601 .askrow{display:flex;gap:10px;flex-wrap:wrap;justify-content:stretch}
  #lvis .s6.l601 .askrow button{flex:1 1 42%;min-height:48px;padding:12px 12px;font-size:16px;font-weight:600;line-height:1.25}
  #lvis .s6.l601 .ghost{opacity:.38;border-style:dashed}
  #lvis .s6.l601 .end{color:var(--gold);font-weight:600}
  #lvis .s6.l601 .casebtns button.hit{border-color:var(--ok);animation:bump 320ms cubic-bezier(.23,1,.32,1)}
  #lvis .s6.l601 .casebtns button.miss{border-color:var(--no);animation:nudge 150ms ease-out}
  #lvis .s6.l601[data-frame="1"] [data-anim]{animation-name:l601rise}
  #lvis .s6.l601[data-frame="2"] [data-anim]{animation-name:l601slide}
  #lvis .s6.l601[data-frame="3"] [data-anim]{animation-name:l601side}
  #lvis .s6.l601[data-frame="4"] [data-anim]{animation-name:l601pop}
  #lvis .s6.l601[data-frame="5"] [data-anim]{animation-name:l601morph}
  #lvis .s6.l601[data-frame="6"] [data-anim]{animation-name:l601side}
  #lvis .s6.l601[data-frame="7"] [data-anim]{animation-name:l601attach}
  #lvis .s6.l601[data-frame="8"] [data-anim]{animation-name:l601slide}
  #lvis .s6.l601[data-frame="9"] [data-anim]{animation-name:l601swap}
  #lvis .s6.l601[data-frame="10"] [data-anim]{animation-name:l601pop}
  #lvis .s6.l601[data-frame="11"] [data-anim]{animation-name:l601attach}
  #lvis .s6.l601[data-frame="12"] [data-anim]{animation-name:l601pop}
  #lvis .s6.l601[data-frame="13"] [data-anim]{animation-name:l601bridge}
  #lvis .s6.l601[data-frame="14"] [data-anim]{animation-name:l601drop}
  #lvis .s6.l601[data-frame="15"] [data-anim]{animation-name:l601slide}
  #lvis .s6.l601[data-frame="16"] [data-anim]{animation-name:l601rise}
  @keyframes l601rise{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:none}}
  @keyframes l601slide{from{opacity:0;transform:translateX(-14px)}to{opacity:1;transform:none}}
  @keyframes l601side{from{opacity:0;transform:translateX(16px)}to{opacity:1;transform:none}}
  @keyframes l601pop{0%{opacity:0;transform:scale(.92)}70%{transform:scale(1.03)}100%{opacity:1;transform:none}}
  @keyframes l601morph{0%{opacity:0;transform:translateY(10px) scale(.96)}100%{opacity:1;transform:none}}
  @keyframes l601attach{from{opacity:0;transform:translateX(-18px)}to{opacity:1;transform:none}}
  @keyframes l601swap{0%{opacity:0;transform:translateY(12px)}100%{opacity:1;transform:none}}
  @keyframes l601bridge{from{opacity:0;transform:scaleX(.86)}to{opacity:1;transform:none}}
  @keyframes l601drop{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:none}}
  @media (max-width:370px){
    #lvis .s6.l601 .fam{grid-template-columns:1fr}
    #lvis .s6.l601 .sent button{font-size:18px}
  }
  @media (prefers-reduced-motion: reduce){
    #lvis .s6.l601 [data-anim]{animation:none!important}
  }`;
  function css(){
    try{
      if(window.RUKIT&&RUKIT.frameCss) RUKIT.frameCss();
      let e=document.getElementById('l601-style');
      if(!e){ e=document.createElement('style'); e.id='l601-style'; document.head.appendChild(e); }
  ...
  const SENT=[['Рыжий','какой?','прилагательное','признак предмета'],
              ['кот','кто?','существительное','предмет'],
              ['быстро','как?','наречие','признак действия'],
              ['прыгнул','что сделал?','глагол','действие'],
              ['на','—','предлог','служебное: мостик'],
              ['стол','что?','существительное','предмет']];
  const P={
    1:['Часть речи — это…',[{k:'a',t:'работа слова в речи',ok:1,fb:'верно: не буквы, а должность'},{k:'b',t:'количество букв',ok:0,fb:'буквы тут ни при чём'}]],
    2:['С чего начинают определение?',[{k:'a',t:'считают слоги',ok:0,fb:'слоги — это фонетика'},{k:'b',t:'задают вопрос',ok:1,fb:'верно: вопрос — ключ'}]],
    3:['Из служебных слов предложение…',[{k:'a',t:'собрать нельзя',ok:1,fb:'верно: они только служат'},{k:'b',t:'легко собрать',ok:0,fb:'«и на но» — это не предложение'}]],
    4:['«Дом» отвечает на вопрос…',[{k:'a',t:'что?',ok:1,fb:'верно: это существительное'},{k:'b',t:'какой?',ok:0,fb:'какой? — вопрос прилагательного'}]],
    5:['«Смех» — это…',[{k:'a',t:'глагол',ok:0,fb:'глагол отвечает что делает?'},{k:'b',t:'существительное',ok:1,fb:'верно: смех — что?'}]],
    6:['К слову «книга» вопрос…',[{k:'a',t:'что?',ok:1,fb:'верно: неодушевлённое'},{k:'b',t:'кто?',ok:0,fb:'кто? — только к живому'}]],
    7:['Прилагательное без предмета…',[{k:'a',t:'живёт само',ok:0,fb:'признак всегда чей-то'},{k:'b',t:'цепляется к предмету',ok:1,fb:'верно: рыжий — это кто?'}]],
    8:['«Спит» — это…',[{k:'a',t:'глагол-состояние',ok:1,fb:'верно: что делает? — спит'},{k:'b',t:'существительное',ok:0,fb:'существительное — сон, а не спит'}]],
    9:['Зачем местоимение?',[{k:'a',t:'не повторять имя',ok:1,fb:'верно: она вместо Маша'},{k:'b',t:'назвать число',ok:0,fb:'число называет числительное'}]],
    10:['«Третий» отвечает на…',[{k:'a',t:'сколько?',ok:0,fb:'сколько? — пять, двое'},{k:'b',t:'который?',ok:1,fb:'верно: это порядок'}]],
    11:['Чем наречие отличается?',[{k:'a',t:'не изменяется',ok:1,fb:'верно: быстро всегда быстро'},{k:'b',t:'отвечает кто? что?',ok:0,fb:'это вопрос существительного'}]],
    12:['«Быстро» и «быстрый» — это…',[{k:'a',t:'одно слово',ok:0,fb:'разные должности, разные части речи'},{k:'b',t:'разные части речи',ok:1,fb:'верно: наречие и прилагательное'}]],
    13:['Предлог пишется…',[{k:'a',t:'отдельно',ok:1,fb:'верно: на столе — два слова'},{k:'b',t:'слитно, как приставка',ok:0,fb:'приставка — часть слова, предлог — отдельное'}]],
    14:['Частица «не» — это…',[{k:'a',t:'часть глагола',ok:0,fb:'не спит — два слова'},{k:'b',t:'отдельное служебное слово',ok:1,fb:'верно: частица пишется отдельно'}]],
    15:['Первый шаг алгоритма — …',[{k:'a',t:'задай вопрос',ok:1,fb:'верно: вопрос открывает должность'},{k:'b',t:'найди корень',ok:0,fb:'корень — это состав слова, урок 602'}]]
  };
  const SC=[['кот','кто?','существительное'],['радость','что?','существительное'],
            ['рыжий','какой?','прилагательное'],['зимний','какой?','прилагательное'],
            ['летит','что делает?','глагол'],['спит','что делает?','глагол'],
            ['она','кто? указывает','местоимение'],['мы','кто? указывает','местоимение'],
            ['пять','сколько?','числительное'],['третий','который?','числительное'],
            ['быстро','как?','наречие'],['вчера','когда?','наречие']];
  const BOX=[['существительное','кто? что?'],['прилагательное','какой?'],
             ['глагол','что делает?'],['местоимение','указывает'],
             ['числительное','сколько? который?'],['наречие','как? где? когда?']];
  window.RUGAME=window.RUGAME||{}; window.RUGAME[601]=SC;
  ...
  function pred(f,body){
    ...
    return body+`<div class="askrow">${p[1].map((o,i)=>BTN(8+i,'chip'+(cur===o.k?(o.ok?' ok':' no'):''),o.t,`s6Pred(${f},'${o.k}')`)).join('')}</div>`+fb;
  }

  function body(s,f){
    if(f===1){
      const cur=SENT.find(x=>x[0]===s.tap);
      return `${T('div',0,'kicker','01 · Слова-работники')}
        ${T('h2',1,'','В одном предложении — разные работы')}
        ${T('p',2,'lead','Кликни слово: рядом появятся вопрос и должность.')}
        <div class="sent">${SENT.map((w,i)=>BTN(3+i,s.tap===w[0]?'chip on':'chip',w[0],`s6Tap('${w[0]}')`)).join('')}</div>
        ${cur?`<div class="job" data-anim style="--i:10"><b>${cur[1]}</b><i>${cur[0]} — ${cur[2]}. ${cur[3]}.</i></div>`
             :T('p',10,'cap','Пока не нажал — слово молчит. Начни с «кот».')}`;
    }
    if(f===2){
      return `${T('div',0,'kicker','02 · Три признака')}
        ${T('h2',1,'','Часть речи узнают по трём делам')}
        <div class="steps">
          ${T('div',2,'st','<span class="n">1</span><span class="t">Что называет?<small>предмет, признак, действие, число</small></span>')}
          ${T('div',3,'st','<span class="n">2</span><span class="t">На какой вопрос отвечает?<small>кто? какой? что делает? как?</small></span>')}
          ${T('div',4,'st','<span class="n">3</span><span class="t">Чем работает в предложении?<small>подлежащее, сказуемое — или только служит</small></span>')}
        </div>
        ${T('p',5,'cap','Не смотри на буквы. Смотри на работу.')}`;
    }
    if(f===3){
      return `${T('div',0,'kicker','03 · Две семьи')}
        ${T('h2',1,'','Самостоятельные и служебные')}
        <div class="pair">
          ${T('div',2,'card','<div class="nm">самостоятельные</div><div class="ex">называют<br>отвечают на вопрос<br>могут быть членами предложения<br><br>кот · рыжий · спит</div>')}
          ${T('div',3,'card','<div class="nm">служебные</div><div class="ex">ничего не называют<br>вопроса нет<br>членами не бывают<br><br>на · и · не</div>')}
        </div>
        <div class="split">${T('span',4,'word wide', s.sv?'<b>и на но</b><i>из служебных предложения нет</i>':'<b>Кот спит на столе</b><i>самостоятельные держат смысл</i>')}</div>
        ${BTN(5,'cta', s.sv?'Вернуть нормальную фразу':'Показать одни служебные',`s6Serv()`)}`;
    }
    if(f===4){
      const on=k=>s['w'+k]?'chip ok':'chip';
      return `${T('div',0,'kicker','04 · Имя существительное')}
        <div class="split">${T('span',1,'word','<b>кот</b><i>кто?</i>')}</div>
        ${T('p',2,'lead','Отвечает на <b style="color:#ffd76a">кто?</b> или <b style="color:#ffd76a">что?</b> и называет предмет.')}
        <div class="row">${BTN(3,on('кот'),'кот',`s6Word('кот')`)}${BTN(4,on('дом'),'дом',`s6Word('дом')`)}${BTN(5,on('мама'),'мама',`s6Word('мама')`)}</div>
        <div class="split">${T('span',6,'tag','кто? — живое')}${T('span',7,'tag','что? — остальное')}</div>
        ${T('p',8,'cap',s.wкот?'«Кот» и «мама» — кто?, «дом» — что?. Все трое — существительные.':'Нажми слово — подпишу вопрос.')}`;
    }
    if(f===5){
      const show=s.trap===1;
      return `${T('div',0,'kicker','05 · Невидимый предмет')}
        ${T('h2',1,'','Даже то, что нельзя потрогать')}
        ${T('p',2,'lead','<b style="color:#ffd76a">Бежит</b> — действие. А <b style="color:#ffd76a">бег</b> — уже предмет: отвечает на что?')}
        <div class="split">
          ${T('span',3,'word', show?'<b>бег</b><i>что?</i>':'<b>бежит</b><i>что делает?</i>')}
        </div>
        ${BTN(4,'cta', show?'Вернуть глагол':'Превратить в существительное',`s6Trap()`)}
        ${T('p',5,'cap','Смех, доброта, радость — та же ловушка: выглядят «как действие», а это существительные.')}`;
    }
    if(f===6){
      return `${T('div',0,'kicker','06 · Кто? и что?')}
        ${T('h2',1,'','Одушевлённые и неодушевлённые')}
        <div class="pair">
          ${T('div',2,'card','<div class="nm">кто?</div><div class="ex">кот · мама · друг<br>живое существо</div>')}
          ${T('div',3,'card','<div class="nm">что?</div><div class="ex">дом · книга · радость<br>всё остальное</div>')}
        </div>
        ${T('p',4,'ex-line','Ловушка: <b>кукла</b> похожа на человека, но вопрос — <b>что?</b> Для языка она неодушевлённая.')}
        ${T('p',5,'cap','Смотри на вопрос, а не на картинку в голове.')}`;
    }
    if(f===7){
      const ADJ=[{n:'кот',e:'ий',q:'какой?',nq:'кто?'},{n:'лиса',e:'ая',q:'какая?',nq:'кто?'},{n:'солнце',e:'ее',q:'какое?',nq:'что?'}];
      const i=s.adj||0, a=ADJ[i];
      return `${T('div',0,'kicker','07 · Имя прилагательное')}
        ${T('p',1,'lead','Признак <b style="color:#ffd76a">цепляется</b> к предмету и меняет окончание вместе с ним.')}
        <div class="split">
          ${T('span',2,'word',`<b>рыж<span class="end">${a.e}</span></b><i>${a.q}</i>`)}
          <svg width="56" height="18" style="flex:none" aria-hidden="true"><line class="draw" x1="4" y1="9" x2="52" y2="9" stroke="#ffd76a" stroke-width="2.4" stroke-linecap="round"/></svg>
          ${T('span',3,'word',`<b>${a.n}</b><i>${a.nq}</i>`)}
        </div>
        <div class="row">${ADJ.map((x,k)=>BTN(4+k,i===k?'chip on':'chip',x.n,`s6Adj(${k})`)).join('')}</div>
        ${T('p',7,'cap','Одно слово «рыжий», три окончания — потому что три разных существительных.')}`;
    }
    if(f===8){
      const V=[['бежит','действие'],['светит','действие'],['спит','состояние'],['радуется','состояние']];
      const n=V.filter(x=>s['v'+x[0]]).length;
      return `${T('div',0,'kicker','08 · Глагол')}
        ${T('p',1,'lead','Отвечает на <b style="color:#ffd76a">что делает?</b> Действие и состояние — оба глаголы.')}
        <div class="row">${V.map((x,k)=>BTN(2+k,s['v'+x[0]]?'chip ok':'chip',x[0],`s6Verb('${x[0]}')`)).join('')}</div>
        <div class="rail">${T('span',6,'tag','действие: бежит, светит')}${T('span',7,'tag','состояние: спит, радуется')}</div>
        ${T('p',8,'score', n?('Отмечено глаголов: '+n+' из 4. Все четыре отвечают «что делает?».'):'Нажми глаголы — они отметятся.')}`;
    }
    if(f===9){
      return `${T('div',0,'kicker','09 · Местоимение')}
        ${T('p',1,'lead','Не называет, а <b style="color:#ffd76a">указывает</b>: я, ты, он, она, мы, вы, они.')}
        <div class="col">
          <div class="split">${T('span',2,'word', s.p?'<b>Она</b><i>указывает</i>':'<b>Маша</b><i>имя</i>')}${T('span',3,'word','<b>читает</b>')}</div>
          ${s.p?T('p',4,'cap','Слово «Маша» ушло, смысл остался. Местоимение заменило имя.'):T('p',4,'cap','Имя повторяется — речь тяжёлая. Нажми кнопку.')}
        </div>
        ${BTN(5,'cta', s.p?'Вернуть имя':'Показать, как короче',`s6Pron()`)}`;
    }
    if(f===10){
      const N=[['пять','сколько?'],['двое','сколько?'],['сто','сколько?'],['третий','который?'],['пятый','который?']];
      return `${T('div',0,'kicker','10 · Имя числительное')}
        ${T('p',1,'lead','Называет число или порядок при счёте — и всегда держит свой вопрос.')}
        <div class="pair">
          ${T('div',2,'card','<div class="nm">сколько?</div><div class="ex">пять · двое · сто<br>количество</div>')}
          ${T('div',3,'card','<div class="nm">который?</div><div class="ex">третий · пятый<br>порядок</div>')}
        </div>
        <div class="row">${N.map((x,k)=>BTN(4+k,s['n'+x[0]]?'chip on':'chip', s['n'+x[0]]?(x[0]+' — '+x[1]):x[0], `s6Num('${x[0]}','${x[1]}')`)).join('')}</div>
        ${T('p',9,'cap','Нажми числительное — подпишу вопрос. «Пятый» — это число, а не признак.')}`;
    }
    if(f===11){
      const A=[['бежит','быстро','как?'],['живёт','вдали','где?'],['пришёл','вечером','когда?']];
      const i=s.adv||0, a=A[i];
      return `${T('div',0,'kicker','11 · Наречие')}
        ${T('p',1,'lead','Признак действия. И оно <b style="color:#ffd76a">не изменяется</b> — окончания нет.')}
        <div class="split">
          ${T('span',2,'word',`<b>${a[0]}</b>`)}
          ${T('span',3,'tag', a[1]+' · '+a[2])}
        </div>
        <div class="row">${A.map((x,k)=>BTN(4+k,i===k?'chip on':'chip',x[1],`s6Adv(${k})`)).join('')}</div>
        ${T('p',7,'ex-line','Сравни: <b>быстрый</b> кот (какой? — прилагательное) и бежит <b>быстро</b> (как? — наречие).')}`;
    }
    if(f===12){
      const F=[['бег','что?','существительное'],['бежит','что делает?','глагол'],['быстрый','какой?','прилагательное'],['быстро','как?','наречие']];
      const cur=F.find(x=>x[0]===s.fam);
      return `${T('div',0,'kicker','12 · Одно гнездо')}
        ${T('h2',1,'','Смысл рядом — должности разные')}
        <div class="fam">${F.map((x,k)=>BTN(2+k,s.fam===x[0]?'chip on':'chip',`<b>${x[0]}</b><i>${x[1]}</i>`,`s6Fam('${x[0]}')`)).join('')}</div>
        ${cur?`<div class="job" data-anim style="--i:8"><b>${cur[2]}</b><i>вопрос ${cur[1]} — поэтому это ${cur[2]}</i></div>`
             :T('p',8,'cap','Кликни слово из гнезда — увидишь, почему оно не сосед.')}`;
    }
    if(f===13){
      const on=s.br!==0;
      return `${T('div',0,'kicker','13 · Предлог')}
        ${T('p',1,'lead','Предлог — <b style="color:#ffd76a">мостик</b> к существительному. Это отдельное слово, не приставка.')}
        <div class="bridge">
          ${T('span',2,'word','<b>кот</b>')}
          ${on?BTN(3,'prep','на',`s6Bridge()`):T('span',3,'tag','мостик убрали')}
          ${T('span',4,'word','<b>столе</b>')}
        </div>
        ${BTN(5,'cta', on?'Убрать предлог':'Вернуть мостик',`s6Bridge()`)}
        ${T('p',6,'cap', on?'Кот на столе — два самостоятельных и мостик между ними.':'Без предлога связь рассыпалась: кот … столе.')}`;
    }
    if(f===14){
      return `${T('div',0,'kicker','14 · Союз и частица')}
        ${T('h2',1,'','Один связывает, другая даёт оттенок')}
        <div class="pair">
          ${T('div',2,'card','<div class="nm">союз</div><div class="ex">и · а · но<br>кот <b>и</b> пёс<br>связывает слова и предложения</div>')}
          ${T('div',3,'card','<div class="nm">частица</div><div class="ex">не · бы · же · ли<br><b>не</b> спит<br>оттенок: отрицание, вопрос</div>')}
        </div>
        ${T('p',4,'ex-line','«Не спит» — <b>два слова</b>. Частица не приросла к глаголу.')}
        ${T('p',5,'cap','Проверка: из «и не бы же» предложения не собрать. Служебные без самостоятельных не живут.')}`;
    }
    if(f===15){
      return `${T('div',0,'kicker','15 · Алгоритм')}
        ${T('h2',1,'','Четыре шага — и часть речи найдена')}
        <div class="steps">
          ${T('div',2,'st','<span class="n">1</span><span class="t">Задай вопрос<small>кто? какой? что делает? как? сколько?</small></span>')}
          ${T('div',3,'st','<span class="n">2</span><span class="t">Что называет?<small>предмет, признак, действие, число — или указывает</small></span>')}
          ${T('div',4,'st','<span class="n">3</span><span class="t">Изменяется ли?<small>наречие не изменяется; служебные — тоже</small></span>')}
          ${T('div',5,'st','<span class="n">4</span><span class="t">Член предложения или служит?<small>если вопроса нет — это служебное</small></span>')}
        </div>
        ${T('p',6,'cap','Дальше — ящик: положи слово на его должность.')}`;
    }
    const i=(s.gIdx||0)%SC.length, it=SC[i], got=s.gRes, done=got!=null, ok=got===it[2];
    const why=BOX.filter(c=>c[0]===it[2])[0];
    return `${T('div',0,'kicker','16 · Тренажёр')}
      <div class="split">${T('span',1,'word',`<b>${it[0]}</b><i>${it[1]}</i>`)}</div>
      ${done
        ? (ok ? window.RUFEED.note('ok','верно',it[0]+' — '+it[2]+(why?' · '+why[1]:''))
              : window.RUFEED.note('no','исправить','<b>'+it[0]+'</b> — это '+it[2]+', а не '+got))
        : `<p class="verdict">Задай вопрос и выбери часть речи.</p>`}
      <div class="casebtns">${BOX.map((c,k)=>
        `<button type="button" data-anim style="--i:${2+k}" class="${done&&got===c[0]?(ok?'hit':'miss'):''}" onclick="s6Sort('${c[0]}')"><span class="nm">${c[0]}</span><span class="qs">${c[1]}</span></button>`).join('')}</div>
      <p class="score">верно: ${s.gOk||0} · ошибок: ${s.gBad||0} · всего: ${SC.length}</p>
      <p class="cap">${done?'Нажми любую кнопку — следующее слово.':'Вопрос на карточке — уже подсказка.'}</p>`;
  }
  ...
  function render(el){
    ...
    el.innerHTML=`<div class="s6 l601" data-frame="${f}">${pred(f, body(s,f))}</div>`;
  }

  window.s6Pred=(f,k)=>{ S()['q'+f]=k; chRender(0); };
  window.s6Tap=(w)=>{ S().tap=w; chRender(0); };
  window.s6Word=(w)=>{ S()['w'+w]=1; chRender(0); };
  window.s6Trap=()=>{ const s=S(); s.trap=s.trap===1?0:1; chRender(0); };
  window.s6Adj=(i)=>{ S().adj=i; chRender(0); };
  window.s6Verb=(w)=>{ const s=S(); s['v'+w]=1; chRender(0); };
  window.s6Pron=()=>{ const s=S(); s.p=s.p?0:1; chRender(0); };
  window.s6Num=(w,q)=>{ S()['n'+w]=q; chRender(0); };
  window.s6Adv=(i)=>{ S().adv=i; chRender(0); };
  window.s6Fam=(w)=>{ S().fam=w; chRender(0); };
  window.s6Bridge=()=>{ const s=S(); s.br=(s.br===0)?1:0; chRender(0); };
  window.s6Serv=()=>{ const s=S(); s.sv=s.sv?0:1; chRender(0); };
  window.s6Sort=(key)=>{
    ...
    try{
      ...
      const crate=[...document.querySelectorAll('#lvis .s6 .casebtns button')].find(b=>(b.getAttribute('onclick')||'').indexOf("'"+key+"'")>0);
    ...
    try{
      ...
      const dx=(to.left+to.width/2)-(from.left+from.width/2), dy=(to.top+to.height/2)-(from.top+from.height/2);
      requestAnimationFrame(()=>{ c.style.transform='translate('+dx.toFixed(1)+'px,'+dy.toFixed(1)+'px) scale(.92)'; c.style.opacity='.08'; });
  ...
  window.WAVE_B[cfg.id]=function(el){
    try{
      ...
      /* на кадре тренажёра вопрос-проверка не нужен: там уже своё задание.
         ...
         predShift=1 у уроков, где перед кадрами добавлен свой кадр (список
         словарных слов в 603 и 604): тогда пояснения сдвинуты на один. */
      const shift=cfg.predShift||0;
      const pIdx=step-shift;
      const pv=(pIdx>=0 && pIdx<8)&&ex.pred&&ex.pred[pIdx], nt=ex.notes&&ex.notes[pIdx];
...
(function(){
  ...
  window.RULESSON({id:603, predShift:1, frames:[
    ()=>`<div class="kicker">01 · Слова урока</div>
      <h2>Слова, которые надо запомнить</h2>
      ${ruWords603()}`,
    ()=>`<div class="kicker">02 · Самая частая ошибка</div>
    ...
    ()=>`<div class="kicker">03 · Ударение — помощник</div>
    ...
    ()=>`<div class="kicker">04 · Правило</div>
    ...
    ()=>`<div class="kicker">05 · Два способа</div>
    ...
    ()=>`<div class="kicker">06 · Ловушка</div>
    ...
    ()=>`<div class="kicker">07 · Словарные слова</div>
    ...
    ()=>`<div class="kicker">08 · Разбор примеров</div>
    ...
    ()=>`<div class="kicker">09 · Ещё одна ошибка</div>
  ...
  window.RULESSON({id:604, predShift:1, frames:[
    ()=>`<div class="kicker">01 · Слова урока</div>
      <h2>Слова, которые надо запомнить</h2>
      ${ruWords604()}`,
    ()=>`<div class="kicker">02 · Слабый звук</div>
    ...
    ()=>`<div class="kicker">03 · Шесть пар</div>
    ...
    ()=>`<div class="kicker">04 · Проверка</div>
    ...
    ()=>`<div class="kicker">05 · И согласные л, м, н, р</div>
    ...
    ()=>`<div class="kicker">06 · Непроизносимые согласные</div>
    ...
    ()=>`<div class="kicker">07 · Проверка</div>
    ...
    ()=>`<div class="kicker">09 · Удвоенные согласные</div>
...
/* ============ 619 и 620 · Словарные слова 1–2 и 3–4 класса ============
   Словарные слова идут по классам: 603 и 604 — это 5–6 класс, здесь младшая
   школа. Первым кадром у каждого урока — список слов (только слова, без
   толкований: толкование у каждого уже в его задаче-тренажёре).
   Кадры у уроков разные: у 619 разбор идёт по гласным, у 620 — по тому,
   откуда слово пришло и что в нём за удвоение. */
(function(){
  const R=window.RUKIT; if(!R) return;
  const GOLD=R.GOLD, GREEN=R.GREEN, MUTED=R.MUTED, CYAN='#7fd1ff', PINK='#e8a0d8';
  const M=R.frameMorph;
  ...
  const W619=ruDictTasksByClass(1,2);
  /* [пропуск, верная буква, отклик, неверный вариант] */
  const G619=W619.slice(0,8).map(t=>[t.gap,t.ans,t.word+' — пишем «'+t.ans+'»',t.choices[1]]);
  const trainer619=(st)=>{
    const it=G619[(st.gIdx||0)%G619.length]||['','','',''];
    /* верный ответ идёт то первым, то вторым: иначе ребёнок жмёт первую кнопку */
    const opts=((st.gIdx||0)%2)? [it[3],it[1]] : [it[1],it[3]];
    return `<div class="kicker">09 · Тренажёр</div>
      <div class="split"><span class="word wide"><b>${it[0]}</b><i>вставь пропущенную букву</i></span></div>
      <div class="row">${opts.map(o=>`<button type="button" class="chip" style="flex:1 1 40%;font-size:20px" onclick="ru619Game('${o}')">${o}</button>`).join('')}</div>
      <div class="wv-sml" style="font-size:16px;color:${MUTED}">${st.gRes==null?'выбери букву':'нажми любую кнопку — следующее слово'}</div>
      <p class="score">верно: ${st.gOk||0} · ошибок: ${st.gBad||0} · всего: ${G619.length}</p>`;
  };
  window.RULESSON({id:619, game:G619, frames:[
    ()=>`<div class="kicker">01 · Слова урока</div>
      <h2>Слова, которые надо запомнить</h2>
      ${ruWords619()}`,
    ()=>`<div class="kicker">02 · Что такое словарное слово</div>
      <h2>Есть слова, к которым правило не подходит</h2>
      <p class="lead">«Гора» проверить можно: гОры. А «корова» — нет: проверочного слова не существует.</p>
      ${pair(card('проверяемое','гора — гОры, лесной — лЕс'),card('словарное','корова, сорока, молоко — запоминаем'))}
      <p class="cap">Словарных слов в русском языке много, и все они в словаре.</p>`,
    ()=>`<div class="kicker">03 · Как запоминать</div>
      <h2>Три приёма, которые работают</h2>
      ${cards([['проговори по слогам','ко-ро-ва: веди пальцем по слогам','🗣'],
               ['найди родственника','«мороз» — морОзы рядом по смыслу','🔎'],
               ['загляни в словарь','сомневаешься — словарь решит спор','📖']])}
      <p class="cap">Запомнить слово — значит вспомнить его образ, а не выучить букву отдельно.</p>`,
    ()=>`<div class="kicker">04 · Опасные пары гласных</div>
      <h2>о или а, е или и</h2>
      <p class="lead">Без ударения эти пары звучат одинаково — на слух букву не выбрать.</p>
      <div class="row">${chip('о / а',GOLD)+chip('е / и',GOLD)+chip('я / е',GOLD)}</div>
      ${pair(card('пишем о','корова · сорока · дорога · молоко'),card('пишем а','работа · газета · капуста · машина'))}
      <p class="cap">Проверь себя: произнеси слово вслух и вспомни, как оно написано в словаре.</p>`,
    ()=>`<div class="kicker">05 · Двойная согласная</div>
      <h2>Класс, группа, перрон, суффикс</h2>
      <p class="lead">Здесь запоминаем не букву, а их количество: пишем две.</p>
      <div class="row">${['класс','группа','перрон','суффикс','русский'].map(w=>chip(w,GREEN)).join('')}</div>
      <p class="cap">Одна «с» — «клас», а правильно «класс»: словарное слово.</p>`,
    ()=>`<div class="kicker">06 · Согласная, которой не слышно</div>
      <h2>Говорим [фдрук], а пишем «вдруг»</h2>
      <p class="lead">В конце слова звонкая согласная оглушается, а в «чувствовать» буква «в» не звучит вовсе.</p>
      ${M([['вдру','#8fd1a8','пишем г'],['г',GOLD,'слышим к']])}
      <p class="cap">Сомневаешься — проверь по словарю: вдруг, чувствовать, участвовать.</p>`,
    ()=>`<div class="kicker">07 · Слова-соседи по звуку</div>
      <h2>«Молоко» и «малако»</h2>
      <p class="lead">Второе — не слово, а то, что мы слышим. Так писать нельзя.</p>
      ${pair(card('слышим','мАлАко, карова, сабака'),card('пишем','молоко, корова, собака'))}
      <p class="cap">Проверка одна: вспомни словарное слово и напиши его целиком.</p>`,
    ()=>`<div class="kicker">08 · Алгоритм</div>
      <h2>Как написать словарное слово</h2>
      ${cards([['1. вспомни',"какое слово ты хочешь написать",'🧠'],
               ['2. проговори','по слогам, медленно, вслух','🗣'],
               ['3. проверь','по словарю или по подсказке-образу','📖']])}
      <p class="cap">Потренируйся ниже: там слова этого урока.</p>`,
    (st)=>trainer619(st)
  ]});

  /* ---------- 620 · Словарные слова 3–4 класса ---------- */
  const W620=ruDictTasksByClass(3,4);
  const G620=W620.slice(0,8).map(t=>[t.gap,t.ans,t.word+' — пишем «'+t.ans+'»',t.choices[1]]);
  const trainer620=(st)=>{
    const it=G620[(st.gIdx||0)%G620.length]||['','','',''];
    const opts=((st.gIdx||0)%2)? [it[3],it[1]] : [it[1],it[3]];
    return `<div class="kicker">09 · Тренажёр</div>
      <div class="split"><span class="word wide"><b>${it[0]}</b><i>вставь пропущенную букву</i></span></div>
      <div class="row">${opts.map(o=>`<button type="button" class="chip" style="flex:1 1 40%;font-size:20px" onclick="ru620Game('${o}')">${o}</button>`).join('')}</div>
      <div class="wv-sml" style="font-size:16px;color:${MUTED}">${st.gRes==null?'выбери букву':'нажми любую кнопку — следующее слово'}</div>
      <p class="score">верно: ${st.gOk||0} · ошибок: ${st.gBad||0} · всего: ${G620.length}</p>`;
  };
  window.RULESSON({id:620, game:G620, frames:[
    ()=>`<div class="kicker">01 · Слова урока</div>
      <h2>Слова, которые надо запомнить</h2>
      ${ruWords620()}`,
    ()=>`<div class="kicker">02 · Откуда слово пришло</div>
      <h2>У многих слов есть паспорт</h2>
      <p class="lead">Слово пришло из другого языка — и принесло с собой буквы. Иногда это и есть подсказка.</p>
      ${cards([['из греческого','аптека, герой, космос, тетрадь','🏛'],
               ['из латинского','квадрат, январь, цитата, секрет','🏺'],
               ['из французского','пейзаж, портфель, пальто','🗼']])}
      <p class="cap">Этимология — не правило, но хорошая зацепка для памяти.</p>`,
    ()=>`<div class="kicker">03 · Удвоенная согласная</div>
      <h2>Две буквы, а слово одно</h2>
      <div class="row">${['троллейбус','рассказ','аннотация','аппетит'].map(w=>chip(w,GREEN)).join('')}</div>
      <p class="lead">На стыке приставки и корня тоже бывает двойная: рас- + сказ.</p>
      <p class="cap">Такие слова проверяют словарём, а не правилом.</p>`,
    ()=>`<div class="kicker">04 · Согласная, которой не слышно</div>
      <h2>Праздник и здесь</h2>
      <p class="lead">В «празднике» не звучит «д», в «здесь» — тоже. А написать их надо.</p>
      ${pair(card('слышим','празник, здезь'),card('пишем','праздник, здесь'))}
      <p class="cap">Проверить нечем — слово запоминают целиком.</p>`,
    ()=>`<div class="kicker">05 · Буква «э» в начале</div>
      <h2>Экскаватор, эскалатор</h2>
      <p class="lead">В начале этих слов слышится [э], и пишется «э», а не «е».</p>
      <div class="row">${chip('экскаватор',CYAN)+chip('эскалатор',CYAN)}</div>
      <p class="cap">Слова пришли из латинского, где и была эта буква.</p>`,
    ()=>`<div class="kicker">06 · Названия месяцев</div>
      <h2>Январь, октябрь, ноябрь, декабрь</h2>
      <p class="lead">Месяцы названы по латинским числам, и буквы в них достались оттуда.</p>
      ${cards([['январь','от «Januarius»','1️⃣'],['октябрь','от «octo» — восемь','8️⃣'],['декабрь','от «decem» — десять','🔟']])}
      <p class="cap">Мягкий знак на конце — у всех: -брь.</p>`,
    ()=>`<div class="kicker">07 · Проверь родственником</div>
      <h2>Иногда проверка всё-таки есть</h2>
      ${pair(card('проверяем','правительство — прАвить, карточка — кАрта'),card('запоминаем','здесь, обед, огонь, секрет'))}
      <p class="cap">Сначала поищи родственника, и только потом учи наизусть.</p>`,
    ()=>`<div class="kicker">08 · Алгоритм</div>
      <h2>Порядок действий</h2>
      ${cards([['1. подумай',"есть ли проверочное слово",'🔎'],
               ['2. вспомни','откуда слово пришло и как звучит','🗣'],
               ['3. проверь','по словарю, если сомневаешься','📖']])}
      <p class="cap">Потренируйся ниже: там слова этого урока.</p>`,
    (st)=>trainer620(st)
  ]});

  /* обработчики тренажёров: как в 603 — верный ответ сравнивается с ключом */
  function game619(i){
    const lk=(typeof lidKey==='function')?lidKey(LV.id):'619';
    if(typeof CHS==='undefined') window.CHS={}; CHS[lk]=CHS[lk]||{}; const st=CHS[lk];
    if(st.gRes!=null){ st.gIdx=((st.gIdx||0)+1)%G619.length; st.gRes=null; chRender(0); return; }
    const it=G619[(st.gIdx||0)%G619.length]; st.gRes=(i===it[1]);
    if(st.gRes) st.gOk=(st.gOk||0)+1; else st.gBad=(st.gBad||0)+1;
    chRender(0);
  }
  function game620(i){
    const lk=(typeof lidKey==='function')?lidKey(LV.id):'620';
    if(typeof CHS==='undefined') window.CHS={}; CHS[lk]=CHS[lk]||{}; const st=CHS[lk];
    if(st.gRes!=null){ st.gIdx=((st.gIdx||0)+1)%G620.length; st.gRes=null; chRender(0); return; }
    const it=G620[(st.gIdx||0)%G620.length]; st.gRes=(i===it[1]);
    if(st.gRes) st.gOk=(st.gOk||0)+1; else st.gBad=(st.gBad||0)+1;
    chRender(0);
  }
  window.ru619Game=function(o){ try{ game619(o); }catch(e){} };
  window.ru620Game=function(o){ try{ game620(o); }catch(e){} };

  /* ---------- карточки уроков ---------- */
  const L619={
    id:619, title:'Словарные слова 1–2 класса', ico:'📕',
    src:'Русский язык · 1–2 класс · Словарные слова', subj:'jun', group:null,
    explain:[
      'Словарные слова нельзя проверить правилом — их запоминают. Первый кадр урока — список слов 1 и 2 класса: только слова, без толкований. Толкование у каждого слова есть в тренажёре.',
      'Проверяемое слово и словарное — разные вещи. «Гора» проверяется словом «гОры», а к слову «корова» проверочного слова не существует: его написание надо запомнить.',
      'Запоминают слова тремя приёмами: проговорить по слогам, найти родственника или образ, посмотреть в словаре. Запомнить слово — значит вспомнить его образ.',
      'Без ударения гласные путаются: «о» и «а», «е» и «и», «я» и «е» звучат похоже. Поэтому в словарных словах гласную запоминают, а не проверяют на слух.',
      'Удвоенную согласную тоже надо запомнить: класс, группа, перрон, суффикс, русский. Здесь важно не то, какая буква, а то, сколько их.',
      'Бывает, что согласной не слышно: «вдруг» звучит как [фдрук], а в «чувствовать» буква «в» не произносится. Пишем её всё равно.',
      'То, что мы слышим, и то, что пишем, — не одно и то же: «мАлАко» слышится, а пишется «молоко». Помогает только память и словарь.',
      'Порядок действий: вспомни слово, проговори его по слогам, проверь по словарю или по подсказке-образу. Потренируйся ниже — там слова этого урока.',
      'Тренажёр: восемь слов урока идут по кругу. Выбери букву — верный ответ сверяется с ключом, а не с подсказкой. Ошибиться не страшно: после проверки видно правильное написание.'
    ],
    check:{ q:'Какое слово нельзя проверить правилом?', choices:['собака','гора','лесной'], ans:0,
      exp:'К «собаке» проверочного слова нет: это словарное слово, его запоминают.' },
    tasks:[
      { q:'Какая буква пропущена: «к_рова»?', kind:'choice', choices:['о','а'], ans:0,
        hints:['Проверить правилом нельзя — вспомни, как пишется слово.','Словарное слово: пишем «о».'], sol:'о' },
      { q:'Сколько «с» в слове «класс»?', kind:'unit', ans:2, tol:0,
        hints:['Это словарное слово с удвоенной согласной.','Класс — две «с».'], sol:'2' }
    ]
  };
  const L620={
    id:620, title:'Словарные слова 3–4 класса', ico:'📗',
    src:'Русский язык · 3–4 класс · Словарные слова', subj:'jun', group:null,
    explain:[
      'Первый кадр урока — список словарных слов 3 и 4 класса: только слова, без толкований, потому что толкование у каждого слова есть в тренажёре.',
      'Многие слова пришли из других языков и принесли с собой буквы: из греческого — аптека, герой, космос; из латинского — квадрат, январь; из французского — пейзаж, пальто. Этимология — не правило, но зацепка для памяти.',
      'Удвоенная согласная встречается и в корне (троллейбус, аннотация, аппетит), и на стыке приставки с корнем: рас- + сказ — рассказ. Такие слова проверяют по словарю.',
      'Непроизносимую согласную запоминают: в «празднике» не слышится «д», в «здесь» — тоже, а пишутся они полностью.',
      'В начале некоторых слов слышится и пишется «э»: экскаватор, эскалатор. Пришли они из латинского языка.',
      'Названия месяцев достались нам от латинских чисел: январь — от Januarius, октябрь — от octo (восемь), декабрь — от decem (десять). Мягкий знак на конце есть у всех: -брь.',
      'Прежде чем учить слово наизусть, поищи родственника: правительство — прАвить, карточка — кАрта. Если родственника нет, слово запоминают целиком.',
      'Порядок действий: подумай, есть ли проверочное слово; вспомни, откуда слово пришло и как оно звучит; проверь по словарю, если сомневаешься. Потренируйся ниже.',
      'Тренажёр: восемь слов урока идут по кругу. Выбери букву — верный ответ сверяется с ключом, а не с подсказкой. После проверки всегда видно правильное написание.'
    ],
    check:{ q:'Как проверить слово «правительство»?', choices:['словом «прАвить»','словом «прав»','никак, это словарное слово'], ans:0,
      exp:'В слове «прАвить» гласная «а» под ударением — она и подсказывает написание.' },
    tasks:[
      { q:'Сколько «н» в слове «аннотация»?', kind:'unit', ans:2, tol:0,
        hints:['Это слово с удвоенной согласной.','Аннотация — две «н».'], sol:'2' },
      { q:'Какая буква пропущена: «_кскаватор»?', kind:'choice', choices:['э','е'], ans:0,
        hints:['Слышится [э], и пишется «э».','Слово пришло из латинского.','Экскаватор — начальная «э».'], sol:'э' }
    ]
  };
  window.RUEXTRA=window.RUEXTRA||{};
  window.RUEXTRA[619]={pred:[
    ['q1','Что делать со словарным словом?',[{k:'a',t:'запомнить',ok:true,fb:'верно: правило его не проверит'},{k:'b',t:'проверить ударением',ok:false,fb:'проверочного слова нет'}]],
    ['q2','Какое слово проверяемое?',[{k:'a',t:'гора',ok:true,fb:'верно: гОры'},{k:'b',t:'корова',ok:false,fb:'«корова» — словарное'}]],
    ['q3','Что помогает запомнить слово?',[{k:'a',t:'образ и слоги',ok:true,fb:'верно: ко-ро-ва'},{k:'b',t:'соседняя буква',ok:false,fb:'соседняя буква не поможет'}]],
    ['q4','«К_рова» — какая буква?',[{k:'a',t:'о',ok:true,fb:'верно: словарное слово'},{k:'b',t:'а',ok:false,fb:'кАрова — ошибка'}]],
    ['q5','Сколько «с» в «классе»?',[{k:'a',t:'две',ok:true,fb:'верно: класс'},{k:'b',t:'одна',ok:false,fb:'клас — ошибка'}]],
    ['q6','Как пишется [фдрук]?',[{k:'a',t:'вдруг',ok:true,fb:'верно: пишем «г»'},{k:'b',t:'вдрук',ok:false,fb:'слышим «к», а пишем «г»'}]],
    ['q7','«Молоко» или «малако»?',[{k:'a',t:'молоко',ok:true,fb:'верно: словарное слово'},{k:'b',t:'малако',ok:false,fb:'так только слышится'}]],
    ['q8','С чего начать письмо слова?',[{k:'a',t:'вспомнить слово и проговорить',ok:true,fb:'верно'},{k:'b',t:'написать как слышится',ok:false,fb:'на слух словарное слово не написать'}]]
  ],notes:[
    ['Список','Слова урока — в первом кадре; толкование каждого ждёт в тренажёре.'],
    ['Разница','Проверяемое слово проверяют, словарное — запоминают.'],
    ['Приёмы','Слоги, родственник, словарь: три способа удержать слово в памяти.'],
    ['Гласные','Без ударения «о» и «а», «е» и «и» звучат одинаково.'],
    ['Двойные','Класс, группа, перрон, суффикс — запоминаем количество букв.'],
    ['Неслышные','Вдруг, чувствовать, участвовать: буква есть, звука нет.'],
    ['Слух','Слышим «малако» — пишем «молоко».'],
    ['Алгоритм','Вспомни → проговори по слогам → проверь по словарю.'],
    ['Тренажёр','Восемь слов урока по кругу: верный ответ сверяется с ключом.']
  ]};
  window.RUEXTRA[620]={pred:[
    ['q1','Что делать со словарным словом?',[{k:'a',t:'запомнить',ok:true,fb:'верно'},{k:'b',t:'проверить правилом',ok:false,fb:'правило не подойдёт'}]],
    ['q2','Что даёт этимология?',[{k:'a',t:'зацепку для памяти',ok:true,fb:'верно: пейзаж — из французского'},{k:'b',t:'готовое правило',ok:false,fb:'это не правило, а подсказка'}]],
    ['q3','«Рассказ» — почему две «с»?',[{k:'a',t:'рас- + сказ',ok:true,fb:'верно: на стыке приставки и корня'},{k:'b',t:'просто так',ok:false,fb:'две «с» на стыке морфем'}]],
    ['q4','Что не слышится в «празднике»?',[{k:'a',t:'«д»',ok:true,fb:'верно: празДник'},{k:'b',t:'«з»',ok:false,fb:'«з» слышится'}]],
    ['q5','«_кскаватор» — какая буква?',[{k:'a',t:'э',ok:true,fb:'верно: из латинского'},{k:'b',t:'е',ok:false,fb:'экскаватор — с «э»'}]],
    ['q6','Откуда название «октябрь»?',[{k:'a',t:'от латинского «octo» — восемь',ok:true,fb:'верно'},{k:'b',t:'от слова «октава»',ok:false,fb:'оба от «восемь», но месяц — от octo'}]],
    ['q7','Как проверить «карточка»?',[{k:'a',t:'словом «кАрта»',ok:true,fb:'верно: «а» под ударением'},{k:'b',t:'словом «картон»',ok:false,fb:'в «картоне» та же безударная'}]],
    ['q8','Первый шаг алгоритма?',[{k:'a',t:'поискать проверочное слово',ok:true,fb:'верно'},{k:'b',t:'сразу учить наизусть',ok:false,fb:'сначала поищи родственника'}]]
  ],notes:[
    ['Список','Слова урока — в первом кадре; толкование каждого ждёт в тренажёре.'],
    ['Этимология','Греческий, латинский и французский след: аптека, квадрат, пейзаж.'],
    ['Двойные','Троллейбус, аннотация, рассказ: запоминаем количество букв.'],
    ['Неслышные','Праздник, здесь: буква пишется, хотя не звучит.'],
    ['Буква «э»','Экскаватор, эскалатор — начальная «э» из латинского.'],
    ['Месяцы','Январь, октябрь, ноябрь, декабрь — от латинских чисел.'],
    ['Проверка','Правительство — прАвить, карточка — кАрта: родственник помогает.'],
    ['Алгоритм','Проверь родственником → вспомни звучание → сверься со словарём.'],
    ['Тренажёр','Восемь слов урока по кругу: верный ответ сверяется с ключом.']
  ]};
  (function(){ const arr=window.ARH_LESSONS||[]; [[619,L619],[620,L620]].forEach(function(p){
    let f=false;
    for(let i=0;i<arr.length;i++){ if(arr[i].id===p[0]){ arr[i]=p[1]; f=true; break; } }
    if(!f) arr.push(p[1]);
  }); })();
})();
...
/* ================= УРОК 610 заново: 16 кадров, живой язык =================
   Однородные члены и обращение. Прежний каркас RULESSON оставлял 9 коротких
   кадров с одинаковыми чипами. Этот блок перекрывает WAVE_B[610]: у каждого
   шага своя сцена (не три карточки), клики, запятые дорисовываются до конца.
   Тренажёр — последний кадр, ключи RUGAME[610] и ru610Game сохранены. */
window.RU610V2 = (function(){
  const GOLD='#ffd76a', GREEN='#8fd1a8', BLUE='#7fd1ff';
  const CSS=`
  #lvis .s6.l610{gap:14px}
  #lvis .s6.l610 .line{display:flex;flex-wrap:wrap;align-items:center;justify-content:center;gap:0;width:100%;
    padding:12px 8px;border-radius:16px;border:1.5px solid var(--line);background:linear-gradient(180deg,#22362c,#17261e)}
  #lvis .s6.l610 .w{font-size:20px;font-weight:600;line-height:1.35;padding:10px 4px;color:var(--ink)}
  #lvis .s6.l610 .w.on{color:var(--gold)}
  #lvis .s6.l610 .w.stem{color:${GREEN}}
  #lvis .s6.l610 .w.call{color:var(--gold);border-bottom:2px solid var(--gold)}
  #lvis .s6.l610 .slot{flex:none;min-width:44px;min-height:48px;padding:0 4px;display:inline-flex;align-items:center;justify-content:center;
    font-size:28px;font-weight:700;line-height:1;color:var(--gold);background:transparent;border:none;border-radius:10px}
  #lvis .s6.l610 .slot.empty{color:rgba(255,215,106,.28)}
  #lvis .s6.l610 .slot .cm{display:inline-block;animation:l610comma 420ms cubic-bezier(.23,1,.32,1) both}
  #lvis .s6.l610 .who{display:flex;flex-direction:column;align-items:center;gap:4px;padding:12px 16px;border-radius:16px;
    background:linear-gradient(180deg,#22362c,#17261e);border:1.5px solid var(--line);text-align:center;min-width:120px}
  #lvis .s6.l610 .who b{font-size:24px;line-height:1.15;color:var(--gold);letter-spacing:-.02em}
  #lvis .s6.l610 .who i{font-style:normal;font-size:16px;line-height:1.35;color:var(--mut)}
  #lvis .s6.l610 .stems{display:flex;gap:10px;width:100%;flex-wrap:wrap;justify-content:center}
  #lvis .s6.l610 .stems .box{flex:1 1 42%;min-width:140px;padding:14px 12px;border-radius:16px;text-align:center;
    background:linear-gradient(180deg,#22362c,#17261e);border:1.5px solid var(--line)}
  #lvis .s6.l610 .stems .box .n{font-size:14px;letter-spacing:.08em;text-transform:uppercase;color:var(--mut)}
  #lvis .s6.l610 .stems .box .t{font-size:20px;font-weight:600;color:var(--gold);margin-top:6px;line-height:1.25}
  #lvis .s6.l610 .askrow button{flex:1 1 42%;min-height:48px;padding:12px;font-size:16px;font-weight:600;line-height:1.25}
  #lvis .s6.l610 .posrow button{flex:1 1 28%;min-height:48px;font-size:16px}
  #lvis .s6.l610 .ghost{opacity:.4}
  #lvis .s6.l610 .casebtns button.hit{border-color:var(--ok);animation:bump 320ms cubic-bezier(.23,1,.32,1)}
  #lvis .s6.l610 .casebtns button.miss{border-color:var(--no);animation:nudge 150ms ease-out}
  #lvis .s6.l610[data-frame="1"] [data-anim]{animation-name:l610rise}
  #lvis .s6.l610[data-frame="2"] [data-anim]{animation-name:l610pop}
  #lvis .s6.l610[data-frame="3"] [data-anim]{animation-name:l610drop}
  #lvis .s6.l610[data-frame="4"] [data-anim]{animation-name:l610slide}
  #lvis .s6.l610[data-frame="5"] [data-anim]{animation-name:l610side}
  #lvis .s6.l610[data-frame="6"] [data-anim]{animation-name:l610pop}
  #lvis .s6.l610[data-frame="7"] [data-anim]{animation-name:l610swap}
  #lvis .s6.l610[data-frame="8"] [data-anim]{animation-name:l610call}
  #lvis .s6.l610[data-frame="9"] [data-anim]{animation-name:l610slide}
  #lvis .s6.l610[data-frame="10"] [data-anim]{animation-name:l610rise}
  #lvis .s6.l610[data-frame="11"] [data-anim]{animation-name:l610swap}
  #lvis .s6.l610[data-frame="12"] [data-anim]{animation-name:l610split}
  #lvis .s6.l610[data-frame="13"] [data-anim]{animation-name:l610side}
  #lvis .s6.l610[data-frame="14"] [data-anim]{animation-name:l610rise}
  #lvis .s6.l610[data-frame="15"] [data-anim]{animation-name:l610pop}
  #lvis .s6.l610[data-frame="16"] [data-anim]{animation-name:l610rise}
  @keyframes l610rise{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:none}}
  @keyframes l610pop{0%{opacity:0;transform:scale(.92)}70%{transform:scale(1.04)}100%{opacity:1;transform:none}}
  @keyframes l610drop{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:none}}
  @keyframes l610slide{from{opacity:0;transform:translateX(-14px)}to{opacity:1;transform:none}}
  @keyframes l610side{from{opacity:0;transform:translateX(14px)}to{opacity:1;transform:none}}
  @keyframes l610swap{0%{opacity:0;transform:translateY(10px)}100%{opacity:1;transform:none}}
  @keyframes l610call{0%{opacity:0;transform:scale(.86)}60%{transform:scale(1.06)}100%{opacity:1;transform:none}}
  @keyframes l610split{from{opacity:0;transform:scaleX(.88)}to{opacity:1;transform:none}}
  @keyframes l610comma{0%{opacity:0;transform:translateY(-12px) scale(.35)}65%{transform:scale(1.18)}100%{opacity:1;transform:none}}
  @media (max-width:370px){
    #lvis .s6.l610 .w{font-size:18px}
    #lvis .s6.l610 .stems{flex-direction:column}
    #lvis .s6.l610 .stems .box{flex:1 1 100%;min-width:0}
    #lvis .s6.l610 .posrow button{flex:1 1 100%}
  }
  @media (prefers-reduced-motion: reduce){
    #lvis .s6.l610 [data-anim],#lvis .s6.l610 .slot .cm{animation:none!important}
  }`;
  function css(){
    try{
      if(window.RUKIT&&RUKIT.frameCss) RUKIT.frameCss();
      let e=document.getElementById('l610-style');
      if(!e){ e=document.createElement('style'); e.id='l610-style'; document.head.appendChild(e); }
      if(e.textContent!==CSS) e.textContent=CSS;
    }catch(e){}
  }
  const S=()=>{ const lk=lidKey(610); if(typeof CHS==='undefined') window.CHS={}; if(!CHS[lk]) CHS[lk]={}; return CHS[lk]; };
  const T=(tag,i,cls,html)=>`<${tag} data-anim style="--i:${i}" class="${cls||''}">${html}</${tag}>`;
  const BTN=(i,cls,html,on)=>`<button type="button" data-anim style="--i:${i}" class="${cls}" onclick="${on}">${html}</button>`;

  const P={
    1:['Почему фраза спотыкается?',[{k:'a',t:'нет запятых между соседями',ok:1,fb:'верно: три имени — две паузы'},{k:'b',t:'мало слов',ok:0,fb:'слов хватает, не хватает знаков'}]],
    2:['Однородные члены — это…',[{k:'a',t:'один вопрос, одно общее слово',ok:1,fb:'верно'},{k:'b',t:'любые слова рядом',ok:0,fb:'нужен общий вопрос и общее слово'}]],
    3:['«Яблони, груши, сливы» — запятые?',[{k:'a',t:'да, это перечисление',ok:1,fb:'верно'},{k:'b',t:'нет, союз же есть',ok:0,fb:'союза тут нет'}]],
    4:['«Яблони и груши» — запятая?',[{k:'a',t:'нет: союз одиночный',ok:1,fb:'верно'},{k:'b',t:'да: перед «и» всегда',ok:0,fb:'перед одиночным «и» не ставят'}]],
    5:['«И яблони, и груши» — запятая?',[{k:'a',t:'да: союз повторяется',ok:1,fb:'верно'},{k:'b',t:'нет',ok:0,fb:'повтор союза требует запятых'}]],
    6:['Перед «а» и «но» запятая…',[{k:'a',t:'ставится всегда',ok:1,fb:'верно'},{k:'b',t:'не ставится',ok:0,fb:'ставится всегда'}]],
    7:['Однородными могут быть…',[{k:'a',t:'разные члены предложения',ok:1,fb:'верно: лишь бы вопрос был один'},{k:'b',t:'только подлежащие',ok:0,fb:'соседями бывают и сказуемые, и дополнения'}]],
    8:['«Маша, помоги мне» — Маша это…',[{k:'a',t:'обращение',ok:1,fb:'верно: оклик, не подлежащее'},{k:'b',t:'подлежащее',ok:0,fb:'подлежащее здесь — скрытое «ты»'}]],
    9:['Обращение в середине выделяют…',[{k:'a',t:'двумя запятыми',ok:1,fb:'верно: имя обнимают с двух сторон'},{k:'b',t:'одной запятой',ok:0,fb:'в середине — две запятые'}]],
    10:['Обращение — член предложения?',[{k:'a',t:'нет',ok:1,fb:'верно: его только выделяют'},{k:'b',t:'да, подлежащее',ok:0,fb:'не член предложения'}]],
    11:['В «Маша помогает» слово Маша — это…',[{k:'a',t:'подлежащее',ok:1,fb:'верно: это рассказ, не оклик'},{k:'b',t:'обращение',ok:0,fb:'здесь нет просьбы'}]],
    12:['Сложное предложение узнают…',[{k:'a',t:'по числу основ',ok:1,fb:'верно: две и больше — сложное'},{k:'b',t:'по числу слов',ok:0,fb:'считают основы, не слова'}]],
    13:['Разница двух фраз с «и» — в…',[{k:'a',t:'числе основ',ok:1,fb:'верно: одна основа против двух'},{k:'b',t:'длине фразы',ok:0,fb:'дело не в длине'}]],
    14:['Первый шаг алгоритма — …',[{k:'a',t:'найти однородных и союзы',ok:1,fb:'верно'},{k:'b',t:'сразу запятую перед «и»',ok:0,fb:'сначала смотри, одиночный ли союз'}]],
    15:['Одиночное «и» между однородными…',[{k:'a',t:'запятую не просит',ok:1,fb:'верно'},{k:'b',t:'всегда с запятой',ok:0,fb:'только повтор «и» просит запятые'}]]
  };
  ...
  function pred(f,body){
    ...
    const fb=cur
      ? `<p class="verdict ${pick&&pick.ok?'ok':'no'}">${pick&&pick.ok?'верно. '+pick.fb:'не так. '+(pick?pick.fb:'подумай ещё')}</p>`
      : `<p class="verdict">Выбери ответ:</p>`;
    return body+`<div class="askrow">${p[1].map((o,i)=>BTN(8+i,'chip'+(cur===o.k?(o.ok?' ok':' no'):''),o.t,`r610Pred(${f},'${o.k}')`)).join('')}</div>`+fb;
  }

  function cm(on){ return on ? '<span class="cm">,</span>' : '<span class="ghost">·</span>'; }

  function body(s,f){
    if(f===1){
      const on=!!s.tap;
      return `${T('div',0,'kicker','01 · Сад без знаков')}
        ${T('h2',1,'','Фраза без паузы спотыкается')}
        ${T('p',2,'lead','Три дерева в ряд, а между ними тишина. Кликни — запятые встанут как вдох.')}
        <div class="line" data-anim style="--i:3">
          <span class="w">росли</span>
          <span class="w ${on?'on':''}">яблони</span>
          ${BTN(4,'slot '+(on?'':'empty'), on?cm(1):'·', 'r610Tap()')}
          <span class="w ${on?'on':''}">груши</span>
          ${BTN(5,'slot '+(on?'':'empty'), on?cm(1):'·', 'r610Tap()')}
          <span class="w ${on?'on':''}">сливы</span>
        </div>
        ${T('p',7,'cap', on?'Вот так дышит перечисление: яблони, груши, сливы.':'Пока не нажал — имена склеились.')}`;
    }
    if(f===2){
      const hit=s.hit||'';
      const q = hit ? 'что росло? — '+hit : 'что росло?';
      return `${T('div',0,'kicker','02 · Слова-соседи')}
        ${T('h2',1,'','Один вопрос на всех')}
        ${T('p',2,'lead','Однородные отвечают на <b style="color:#ffd76a">один вопрос</b> и держатся за одно слово.')}
        <div class="split">${T('span',3,'tag','росли')}</div>
        <div class="row">
          ${BTN(4,hit==='яблони'?'chip on':'chip','яблони',"r610Hit('яблони')")}
          ${BTN(5,hit==='груши'?'chip on':'chip','груши',"r610Hit('груши')")}
          ${BTN(6,hit==='сливы'?'chip on':'chip','сливы',"r610Hit('сливы')")}
        </div>
        <div class="who" data-anim style="--i:7"><b>${q}</b><i>три ответа, один вопрос — значит, однородные</i></div>`;
    }
    if(f===3){
      const on=!!s.list;
      return `${T('div',0,'kicker','03 · Перечисление')}
        ${T('h2',1,'','Нет союза — есть запятая')}
        ${T('p',2,'lead','Соседей называют по очереди. Между ними — пауза. Пауза на письме и есть запятая.')}
        <div class="line" data-anim style="--i:3">
          <span class="w on">яблони</span>${on?`<span class="slot">${cm(1)}</span>`:'<span class="slot empty"> </span>'}
          <span class="w on">груши</span>${on?`<span class="slot">${cm(1)}</span>`:'<span class="slot empty"> </span>'}
          <span class="w on">сливы</span>
        </div>
        ${BTN(4,'cta', on?'Убрать запятые':'Поставить запятые','r610List()')}
        ${T('p',5,'cap', on?'Запятые дорисовались до конца: две паузы на трёх соседей.':'Нажми — знаки встанут на место и останутся.')}`;
    }
    if(f===4){
      const withI=s.un!==0;
      return `${T('div',0,'kicker','04 · Одиночный союз')}
        ${T('h2',1,'','«И» сам делает работу запятой')}
        ${T('p',2,'lead','Один союз <b style="color:#ffd76a">и</b> — и запятая уходит. Он уже разделил слова.')}
        <div class="line" data-anim style="--i:3">
          <span class="w on">яблони</span>
          ${withI?`<span class="w" style="color:#7fd1ff;padding:10px 8px">и</span>`:`<span class="slot">${cm(1)}</span>`}
          <span class="w on">груши</span>
        </div>
        ${BTN(4,'cta', withI?'Вернуть запятую':'Поставить союз «и»','r610Un()')}
        ${T('p',5,'cap', withI?'Яблони и груши — запятая не нужна. Так же с «или» и «да» в значении «и».':'Без союза пауза остаётся: яблони, груши.')}`;
    }
    if(f===5){
      const n=s.rep||0;
      return `${T('div',0,'kicker','05 · Союз повторяется')}
        ${T('h2',1,'','Каждый повтор просит паузу')}
        ${T('p',2,'lead','И яблони, и груши, и сливы. Перед каждым «и», кроме первого, — запятая.')}
        <div class="line" data-anim style="--i:3">
          <span class="w" style="color:#7fd1ff">и</span>
          <span class="w on">яблони</span>
          <span class="slot">${n>=1?cm(1):''}</span>
          <span class="w" style="color:#7fd1ff">и</span>
          <span class="w on">груши</span>
          <span class="slot">${n>=2?cm(1):''}</span>
          <span class="w" style="color:#7fd1ff">и</span>
          <span class="w on">сливы</span>
        </div>
        ${BTN(4,'cta', n>=2?'Сначала':'Следующая запятая','r610Rep()')}
        ${T('p',5,'cap', n>=2?'Две запятые на три «и». Первое «и» открывает ряд — перед ним знака нет.':'Нажимай: запятые появятся по одной и дорисуются.')}`;
    }
    if(f===6){
      const which=s.an||0;
      const EX=[['не груши','а','сливы'],['маленький','но','смелый']];
      const e=EX[which];
      return `${T('div',0,'kicker','06 · «А» и «но»')}
        ${T('h2',1,'','Эти союзы всегда спорят')}
        ${T('p',2,'lead','Перед <b style="color:#ffd76a">а</b> и <b style="color:#ffd76a">но</b> запятая ставится всегда. Они противопоставляют.')}
        <div class="line" data-anim style="--i:3">
          <span class="w">${e[0]}</span>
          <span class="slot">${cm(1)}</span>
          <span class="w" style="color:#ffd76a">${e[1]}</span>
          <span class="w on">${e[2]}</span>
        </div>
        <div class="row">${EX.map((x,k)=>BTN(4+k,which===k?'chip on':'chip',x[1],'r610An('+k+')')).join('')}</div>
        ${T('p',6,'cap','Не груши, а сливы. Маленький, но смелый. Спор — значит, пауза.')}`;
    }
    if(f===7){
      const K=[['подлежащие','книга, тетрадь, ручка лежат','что лежит?'],
               ['сказуемые','дети читают, рисуют и поют','что делают?'],
               ['дополнения','люблю яблоки, груши, сливы','люблю что?']];
      const i=s.kind||0, k=K[i];
      return `${T('div',0,'kicker','07 · Любые члены')}
        ${T('h2',1,'','Соседями бывают не только деревья')}
        ${T('p',2,'lead','Главное — <b style="color:#ffd76a">один вопрос</b>. Член предложения может быть любой.')}
        <div class="who" data-anim style="--i:3"><b>${k[1]}</b><i>${k[0]} · вопрос: ${k[2]}</i></div>
        <div class="row">${K.map((x,n)=>BTN(4+n,i===n?'chip on':'chip',x[0],'r610Kind('+n+')')).join('')}</div>
        ${T('p',7,'cap','Кликни роль — подставится своя фраза. Один экран, три разные картины.')}`;
    }
    if(f===8){
      const on=!!s.call;
      return `${T('div',0,'kicker','08 · Оклик')}
        ${T('h2',1,'','Эй, Маша! Это обращение')}
        ${T('p',2,'lead','Обращение — слово, которым называют того, <b style="color:#ffd76a">к кому говорят</b>. Не про кого — а к кому.')}
        <div class="line" data-anim style="--i:3">
          <span class="w ${on?'call':''}">Маша</span>
          <span class="slot">${on?cm(1):''}</span>
          <span class="w">помоги</span>
          <span class="w">мне</span>
        </div>
        ${BTN(4,'cta', on?'Убрать оклик':'Окликнуть Машу','r610Call()')}
        ${T('p',5,'cap', on?'Запятая после имени — как вдох перед просьбой.':'Пока имя не окликнули, это просто слово в ряду.')}`;
    }
    if(f===9){
      const pos=s.pos||0;
      const start = pos===0 ? `<span class="w call">Маша</span><span class="slot">${cm(1)}</span><span class="w">помоги</span><span class="w">мне</span>`
                  : pos===1 ? `<span class="w">Спасибо</span><span class="slot">${cm(1)}</span><span class="w call">Маша</span><span class="slot">${cm(1)}</span><span class="w">за помощь</span>`
                            : `<span class="w">Помоги</span><span class="w">мне</span><span class="slot">${cm(1)}</span><span class="w call">Маша</span>`;
      const cap = pos===0 ? 'В начале — одна запятая после имени.'
                : pos===1 ? 'В середине — две: имя обнимают с двух сторон.'
                          : 'В конце — одна запятая перед именем.';
      return `${T('div',0,'kicker','09 · Три места')}
        ${T('h2',1,'','Имя можно поставить куда угодно')}
        ${T('p',2,'lead','Меняется место — меняется число запятых. Смысл оклика тот же.')}
        <div class="line" data-anim style="--i:3">${start}</div>
        <div class="row posrow">
          ${BTN(4,pos===0?'chip on':'chip','в начале','r610Pos(0)')}
          ${BTN(5,pos===1?'chip on':'chip','в середине','r610Pos(1)')}
          ${BTN(6,pos===2?'chip on':'chip','в конце','r610Pos(2)')}
        </div>
        ${T('p',7,'cap',cap)}`;
    }
    if(f===10){
      const tryU=!!s.und;
      return `${T('div',0,'kicker','10 · Не член предложения')}
        ${T('h2',1,'','Обращение за партой не сидит')}
        ${T('p',2,'lead','Его нельзя подчеркнуть как подлежащее. В «Маша, помоги» подлежащее — скрытое <b style="color:#ffd76a">ты</b>.')}
        <div class="line" data-anim style="--i:3">
          <span class="w call" style="${tryU?'text-decoration:line-through;opacity:.55':''}">Маша</span>
          <span class="slot">${cm(1)}</span>
          <span class="w">помоги</span>
        </div>
        <div class="who" data-anim style="--i:4"><b>${tryU?'не вышло':'попробуй подчеркнуть'}</b><i>${tryU?'Маша — не подлежащее. Подлежащее здесь — ты.':'Нажми кнопку: знак не пристанет.'}</i></div>
        ${BTN(5,'cta', tryU?'Вернуть имя':'Подчеркнуть как подлежащее','r610Und()')}`;
    }
    if(f===11){
      const addr=!!s.trap;
      return `${T('div',0,'kicker','11 · Ловушка')}
        ${T('h2',1,'','Рассказ или просьба?')}
        ${T('p',2,'lead','Одно имя — две работы. Смотри на глагол, не на буквы.')}
        <div class="line" data-anim style="--i:3">
          <span class="w ${addr?'call':''}">Маша</span>
          ${addr?`<span class="slot">${cm(1)}</span><span class="w">помоги</span>`:'<span class="w">помогает</span>'}
        </div>
        ${BTN(4,'cta', addr?'Показать рассказ':'Показать просьбу','r610Trap()')}
        ${T('p',5,'cap', addr?'Просьба: Маша — обращение, запятая нужна.':'Рассказ: Маша помогает — подлежащее, запятой нет.')}`;
    }
    if(f===12){
      const show=s.stem!==0;
      return `${T('div',0,'kicker','12 · Две основы')}
        ${T('h2',1,'','Это уже сложное предложение')}
        ${T('p',2,'lead','«Светит солнце, и поют птицы». Основ две — значит, между частями нужна запятая.')}
        <div class="line" data-anim style="--i:3">
          <span class="w ${show?'stem':''}">Светит солнце</span>
          <span class="slot">${show?cm(1):''}</span>
          <span class="w" style="color:#7fd1ff">и</span>
          <span class="w ${show?'stem':''}">поют птицы</span>
        </div>
        <div class="stems">
          ${T('div',4,'box','<div class="n">первая основа</div><div class="t">солнце светит</div>')}
          ${T('div',5,'box','<div class="n">вторая основа</div><div class="t">птицы поют</div>')}
        </div>
        ${BTN(6,'cta', show?'Спрятать основы':'Показать две основы','r610Stem()')}`;
    }
    if(f===13){
      const two=!!s.vs;
      return `${T('div',0,'kicker','13 · Один «и», два дела')}
        ${T('h2',1,'','Сначала посчитай основы')}
        ${T('p',2,'lead','Одно и то же «и». Разный счёт основ — разный знак.')}
        <div class="line" data-anim style="--i:3">
          ${two
            ? `<span class="w stem">Светит солнце</span><span class="slot">${cm(1)}</span><span class="w" style="color:#7fd1ff">и</span><span class="w stem">поют птицы</span>`
            : `<span class="w on">яблони</span><span class="w" style="color:#7fd1ff;padding:10px 8px">и</span><span class="w on">груши</span>`}
        </div>
        ${BTN(4,'cta', two?'Одна основа: яблони и груши':'Две основы: солнце и птицы','r610Vs()')}
        ${T('p',5,'cap', two?'Две основы — запятая перед «и». Это сложное.':'Одна основа — «и» между однородными, запятой нет.')}`;
    }
    if(f===14){
      return `${T('div',0,'kicker','14 · Алгоритм')}
        ${T('h2',1,'','Четыре шага — и знак на месте')}
        <div class="steps">
          ${T('div',2,'st','<span class="n">1</span><span class="t">Найди однородных<small>один вопрос, одно общее слово</small></span>')}
          ${T('div',3,'st','<span class="n">2</span><span class="t">Посмотри на союзы<small>нет союза / повтор «и» / «а», «но» — запятая; одиночное «и» — нет</small></span>')}
          ${T('div',4,'st','<span class="n">3</span><span class="t">Найди обращение<small>оклик выделяют запятыми с той стороны, где речь</small></span>')}
          ${T('div',5,'st','<span class="n">4</span><span class="t">Посчитай основы<small>две и больше — запятая между частями</small></span>')}
        </div>
        ${T('p',6,'cap','Не ставь запятую «на всякий случай». Сначала вопрос, потом союз, потом счёт основ.')}`;
    }
    if(f===15){
      return `${T('div',0,'kicker','15 · В карман')}
        ${T('h2',1,'','Три фразы, которые стоит унести')}
        <div class="steps">
          ${T('div',2,'st','<span class="n">,</span><span class="t">Перечисление без союза<small>яблони, груши, сливы</small></span>')}
          ${T('div',3,'st','<span class="n">и</span><span class="t">Одиночное «и» — без запятой<small>яблони и груши</small></span>')}
          ${T('div',4,'st','<span class="n">эй</span><span class="t">Обращение всегда выделяют<small>Маша, помоги мне</small></span>')}
        </div>
        ${T('p',5,'cap','Дальше — сам: прочитай фразу и скажи, где знак.')}`;
    }
    const G=(window.RUGAME&&window.RUGAME[610])||[];
    const i=(s.gIdx||0)%(G.length||1), it=G[i]||['','',''], got=s.gRes, done=got!=null;
    const btns=[['нет','запятые не нужны'],['одна','одна запятая'],['две','две запятые'],['зап','перечисление'],['перед а','перед «а»'],['слож','две основы']];
    return `${T('div',0,'kicker','16 · Тренажёр')}
      <div class="split">${T('span',1,'word wide',`<b>${it[0]}</b><i>где нужны запятые?</i>`)}</div>
      ${done
        ? (got ? window.RUFEED.note('ok','верно',it[2])
               : window.RUFEED.note('no','исправить','Правильно: '+it[2]))
        : `<p class="verdict">Найди соседей или оклик и выбери ответ.</p>`}
      <div class="casebtns">${btns.map((b,k)=>
        `<button type="button" data-anim style="--i:${2+k}" class="${done&&it[1]===b[0]?(got?'hit':''):''}" onclick="ru610Game('${b[0]}')"><span class="nm">${b[1]}</span><span class="qs">${b[0]==='зап'?'без союзов':(b[0]==='слож'?'две основы':'знак')}</span></button>`).join('')}</div>
      <p class="score">верно: ${s.gOk||0} · ошибок: ${s.gBad||0} · всего: ${G.length}</p>
      <p class="cap">${done?'Нажми любую кнопку — следующая фраза.':'Сначала вопрос, потом союз, потом основы.'}</p>`;
  }
  ...
  function render(el){
    ...
    const st=(typeof LV!=='undefined'&&LV.step)||0;
    const f=Math.min(16, Math.max(1, st+1));
    el.innerHTML=`<div class="s6 l610" data-frame="${f}">${pred(f, body(s,f))}</div>`;
  }

  window.r610Pred=(f,k)=>{ S()['q'+f]=k; chRender(0); };
  window.r610Tap=()=>{ const s=S(); s.tap=s.tap?0:1; chRender(0); };
  window.r610Hit=(w)=>{ S().hit=w; chRender(0); };
  window.r610List=()=>{ const s=S(); s.list=(s.list===0)?1:0; chRender(0); };
  window.r610Un=()=>{ const s=S(); s.un=(s.un===0)?1:0; chRender(0); };
  window.r610Rep=()=>{ const s=S(); s.rep=((s.rep||0)+1)%3; chRender(0); };
  window.r610An=(i)=>{ S().an=i; chRender(0); };
  window.r610Kind=(i)=>{ S().kind=i; chRender(0); };
  window.r610Call=()=>{ const s=S(); s.call=s.call?0:1; chRender(0); };
  window.r610Pos=(i)=>{ S().pos=i; chRender(0); };
  window.r610Und=()=>{ const s=S(); s.und=s.und?0:1; chRender(0); };
  window.r610Trap=()=>{ const s=S(); s.trap=(s.trap===0)?1:0; chRender(0); };
  window.r610Stem=()=>{ const s=S(); s.stem=(s.stem===0)?1:0; chRender(0); };
  window.r610Vs=()=>{ const s=S(); s.vs=s.vs?0:1; chRender(0); };

  if(window.WAVE_B){
    const prev=window.WAVE_B[610];
    window.WAVE_B[610]=function(el){ try{ render(el); }catch(e){ try{ prev(el); }catch(e2){} } };
  }
  return {render:render};
})();

/* ================= УРОКИ 603 и 604 заново · 16 кадров, живой язык =================
   Как 601 и 610: свой рендерер поверх WAVE_B, у каждого кадра своя сцена и своё
   движение, вопрос-проверка внутри кадра, тренажёр — RULETTER (буква влетает в слово).
   Методика — Ладыженская, 5 класс: §8 проверяемые безударные, §9 непроверяемые;
   парные согласные, непроизносимые, удвоенные. Не выдумываем этимологий.
   «Молоко» — словарное, не проверяем через «молочный». */
window.RU603V2 = (function(){
  const CSS=`
  #lvis .s6.l603{gap:14px}
  #lvis .s6.l603 .rw{gap:10px}
  #lvis .s6.l603 .rw-h{font-size:14px;letter-spacing:.08em}
  #lvis .s6.l603 .rw-lab{font-size:14px;color:var(--gold)}
  #lvis .s6.l603 .rw-w{font-size:16px;line-height:1.2;padding:10px 12px;min-height:44px;display:inline-flex;align-items:center}
  #lvis .s6.l603 .rw-note{font-size:14px;line-height:1.45}
  #lvis .s6.l603 .echo{display:flex;gap:10px;flex-wrap:wrap;justify-content:center;width:100%}
  #lvis .s6.l603 .echo button{flex:1 1 42%;min-height:72px;padding:12px 14px;display:flex;flex-direction:column;align-items:center;gap:4px;font-size:16px}
  #lvis .s6.l603 .echo button b{font-size:32px;line-height:1.1;color:var(--gold)}
  #lvis .s6.l603 .echo button i{font-style:normal;font-size:14px;color:var(--mut)}
  #lvis .s6.l603 .ipa{width:100%;text-align:center;font-size:24px;font-weight:600;color:var(--gold);padding:8px 0}
  #lvis .s6.l603 .lamp{display:flex;flex-direction:column;align-items:center;gap:2px}
  #lvis .s6.l603 .lamp .mark{font-size:32px;color:var(--gold);line-height:1;min-height:32px}
  #lvis .s6.l603 .askrow{display:flex;gap:10px;flex-wrap:wrap;justify-content:stretch}
  #lvis .s6.l603 .askrow button{flex:1 1 42%;min-height:48px;padding:12px;font-size:16px;font-weight:600;line-height:1.25}
  #lvis .s6.l603 .fam{display:grid;grid-template-columns:1fr 1fr;gap:10px;width:100%}
  #lvis .s6.l603 .fam button{display:flex;flex-direction:column;align-items:flex-start;gap:2px;min-height:56px;padding:12px 14px;text-align:left;font-size:16px}
  #lvis .s6.l603 .fam button b{font-size:20px;line-height:1.15;color:var(--gold)}
  #lvis .s6.l603 .fam button i{font-style:normal;font-size:14px;color:var(--mut)}
  #lvis .s6.l603 .job{display:flex;flex-direction:column;align-items:center;gap:4px;padding:14px 16px;border-radius:16px;
    background:linear-gradient(180deg,#22362c,#17261e);border:1.5px solid var(--line);text-align:center}
  #lvis .s6.l603 .job b{font-size:24px;line-height:1.15;color:var(--gold)}
  #lvis .s6.l603 .job i{font-style:normal;font-size:16px;line-height:1.35;color:var(--mut)}
  #lvis .s6.l603 .acc{color:var(--gold);font-weight:700}
  #lvis .s6.l603 .word{flex-direction:column;align-items:center;gap:4px;white-space:normal;max-width:100%}
  #lvis .s6.l603 .word i{white-space:normal;text-align:center}
  #lvis .s6.l603 .card.on{border-color:var(--gold);background:rgba(255,215,106,.12)}
  #lvis .s6.l603 .nb{display:flex;flex-direction:column;width:100%;border-radius:16px;border:1.5px solid var(--line);overflow:hidden}
  #lvis .s6.l603 .nb button{min-height:56px;border-radius:0;border:none;border-bottom:1px dashed var(--line);text-align:left;padding:14px 16px;font-size:16px}
  #lvis .s6.l603 .nb button:last-child{border-bottom:none}
  #lvis .s6.l603 .nb button b{display:block;font-size:16px;color:var(--gold)}
  #lvis .s6.l603 .nb button i{display:block;font-style:normal;font-size:14px;color:var(--mut);margin-top:2px}
  #lvis .s6.l603 .pkt{display:flex;flex-direction:column;gap:10px;width:100%}
  #lvis .s6.l603 .pkt button{min-height:56px;text-align:left;padding:14px 16px;font-size:16px}
  #lvis .s6.l603 .pkt button b{display:block;font-size:20px;color:var(--gold);line-height:1.2}
  #lvis .s6.l603 .pkt button i{display:block;font-style:normal;font-size:14px;color:var(--mut);margin-top:4px}
  #lvis .s6.l603 .steps .st.dim{opacity:.4}
  #lvis .s6.l603 .steps .st.on{border-color:var(--gold)}
  #lvis .s6.l603[data-frame="1"] [data-anim]{animation-name:l603f1}
  #lvis .s6.l603[data-frame="2"] [data-anim]{animation-name:l603f2}
  #lvis .s6.l603[data-frame="3"] [data-anim]{animation-name:l603f3}
  #lvis .s6.l603[data-frame="4"] [data-anim]{animation-name:l603f4}
  #lvis .s6.l603[data-frame="5"] [data-anim]{animation-name:l603f5}
  #lvis .s6.l603[data-frame="6"] [data-anim]{animation-name:l603f6}
  #lvis .s6.l603[data-frame="7"] [data-anim]{animation-name:l603f7}
  #lvis .s6.l603[data-frame="8"] [data-anim]{animation-name:l603f8}
  #lvis .s6.l603[data-frame="9"] [data-anim]{animation-name:l603f9}
  #lvis .s6.l603[data-frame="10"] [data-anim]{animation-name:l603f10}
  #lvis .s6.l603[data-frame="11"] [data-anim]{animation-name:l603f11}
  #lvis .s6.l603[data-frame="12"] [data-anim]{animation-name:l603f12}
  #lvis .s6.l603[data-frame="13"] [data-anim]{animation-name:l603f13}
  #lvis .s6.l603[data-frame="14"] [data-anim]{animation-name:l603f14}
  #lvis .s6.l603[data-frame="15"] [data-anim]{animation-name:l603f15}
  #lvis .s6.l603[data-frame="16"] [data-anim]{animation-name:l603f16}
  @keyframes l603f1{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:none}}
  @keyframes l603f2{0%{opacity:0;transform:scale(.94)}100%{opacity:1;transform:none}}
  @keyframes l603f3{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:none}}
  @keyframes l603f4{from{opacity:0;transform:translateX(-14px)}to{opacity:1;transform:none}}
  @keyframes l603f5{from{opacity:0;transform:translateX(14px)}to{opacity:1;transform:none}}
  @keyframes l603f6{0%{opacity:0;transform:scale(.92)}70%{transform:scale(1.03)}100%{opacity:1;transform:none}}
  @keyframes l603f7{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:none}}
  @keyframes l603f8{from{opacity:0;transform:translateX(-12px)}to{opacity:1;transform:none}}
  @keyframes l603f9{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:none}}
  @keyframes l603f10{0%{opacity:0;transform:scale(.96)}100%{opacity:1;transform:none}}
  @keyframes l603f11{from{opacity:0;transform:translateX(12px)}to{opacity:1;transform:none}}
  @keyframes l603f12{from{opacity:0;transform:translateY(-10px)}to{opacity:1;transform:none}}
  @keyframes l603f13{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:none}}
  @keyframes l603f14{from{opacity:0;transform:translateX(-10px)}to{opacity:1;transform:none}}
  @keyframes l603f15{0%{opacity:0;transform:scale(.94)}100%{opacity:1;transform:none}}
  @keyframes l603f16{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:none}}
  @media (max-width:370px){
    #lvis .s6.l603 .fam{grid-template-columns:1fr}
    #lvis .s6.l603 .echo button b{font-size:24px}
    #lvis .s6.l603 .word b{font-size:32px}
    #lvis .s6.l603 .split{flex-direction:column}
  }
  @media (prefers-reduced-motion: reduce){
    #lvis .s6.l603 [data-anim]{animation:none!important}
  }`;
  function css(){
    try{
      if(window.RUKIT&&RUKIT.frameCss) RUKIT.frameCss();
      let e=document.getElementById('l603-style');
      if(!e){ e=document.createElement('style'); e.id='l603-style'; document.head.appendChild(e); }
      if(e.textContent!==CSS) e.textContent=CSS;
    }catch(e){}
  }
  const S=()=>{ const lk=lidKey(603); if(typeof CHS==='undefined') window.CHS={}; if(!CHS[lk]) CHS[lk]={}; return CHS[lk]; };
  const T=(tag,i,cls,html)=>`<${tag} data-anim style="--i:${i}" class="${cls||''}">${html}</${tag}>`;
  const BTN=(i,cls,html,on)=>`<button type="button" data-anim style="--i:${i}" class="${cls}" onclick="${on}">${html}</button>`;
  const G=[['м_лодой','о','мОлод — молодость'],['л_сной','е','лЕс'],['г_ра','о','гОры'],
           ['п_сьмо','и','пИсьма'],['з_мля','е','зЕмли'],['х_лодный','о','хОлод']];
  window.RUGAME=window.RUGAME||{}; window.RUGAME[603]=G;
  const P={
    2:['Почему «лиса» и «леса» путают?',[{k:'a',t:'без ударения звучат похоже',ok:1,fb:'верно: гласная теряет ясность'},{k:'b',t:'буквы рядом на клавиатуре',ok:0,fb:'дело не в клавишах — в слухе'}]],
    3:['Что делает ударение?',[{k:'a',t:'делает гласную ясной',ok:1,fb:'верно: лЕс слышно сразу'},{k:'b',t:'меняет смысл слова',ok:0,fb:'смысл тот же, ясность другая'}]],
    4:['Как проверить безударную в корне?',[{k:'a',t:'поставить её под ударение',ok:1,fb:'верно: лиса — лИс'},{k:'b',t:'посмотреть на соседнюю букву',ok:0,fb:'соседняя буква не подскажет'}]],
    5:['Проверка для «гора»?',[{k:'a',t:'гОры или гОрный',ok:1,fb:'верно: форма и родственник'},{k:'b',t:'горе',ok:0,fb:'«горе» — другое слово по смыслу'}]],
    6:['Проверка для «письмо»?',[{k:'a',t:'пИсьма',ok:1,fb:'верно: и под ударением'},{k:'b',t:'писать',ok:0,fb:'в «писать» та же гласная снова без ударения'}]],
    7:['«Вода — водитель» можно проверить?',[{k:'a',t:'нет: разный смысл',ok:1,fb:'верно: река и руль — не родня'},{k:'b',t:'да: корень один',ok:0,fb:'буквы похожи, смысл разный'}]],
    8:['Какая пара — родня?',[{k:'a',t:'земля — земли',ok:1,fb:'верно: одна семья, е под ударением'},{k:'b',t:'ложка — ложь',ok:0,fb:'ложка — про еду, ложь — про неправду'}]],
    9:['Что делать со словарным словом?',[{k:'a',t:'запомнить или открыть словарь',ok:1,fb:'верно: проверки ударением нет'},{k:'b',t:'подобрать любое похожее',ok:0,fb:'похожее по звуку — ещё не родственник'}]],
    10:['«Земля» — какая буква?',[{k:'a',t:'е: зЕмли',ok:1,fb:'верно: е под ударением'},{k:'b',t:'и: зИмли',ok:0,fb:'зИмля — ошибка'}]],
    11:['Зачем произносить проверку вслух?',[{k:'a',t:'под ударением буква слышна',ok:1,fb:'верно: ушам можно верить, когда есть ударение'},{k:'b',t:'чтобы слово стало громче',ok:0,fb:'громкость не меняет букву'}]],
    12:['«Молодой» проверяем…',[{k:'a',t:'мОлод, молодость',ok:1,fb:'верно: о под ударением'},{k:'b',t:'малый',ok:0,fb:'малый — другое слово'}]],
    13:['Первый шаг алгоритма — …',[{k:'a',t:'найти корень и ударение',ok:1,fb:'верно: без корня нечего проверять'},{k:'b',t:'посчитать слоги',ok:0,fb:'слоги тут ни при чём'}]],
    14:['Нет проверочного слова. Что делать?',[{k:'a',t:'смотреть словарь',ok:1,fb:'верно: это словарное'},{k:'b',t:'написать как слышится',ok:0,fb:'на слух как раз и ошибаются'}]],
    15:['Три вещи в карман. Первая — …',[{k:'a',t:'ударение проверяет гласную',ok:1,fb:'верно: фонарик всего правила'},{k:'b',t:'писать как удобно',ok:0,fb:'удобство — не правило'}]]
  };
  function pred(f,body){
    const p=P[f]; if(!p) return body;
    const s=S(), cur=s['q'+f];
    ...
    const fb=cur
      ? `<p class="verdict ${pick&&pick.ok?'ok':'no'}">${pick&&pick.ok?'верно. '+pick.fb:'не так. '+(pick?pick.fb:'подумай ещё')}</p>`
      : `<p class="verdict">Выбери ответ:</p>`;
    return body+`<div class="askrow">${p[1].map((o,i)=>BTN(8+i,'chip'+(cur===o.k?(o.ok?' ok':' no'):''),o.t,`r603Pred(${f},'${o.k}')`)).join('')}</div>`+fb;
  }
  function body(s,f){
    if(f===1){
      return `${T('div',0,'kicker','01 · Слова урока')}
        ${T('h2',1,'','Сначала — список, который правило не возьмёт')}
        ${T('p',2,'lead','Эти слова запоминают. Посмотри, какая гласная в каком слове опасна.')}
        ${typeof ruWords603==='function'?ruWords603():''}`;
    }
    if(f===2){
      const on=s.hear||'';
      return `${T('div',0,'kicker','02 · Слышим одно')}
        ${T('h2',1,'','Скажи вслух «лиса» и «леса»')}
        ${T('p',2,'lead','Разницы нет. Без ударения гласная теряет ясность — на слух букву не выбрать.')}
        <div class="echo">
          ${BTN(3,on==='лиса'?'chip on':'chip','<b>лиса</b><i>зверь</i>',`r603Hear('лиса')`)}
          ${BTN(4,on==='леса'?'chip on':'chip','<b>леса</b><i>деревья</i>',`r603Hear('леса')`)}
        </div>
        ${on?T('div',5,'ipa','оба: [л’иса]'):T('p',5,'cap','Нажми слово. Услышишь одно и то же — а пишутся по-разному.')}`;
    }
    if(f===3){
      const on=!!s.str;
      return `${T('div',0,'kicker','03 · Фонарик')}
        ${T('h2',1,'','Поставь ударение — и буква перестанет спорить')}
        <div class="split">
          <div class="lamp" data-anim style="--i:2">
            <span class="mark">${on?'´':''}</span>
            ${T('span',2,'word', on?'<b>л<span class="acc">е</span>с</b><i>слышно сразу</i>':'<b>лес</b><i>пока без ударения</i>')}
          </div>
          ${T('span',3,'word','<b>лесной</b><i>та же буква, уже сомнительно</i>')}
        </div>
        ${BTN(4,'cta', on?'Убрать ударение':'Поставить ударение',`r603Str()`)}
        ${T('p',5,'cap','Одна семья: лес — лесной. Фонарик зажёгся — и е больше не прячется.')}`;
    }
    if(f===4){
      return `${T('div',0,'kicker','04 · Правило')}
        ${T('h2',1,'','Безударную в корне проверяй ударением')}
        ${T('p',2,'lead','Подбери слово, где <b style="color:#ffd76a">та же гласная</b> стоит под ударением.')}
        ${window.RUKIT&&RUKIT.frameMorph?RUKIT.frameMorph([['л','#8fd1a8','корень'],['Ис','#ffd76a','ударная'],['а','#e8a0d8','']]):''}
        <div class="row">${T('span',4,'tag','лиса → лИс')}${T('span',5,'tag','гора → гОры')}${T('span',6,'tag','письмо → пИсьма')}</div>
        ${T('p',7,'cap','Написал ту букву, которую услышал под ударением. Всё правило — в этой фразе.')}`;
    }
    if(f===5){
      const way=s.way||0;
      return `${T('div',0,'kicker','05 · Два пути')}
        ${T('h2',1,'','Форма слова или родственник')}
        ${T('p',2,'lead','Корень один — буква одна. Два способа довести гласную до ударения.')}
        <div class="pair">
          ${T('div',3,'card'+(way===1?' on':''),'<div class="nm">измени форму</div><div class="ex">гора — гОры<br>окно — Окна<br>стена — стЕны</div>')}
          ${T('div',4,'card'+(way===2?' on':''),'<div class="nm">найди родственника</div><div class="ex">гора — гОрный<br>окно — окОнный<br>лесной — лЕс</div>')}
        </div>
        <div class="row">${BTN(5,way===1?'chip on':'chip','показать форму',`r603Way(1)`)}${BTN(6,way===2?'chip on':'chip','показать родственника',`r603Way(2)`)}</div>
        ${T('p',7,'cap', way===1?'Гора — горы: то же слово, только во множественном. О вышло под ударение.':(way===2?'Горный — тот же корень, другое слово. О снова под ударением.':'Нажми путь — увидишь, как буква выходит на свет.'))}`;
    }
    if(f===6){
      const pick=s.chk;
      const ok=pick==='письма';
      return `${T('div',0,'kicker','06 · Подбери сам')}
        ${T('h2',1,'','Какое слово проверит «письмо»?')}
        <div class="split">${T('span',2,'word','<b>п_сьмо</b><i>какая буква?</i>')}</div>
        <div class="row">
          ${BTN(3,pick==='письма'?'chip ok':'chip','пИсьма',`r603Chk('письма')`)}
          ${BTN(4,pick==='писать'?'chip no':'chip','писать',`r603Chk('писать')`)}
          ${BTN(5,pick==='песок'?'chip no':'chip','песок',`r603Chk('песок')`)}
        </div>
        ${pick?`<div class="job" data-anim style="--i:8"><b>${ok?'пИсьма':(pick==='писать'?'писать — гласная снова безударная':'песок — другой корень')}</b><i>${ok?'И под ударением. Пишем и.':'Это не проверка.'}</i></div>`
              :T('p',8,'cap','Проверочное — то, где сомнительная гласная стала ударной.')}`;
    }
    if(f===7){
      const t=s.trap;
      return `${T('div',0,'kicker','07 · Ловушка смысла')}
        ${T('h2',1,'','Похожие — ещё не родня')}
        ${T('p',2,'lead','«Вода» про реку. «Водитель» про руль. Проверять одно другим нельзя.')}
        <div class="pair">
          ${T('div',3,'card'+(t==='вода'?' on':''),'<div class="nm">вода</div><div class="ex">вОды — вот проверка<br>река, вода, водяной</div>')}
          ${T('div',4,'card'+(t==='вод'?' on':''),'<div class="nm">водитель</div><div class="ex">от «водить»<br>руль, не река</div>')}
        </div>
        <div class="row">${BTN(5,t==='вода'?'chip ok':'chip','вода — родня',`r603Trap('вода')`)}${BTN(6,t==='вод'?'chip no':'chip','водитель — родня?',`r603Trap('вод')`)}</div>
        ${T('p',7,'cap', t==='вода'?'Воды, водяной, подводный — одна семья. Пишем о.':(t==='вод'?'Водитель — от «водить». Другой смысл, другая проверка.':'Кликни. Смотри на смысл, не на первые буквы.'))}`;
    }
    if(f===8){
      const p=s.kin;
      return `${T('div',0,'kicker','08 · Кто родня')}
        ${T('h2',1,'','Проверочное обязано быть родственником')}
        <div class="fam">
          ${BTN(2,p==='земля'?'chip ok':'chip','<b>земля — земли</b><i>одна семья</i>',`r603Kin('земля')`)}
          ${BTN(3,p==='ложка'?'chip no':'chip','<b>ложка — ложь</b><i>еда и неправда</i>',`r603Kin('ложка')`)}
          ${BTN(4,p==='горе'?'chip no':'chip','<b>гора — горе</b><i>холм и печаль</i>',`r603Kin('горе')`)}
          ${BTN(5,p==='холод'?'chip ok':'chip','<b>холодный — холод</b><i>одна семья</i>',`r603Kin('холод')`)}
        </div>
        ${p?`<div class="job" data-anim style="--i:8"><b>${p==='земля'||p==='холод'?'Родня. Можно проверять.':'Чужие. Проверять нельзя.'}</b><i>${p==='земля'?'зЕмли — пишем е.':(p==='холод'?'хОлод — пишем о.':(p==='ложка'?'Ложка — ложечка, а не ложь.':'Гора — горы, а не горе.'))}</i></div>`
           :T('p',8,'cap','Нажми пару. Родня — общий смысл, не общие буквы.')}`;
    }
    if(f===9){
      const W=['собака','корова','ворона','молоко','работа'];
      const why={собака:'со-ба-ка: о не проверить',корова:'ко-ро-ва: о не проверить',ворона:'во-ро-на: о не проверить',молоко:'мо-ло-ко: о не проверить — это словарное',работа:'ра-бо-та: о не проверить'};
      const cur=s.dic;
      return `${T('div',0,'kicker','09 · Словарные')}
        ${T('h2',1,'','Есть слова без фонарика')}
        ${T('p',2,'lead','Проверочного нет. Их запоминают или смотрят в словаре.')}
        <div class="row">${W.map((w,i)=>BTN(3+i,cur===w?'chip on':'chip',w,`r603Dic('${w}')`)).join('')}</div>
        ${cur?`<div class="job" data-anim style="--i:9"><b>${cur}</b><i>${why[cur]||'проверки ударением нет — запомни целиком'}</i></div>`
             :T('p',8,'cap','Нажми слово. Правило сюда не дотягивается. Не ищи «похожие» — их нет.')}`;
    }
    if(f===10){
      const EX=[{w:'голова',c:'гОловы',l:'о'},{w:'земля',c:'зЕмли',l:'е'},{w:'пятак',c:'пЯть',l:'я'}];
      const i=s.ex||0, a=EX[i], show=!!s.exOn;
      return `${T('div',0,'kicker','10 · Разбор')}
        ${T('h2',1,'','Кликни слово — проверка выйдет сама')}
        <div class="split">${T('span',2,'word', show?`<b>${a.w}</b><i>${a.c} → пишем ${a.l}</i>`:`<b>${a.w}</b><i>пока без проверки</i>`)}</div>
        <div class="row">${EX.map((x,k)=>BTN(3+k,i===k?'chip on':'chip',x.w,`r603Ex(${k})`)).join('')}</div>
        ${BTN(6,'cta', show?'Скрыть проверку':'Показать проверку',`r603ExOn()`)}
        ${T('p',7,'cap','Каждый раз одно и то же: ищем слово, где гласная станет ударной.')}`;
    }
    if(f===11){
      const pair=s.pair||0;
      const pairs=[['о','а','гОры, не гАра'],['е','и','зЕмли, не зИмля']];
      const a=pairs[pair];
      return `${T('div',0,'kicker','11 · Вслух')}
        ${T('h2',1,'','«а» вместо «о», «и» вместо «е»')}
        ${T('p',2,'lead','Спасение одно: произнеси проверочное <b style="color:#ffd76a">вслух</b>. Под ударением буква не врёт.')}
        <div class="split">
          ${T('span',3,'word',`<b>${a[0]}</b>`)}
          ${T('span',4,'tag','или')}
          ${T('span',5,'word',`<b>${a[1]}</b>`)}
        </div>
        <div class="row">${BTN(6,pair===0?'chip on':'chip','о / а',`r603Pair(0)`)}${BTN(7,pair===1?'chip on':'chip','е / и',`r603Pair(1)`)}</div>
        ${T('p',8,'cap', a[2]+'. Услышал под ударением — и написал.')}`;
    }
    if(f===12){
      const pick=s.mol;
      return `${T('div',0,'kicker','12 · Проба пера')}
        ${T('h2',1,'','м_лодой — какая буква?')}
        <div class="split">${T('span',2,'word','<b>м_лодой</b><i>подбери проверку</i>')}</div>
        <div class="row">
          ${BTN(3,pick==='о'?'chip ok':'chip','о',`r603Mol('о')`)}
          ${BTN(4,pick==='а'?'chip no':'chip','а',`r603Mol('а')`)}
        </div>
        ${pick?`<div class="job" data-anim style="--i:6"><b>${pick==='о'?'мОлод, молодость. Пишем о.':'малый — другое слово. Это не проверка.'}</b><i>${pick==='о'?'О вышло под ударение — и спор кончился.':'Ищи родственника по смыслу, не по первым буквам.'}</i></div>`
              :T('p',6,'cap','Не угадывай. Сначала проверка, потом буква.')}`;
    }
    if(f===13){
      const n=s.alg||0;
      const ST=[
        ['1','Найди корень','сомнительная гласная живёт в корне, не в приставке'],
        ['2','Падает ли ударение?','если да — пиши как слышишь; если нет — дальше'],
        ['3','Подбери проверку','форму слова или родственника по смыслу'],
        ['4','Напиши ту букву','которую услышал под ударением. Нет проверки — словарь']
      ];
      return `${T('div',0,'kicker','13 · Алгоритм')}
        ${T('h2',1,'','Четыре шага — и буква на месте')}
        ...
        ${BTN(6,'cta', n>=4?'Сначала':'Следующий шаг',`r603Alg()`)}
        ${T('p',7,'cap', n?('Шаг '+n+' из 4. Не ставь букву «на слух» — сначала корень, потом фонарик.'):'Нажимай: шаги загорятся по одному.')}`;
    }
    if(f===14){
      const i=s.nb||0;
      const L=[
        ['Проговори по слогам','со-ба-ка, ко-ро-ва — веди пальцем по слову'],
        ['Запомни опасную букву','крупно, в цвете, в своей тетради'],
        ['Открой словарь','сомневаешься — не угадывай']
      ];
      return `${T('div',0,'kicker','14 · Если проверки нет')}
        ${T('h2',1,'','Тетрадь для словарного')}
        ${T('p',2,'lead','Слово запоминают целиком. Три приёма — три строки.')}
        <div class="nb">${L.map((x,k)=>BTN(3+k,i===k+1?'chip on':'chip',
          i===k+1?`<b>${x[0]}</b><i>${x[1]}</i>`:`<b>${x[0]}</b><i>нажми — откроется строка</i>`,`r603Nb(${k+1})`)).join('')}</div>
        ${T('p',6,'cap','Не по одной букве «на удачу». Словарное держат целиком.')}`;
    }
    if(f===15){
      const i=s.pkt||0;
      const C=[
        ['´  Ударение проверяет гласную','лиса — лИс, гора — гОры'],
        ['≈  Родственник — по смыслу','вода и водитель — не родня'],
        ['§  Нет проверки — словарь','собака, корова, молоко, работа']
      ];
      return `${T('div',0,'kicker','15 · В карман')}
        ${T('h2',1,'','Три фразы, которые стоит унести')}
        <div class="pkt">${C.map((x,k)=>BTN(2+k,i===k+1?'chip on':'chip',
          `<b>${x[0]}</b>${i===k+1?`<i>${x[1]}</i>`:''}`,`r603Pkt(${k+1})`)).join('')}</div>
        ${T('p',5,'cap', i?'Держи эти три. Дальше сам: вставь букву. Она прилетит в слово.':'Нажми карточку — фраза перевернётся.')}`;
    }
    return '';
  }
  function render(el){
    ...
    const st=(typeof LV!=='undefined'&&LV.step)||0;
    const f=Math.min(16, Math.max(1, st+1));
    if(f===16 && window.RULETTER&&RULETTER.draw){
      const box=document.createElement('div');
      try{ RULETTER.draw(box,603); }catch(e){ box.innerHTML=''; }
      el.innerHTML=`<div class="s6 l603" data-frame="16">${T('div',0,'kicker','16 · Тренажёр')}${box.innerHTML}</div>`;
      return;
    }
    el.innerHTML=`<div class="s6 l603" data-frame="${f}">${pred(f, body(s,f))}</div>`;
  }
  window.r603Pred=(f,k)=>{ S()['q'+f]=k; chRender(0); };
  window.r603Hear=(w)=>{ S().hear=w; chRender(0); };
  window.r603Str=()=>{ const s=S(); s.str=s.str?0:1; chRender(0); };
  window.r603Way=(n)=>{ S().way=n; chRender(0); };
  window.r603Chk=(w)=>{ S().chk=w; chRender(0); };
  window.r603Trap=(w)=>{ S().trap=w; chRender(0); };
  window.r603Kin=(w)=>{ S().kin=w; chRender(0); };
  window.r603Dic=(w)=>{ S().dic=w; chRender(0); };
  window.r603Ex=(i)=>{ S().ex=i; S().exOn=1; chRender(0); };
  window.r603ExOn=()=>{ const s=S(); s.exOn=s.exOn?0:1; chRender(0); };
  window.r603Pair=(i)=>{ S().pair=i; chRender(0); };
  window.r603Mol=(k)=>{ S().mol=k; chRender(0); };
  window.r603Alg=()=>{ const s=S(); s.alg=s.alg>=4?0:(s.alg||0)+1; chRender(0); };
  window.r603Nb=(i)=>{ S().nb=i; chRender(0); };
  window.r603Pkt=(i)=>{ S().pkt=i; chRender(0); };
  if(window.WAVE_B){
    const prev=window.WAVE_B[603];
    window.WAVE_B[603]=function(el){ try{ render(el); }catch(e){ try{ prev(el); }catch(e2){} } };
  }
  return {render:render};
})();

window.RU604V2 = (function(){
  const CSS=`
  #lvis .s6.l604{gap:14px}
  #lvis .s6.l604 .rw{gap:10px}
  #lvis .s6.l604 .rw-h{font-size:14px;letter-spacing:.08em}
  #lvis .s6.l604 .rw-lab{font-size:14px;color:var(--gold)}
  #lvis .s6.l604 .rw-w{font-size:16px;line-height:1.2;padding:10px 12px;min-height:44px;display:inline-flex;align-items:center}
  #lvis .s6.l604 .rw-note{font-size:14px;line-height:1.45}
  #lvis .s6.l604 .hear{display:flex;gap:10px;flex-wrap:wrap;justify-content:center;width:100%}
  #lvis .s6.l604 .hear button{flex:1 1 42%;min-height:72px;padding:12px 14px;display:flex;flex-direction:column;align-items:center;gap:4px;font-size:16px}
  #lvis .s6.l604 .hear button b{font-size:32px;line-height:1.1;color:var(--gold)}
  #lvis .s6.l604 .hear button i{font-style:normal;font-size:14px;color:var(--mut)}
  #lvis .s6.l604 .pairs{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;width:100%}
  #lvis .s6.l604 .pairs button{min-height:52px;font-size:20px;font-weight:600}
  #lvis .s6.l604 .askrow{display:flex;gap:10px;flex-wrap:wrap;justify-content:stretch}
  #lvis .s6.l604 .askrow button{flex:1 1 42%;min-height:48px;padding:12px;font-size:16px;font-weight:600;line-height:1.25}
  #lvis .s6.l604 .job{display:flex;flex-direction:column;align-items:center;gap:4px;padding:14px 16px;border-radius:16px;
    background:linear-gradient(180deg,#22362c,#17261e);border:1.5px solid var(--line);text-align:center}
  #lvis .s6.l604 .job b{font-size:24px;line-height:1.15;color:var(--gold)}
  #lvis .s6.l604 .job i{font-style:normal;font-size:16px;line-height:1.35;color:var(--mut)}
  #lvis .s6.l604 .ghost{opacity:.4}
  #lvis .s6.l604 .word{flex-direction:column;align-items:center;gap:4px;white-space:normal;max-width:100%}
  #lvis .s6.l604 .word i{white-space:normal;text-align:center}
  #lvis .s6.l604 .pull{display:flex;align-items:center;gap:8px;flex-wrap:wrap;justify-content:center}
  #lvis .s6.l604 .card.on{border-color:var(--gold);background:rgba(255,215,106,.12)}
  #lvis .s6.l604 .mute{text-decoration:line-through;opacity:.45}
  #lvis .s6.l604 .pkt{display:flex;flex-direction:column;gap:10px;width:100%}
  #lvis .s6.l604 .pkt button{min-height:56px;text-align:left;padding:14px 16px;font-size:16px}
  #lvis .s6.l604 .pkt button b{display:block;font-size:20px;color:var(--gold);line-height:1.2}
  #lvis .s6.l604 .pkt button i{display:block;font-style:normal;font-size:14px;color:var(--mut);margin-top:4px}
  #lvis .s6.l604 .steps .st.dim{opacity:.4}
  #lvis .s6.l604 .steps .st.on{border-color:var(--gold)}
  #lvis .s6.l604 .son{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;width:100%}
  #lvis .s6.l604 .son button{min-height:52px;font-size:24px;font-weight:600}
  #lvis .s6.l604[data-frame="1"] [data-anim]{animation-name:l604f1}
  #lvis .s6.l604[data-frame="2"] [data-anim]{animation-name:l604f2}
  #lvis .s6.l604[data-frame="3"] [data-anim]{animation-name:l604f3}
  #lvis .s6.l604[data-frame="4"] [data-anim]{animation-name:l604f4}
  #lvis .s6.l604[data-frame="5"] [data-anim]{animation-name:l604f5}
  #lvis .s6.l604[data-frame="6"] [data-anim]{animation-name:l604f6}
  #lvis .s6.l604[data-frame="7"] [data-anim]{animation-name:l604f7}
  #lvis .s6.l604[data-frame="8"] [data-anim]{animation-name:l604f8}
  #lvis .s6.l604[data-frame="9"] [data-anim]{animation-name:l604f9}
  #lvis .s6.l604[data-frame="10"] [data-anim]{animation-name:l604f10}
  #lvis .s6.l604[data-frame="11"] [data-anim]{animation-name:l604f11}
  #lvis .s6.l604[data-frame="12"] [data-anim]{animation-name:l604f12}
  #lvis .s6.l604[data-frame="13"] [data-anim]{animation-name:l604f13}
  #lvis .s6.l604[data-frame="14"] [data-anim]{animation-name:l604f14}
  #lvis .s6.l604[data-frame="15"] [data-anim]{animation-name:l604f15}
  #lvis .s6.l604[data-frame="16"] [data-anim]{animation-name:l604f16}
  @keyframes l604f1{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:none}}
  @keyframes l604f2{0%{opacity:0;transform:scale(.94)}100%{opacity:1;transform:none}}
  @keyframes l604f3{from{opacity:0;transform:translateX(12px)}to{opacity:1;transform:none}}
  @keyframes l604f4{from{opacity:0;transform:translateX(-14px)}to{opacity:1;transform:none}}
  @keyframes l604f5{0%{opacity:0;transform:scale(.92)}70%{transform:scale(1.03)}100%{opacity:1;transform:none}}
  @keyframes l604f6{from{opacity:0;transform:translateY(-12px)}to{opacity:1;transform:none}}
  @keyframes l604f7{from{opacity:0;transform:translateX(-12px)}to{opacity:1;transform:none}}
  @keyframes l604f8{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:none}}
  @keyframes l604f9{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:none}}
  @keyframes l604f10{0%{opacity:0;transform:scale(.96)}100%{opacity:1;transform:none}}
  @keyframes l604f11{from{opacity:0;transform:translateX(12px)}to{opacity:1;transform:none}}
  @keyframes l604f12{from{opacity:0;transform:translateY(-10px)}to{opacity:1;transform:none}}
  @keyframes l604f13{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:none}}
  @keyframes l604f14{from{opacity:0;transform:translateX(-10px)}to{opacity:1;transform:none}}
  @keyframes l604f15{0%{opacity:0;transform:scale(.94)}100%{opacity:1;transform:none}}
  @keyframes l604f16{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:none}}
  @media (max-width:370px){
    #lvis .s6.l604 .pairs{grid-template-columns:repeat(2,1fr)}
    #lvis .s6.l604 .son{grid-template-columns:repeat(2,1fr)}
    #lvis .s6.l604 .hear button b{font-size:24px}
    #lvis .s6.l604 .word b{font-size:32px}
    #lvis .s6.l604 .split{flex-direction:column}
  }
  @media (prefers-reduced-motion: reduce){
    #lvis .s6.l604 [data-anim]{animation:none!important}
  }`;
  function css(){
    try{
      if(window.RUKIT&&RUKIT.frameCss) RUKIT.frameCss();
      let e=document.getElementById('l604-style');
      if(!e){ e=document.createElement('style'); e.id='l604-style'; document.head.appendChild(e); }
      if(e.textContent!==CSS) e.textContent=CSS;
    }catch(e){}
  }
  const S=()=>{ const lk=lidKey(604); if(typeof CHS==='undefined') window.CHS={}; if(!CHS[lk]) CHS[lk]={}; return CHS[lk]; };
  const T=(tag,i,cls,html)=>`<${tag} data-anim style="--i:${i}" class="${cls||''}">${html}</${tag}>`;
  const BTN=(i,cls,html,on)=>`<button type="button" data-anim style="--i:${i}" class="${cls}" onclick="${on}">${html}</button>`;
  const G=[['сне_','г','снега'],['гла_','з','глаза'],['ло_ка','ж','ложечка'],
           ['моро_','з','морозы'],['зу_','б','зубы'],['кни_ка','ж','книжечка']];
  window.RUGAME=window.RUGAME||{}; window.RUGAME[604]=G;
  const P={
    2:['Почему «зуб» звучит как [зуп]?',[{k:'a',t:'звук ослаб на конце',ok:1,fb:'верно: слабая позиция, звонкий оглушается'},{k:'b',t:'мы говорим неправильно',ok:0,fb:'так устроен язык: на конце звонкий становится глухим'}]],
    3:['Сколько пар согласных?',[{k:'a',t:'шесть',ok:1,fb:'верно: б—п, в—ф, г—к, д—т, ж—ш, з—с'},{k:'b',t:'три',ok:0,fb:'пар шесть, не три'}]],
    4:['Где согласная путается?',[{k:'a',t:'на конце и перед глухой',ok:1,fb:'верно: зуб и ложка — обе слабые позиции'},{k:'b',t:'только в начале слова',ok:0,fb:'в начале как раз слышно ясно'}]],
    5:['Как проверить «зуб»?',[{k:'a',t:'зубы: после б гласный',ok:1,fb:'верно: гласный вытягивает настоящий звук'},{k:'b',t:'произнести громче',ok:0,fb:'громкость букву не меняет'}]],
    6:['Кроме гласного помогают…',[{k:'a',t:'л, м, н, р',ok:1,fb:'верно: зубной — согласная ясна'},{k:'b',t:'любая глухая',ok:0,fb:'глухая как раз и путает'}]],
    7:['Проверка для «снег»?',[{k:'a',t:'снега',ok:1,fb:'верно: после г стоит гласный'},{k:'b',t:'снегки',ok:0,fb:'такого слова нет'}]],
    8:['В «солнце» буква л…',[{k:'a',t:'пишется, хотя не слышна',ok:1,fb:'верно: непроизносимая согласная'},{k:'b',t:'не нужна, раз не слышна',ok:0,fb:'без л будет ошибка: солнышко это доказывает'}]],
    9:['Где прячутся непроизносимые?',[{k:'a',t:'стн, здн, лнц, вств, рдц',ok:1,fb:'верно: местный, праздник, солнце, чувство, сердце'},{k:'b',t:'только на конце слова',ok:0,fb:'они сидят внутри сочетаний'}]],
    10:['Проверка для «сердце»?',[{k:'a',t:'сердечко',ok:1,fb:'верно: д зазвучало'},{k:'b',t:'серце без д',ok:0,fb:'без д — ошибка'}]],
    11:['В «вкусный» нужна т?',[{k:'a',t:'нет: вкусен',ok:1,fb:'верно: проверка показала, что буквы нет'},{k:'b',t:'да: как в «местный»',ok:0,fb:'местный — место, а вкусный — вкусен. Разные слова'}]],
    12:['«Класс» как проверить?',[{k:'a',t:'никак: это словарное',ok:1,fb:'верно: удвоенные согласные запоминают'},{k:'b',t:'класы',ok:0,fb:'проверки нет'}]],
    13:['После сомнительной согласной в проверке стоит…',[{k:'a',t:'гласный (или л, м, н, р)',ok:1,fb:'верно: зубы, зубной'},{k:'b',t:'ещё один согласный',ok:0,fb:'согласный не вытягивает звук'}]],
    14:['«Местный» и «вкусный» отличаются тем, что…',[{k:'a',t:'в одном т есть, в другом нет',ok:1,fb:'верно: место — есть, вкусен — нет'},{k:'b',t:'оба пишутся одинаково',ok:0,fb:'проверка как раз и разделяет их'}]],
    15:['Три вещи в карман. Первая — …',[{k:'a',t:'гласный после согласной',ok:1,fb:'верно: зуб — зубы'},{k:'b',t:'писать как слышится',ok:0,fb:'на конце как раз слышится не то'}]]
  };
  function pred(f,body){
    const p=P[f]; if(!p) return body;
    const s=S(), cur=s['q'+f];
    ...
    const fb=cur
      ? `<p class="verdict ${pick&&pick.ok?'ok':'no'}">${pick&&pick.ok?'верно. '+pick.fb:'не так. '+(pick?pick.fb:'подумай ещё')}</p>`
      : `<p class="verdict">Выбери ответ:</p>`;
    return body+`<div class="askrow">${p[1].map((o,i)=>BTN(8+i,'chip'+(cur===o.k?(o.ok?' ok':' no'):''),o.t,`r604Pred(${f},'${o.k}')`)).join('')}</div>`+fb;
  }
  function body(s,f){
    if(f===1){
      return `${T('div',0,'kicker','01 · Слова урока')}
        ${T('h2',1,'','Сначала — список, который слух не выдаст')}
        ${T('p',2,'lead','Одни слова держат удвоенную согласную, другие — ту, которую не произносим.')}
        ${typeof ruWords604==='function'?ruWords604():''}`;
    }
    if(f===2){
      const on=!!s.hear;
      return `${T('div',0,'kicker','02 · Слышим одно, пишем другое')}
        ${T('h2',1,'','«Зуб» на конце звучит как [зуп]')}
        ${T('p',2,'lead','Слышится один звук, а буква может быть другой. Это слабая позиция.')}
        <div class="hear">
          ${BTN(3,!on?'chip on':'chip','<b>зу<span class="'+(on?'mute':'')+'">б</span></b><i>как пишем</i>',`r604Hear(0)`)}
          ${BTN(4,on?'chip on':'chip','<b>[зуп]</b><i>как слышим</i>',`r604Hear(1)`)}
        </div>
        ${T('p',5,'cap', on?'На конце звонкий оглушается. Слуху здесь верить нельзя.':'Нажми: увидишь, как звук и буква разошлись.')}`;
    }
    if(f===3){
      const PAIRS=[['б—п','звонкий б, глухой п'],['в—ф','звонкий в, глухой ф'],['г—к','звонкий г, глухой к'],
                   ['д—т','звонкий д, глухой т'],['ж—ш','звонкий ж, глухой ш'],['з—с','звонкий з, глухой с']];
      const cur=s.pr;
      const hit=PAIRS.find(x=>x[0]===cur);
      return `${T('div',0,'kicker','03 · Шесть пар')}
        ${T('h2',1,'','В паре один звонкий, другой глухой')}
        ${T('p',2,'lead','В слабой позиции они путаются. Запомни пары — будешь знать, из чего выбирать.')}
        <div class="pairs">${PAIRS.map((p,i)=>BTN(3+i,cur===p[0]?'chip on':'chip',p[0],`r604Pr('${p[0]}')`)).join('')}</div>
        ${T('p',9,'cap', hit?(hit[1]+'. На конце часто слышен правый, а пишется левый.'):'Нажми пару. Слева звонкий, справа глухой.')}`;
    }
    if(f===4){
      const w=s.pos||0;
      return `${T('div',0,'kicker','04 · Где опасно')}
        ${T('h2',1,'','Два места, где согласная спорит')}
        <div class="pair">
          ${T('div',2,'card'+(w===0?' on':''),'<div class="nm">конец слова</div><div class="ex">зуб → [зуп]<br>снег → [сн’эк]<br>глаз → [глас]</div>')}
          ${T('div',3,'card'+(w===1?' on':''),'<div class="nm">перед глухой</div><div class="ex">ложка → [лошка]<br>книжка → [книшка]<br>сказка → [скаска]</div>')}
        </div>
        <div class="row">${BTN(4,w===0?'chip on':'chip','конец слова',`r604Pos(0)`)}${BTN(5,w===1?'chip on':'chip','перед глухой',`r604Pos(1)`)}</div>
        ${T('p',6,'cap', w===1?'Перед п, ф, к, т, ш, с звонкий тоже слабеет. Ложка — слышим [ш], пишем ж.':'На конце слова звонкий становится глухим. Зуб — слышим [п], пишем б.')}`;
    }
    if(f===5){
      const on=!!s.pull;
      return `${T('div',0,'kicker','05 · Проверка')}
        ${T('h2',1,'','Поставь после согласной гласный')}
        ${T('p',2,'lead','Гласный <b style="color:#ffd76a">вытягивает</b> настоящий звук.')}
        <div class="pull">
          ${T('span',3,'word', on?'<b>зубы</b><i>слышим [б]</i>':'<b>зуб</b><i>слышим [п]</i>')}
          ${on?T('span',4,'tag','гласный встал — и б на месте'):''}
        </div>
        ${BTN(5,'cta', on?'Вернуть «зуб»':'Поставить гласный: зубы',`r604Pull()`)}
        <div class="row">${T('span',6,'tag','снег → снега')}${T('span',7,'tag','глаз → глаза')}</div>
        ${T('p',8,'cap','Изменил слово так, чтобы после согласной стоял гласный — и услышал правду.')}`;
    }
    if(f===6){
      const on=s.son||'н';
      const help={л:{w:'корабль',why:'б перед л слышно ясно'},м:{w:'обман',why:'б перед м слышно ясно'},
                  н:{w:'зубной',why:'зуб → зубной: б перед н'},р:{w:'добрый',why:'б перед р слышно ясно'}};
      const h=help[on]||help.н;
      return `${T('div',0,'kicker','06 · Ещё помощники')}
        ${T('h2',1,'','После л, м, н, р согласная тоже ясна')}
        ${T('p',2,'lead','Не только гласный. Эти четыре тоже держат звук.')}
        <div class="son">${['л','м','н','р'].map((x,i)=>BTN(3+i,on===x?'chip on':'chip',x,`r604Son('${x}')`)).join('')}</div>
        <div class="split">${T('span',7,'word',`<b>${h.w}</b><i>${h.why}</i>`)}</div>
        ${T('p',9,'cap','Зуб — зубной: вот проверка через н. Ищем такое место, как для гласной.')}`;
    }
    if(f===7){
      const EX=[{w:'снег',c:'снега',l:'г'},{w:'ложка',c:'ложечка',l:'ж'},{w:'глаз',c:'глаза',l:'з'},{w:'мороз',c:'морозы',l:'з'}];
      const i=s.ex||0, a=EX[i], show=!!s.exOn;
      return `${T('div',0,'kicker','07 · Разбор')}
        ${T('h2',1,'','Кликни слово — проверка выйдет сама')}
        <div class="split">${T('span',2,'word', show?`<b>${a.w}</b><i>${a.c} → пишем ${a.l}</i>`:`<b>${a.w}</b><i>пока без проверки</i>`)}</div>
        <div class="row">${EX.map((x,k)=>BTN(3+k,i===k?'chip on':'chip',x.w,`r604Ex(${k})`)).join('')}</div>
        ${BTN(7,'cta', show?'Скрыть проверку':'Показать проверку',`r604ExOn()`)}
        ${T('p',8,'cap','Слышим одно, проверяем другое, пишем то, что услышали в проверке.')}`;
    }
    if(f===8){
      const on=!!s.sun;
      return `${T('div',0,'kicker','08 · Буква есть, звука нет')}
        ${T('h2',1,'','Пишем «солнце», слышим [сонцэ]')}
        ${T('p',2,'lead','Согласная спряталась. Её не произносим — и всё равно пишем.')}
        <div class="hear">
          ${BTN(3,!on?'chip on':'chip','<b>со<span class="'+(on?'mute':'')+'">л</span>нце</b><i>как пишем</i>',`r604Sun(0)`)}
          ${BTN(4,on?'chip on':'chip','<b>[сонцэ]</b><i>как слышим</i>',`r604Sun(1)`)}
        </div>
        ${T('p',5,'cap', on?'Буквы л нет в звуке — и она есть в слове. Такие согласные называют непроизносимыми.':'Нажми: увидишь, как буква пропала из слуха, но не из письма.')}`;
    }
    if(f===9){
      const C=[['стн','местный','т не звучит'],['здн','праздник','д не звучит'],['лнц','солнце','л не звучит'],['вств','чувство','в не звучит'],['рдц','сердце','д не звучит']];
      const i=s.cmb==null?-1:s.cmb;
      return `${T('div',0,'kicker','09 · Где прячутся')}
        ${T('h2',1,'','Пять сочетаний — и все они ловушка')}
        ${T('p',2,'lead','Увидел стн, здн, лнц, вств, рдц — остановись: тут может быть немая буква.')}
        <div class="row">${C.map((x,k)=>BTN(3+k,i===k?'chip on':'chip',x[0],`r604Cmb(${k})`)).join('')}</div>
        ${i>=0?`<div class="job" data-anim style="--i:9"><b>${C[i][1]}</b><i>${C[i][2]} — буква пишется, звука нет</i></div>`
              :T('p',8,'cap','Нажми сочетание — выйдет слово, где оно живёт.')}`;
    }
    if(f===10){
      const EX=[{w:'солнце',c:'солнышко'},{w:'сердце',c:'сердечко'},{w:'местный',c:'место'},{w:'здравствуй',c:'здравый'}];
      const i=s.un||0, a=EX[i], show=!!s.unOn;
      return `${T('div',0,'kicker','10 · Заставь зазвучать')}
        ${T('h2',1,'','Подбери слово, где согласная слышна')}
        <div class="split">${T('span',2,'word', show?`<b>${a.w}</b><i>${a.c}</i>`:`<b>${a.w}</b><i>пока молчит</i>`)}</div>
        <div class="row">${EX.map((x,k)=>BTN(3+k,i===k?'chip on':'chip',x.w,`r604Un(${k})`)).join('')}</div>
        ${BTN(7,'cta', show?'Скрыть проверку':'Показать проверку',`r604UnOn()`)}
        ${T('p',8,'cap','Согласная зазвучала — значит, буква нужна. Не зазвучала — смотри кадр про ловушку.')}`;
    }
    if(f===11){
      const t=s.extra||0;
      const rows=[{w:'вкусный',c:'вкусен',need:0},{w:'опасный',c:'опасен',need:0},{w:'местный',c:'место',need:1}];
      const a=rows[t];
      return `${T('div',0,'kicker','11 · Ловушка: буквы нет')}
        ${T('h2',1,'','Иногда согласная только кажется')}
        ${T('p',2,'lead','«Вкусный» хочется написать с т. Проверка «вкусен» говорит: буквы нет.')}
        <div class="split">${T('span',3,'word',`<b>${a.w}</b><i>${a.c}${a.need?' — т есть':' — т нет'}</i>`)}</div>
        <div class="row">${rows.map((x,k)=>BTN(4+k,t===k?'chip on':'chip',x.w,`r604Extra(${k})`)).join('')}</div>
        ${T('p',7,'cap', a.need?'Место — т звучит, пишем местный.':'Вкусен, опасен — т не появляется. Лишнюю букву не ставим.')}`;
    }
    if(f===12){
      const W=['класс','суббота','аллея','хоккей'];
      const why={класс:'сс — две с подряд',суббота:'бб — две б подряд',аллея:'лл — две л подряд',хоккей:'кк — две к подряд'};
      const cur=s.dbl;
      return `${T('div',0,'kicker','12 · Удвоенные')}
        ${T('h2',1,'','Их нельзя проверить')}
        ${T('p',2,'lead','Две одинаковые согласные подряд — словарное слово. Слух количество не выдаст.')}
        <div class="row">${W.map((w,i)=>BTN(3+i,cur===w?'chip on':'chip',w,`r604Dbl('${w}')`)).join('')}</div>
        ${cur?`<div class="job" data-anim style="--i:8"><b>${cur}</b><i>${why[cur]} — запоминают по словарю</i></div>`
             :T('p',8,'cap','Нажми слово. Правила «поставь гласный» здесь нет.')}`;
    }
    if(f===13){
      const n=s.alg||0;
      const ST=[
        ['1','Найди сомнительную','конец слова, перед глухой, или сочетание стн / здн / лнц'],
        ['2','Подбери проверку','чтобы после неё стоял гласный — или л, м, н, р'],
        ['3','Послушай','зазвучала — буква нужна; не зазвучала — буквы нет'],
        ['4','Нет проверки — словарь','удвоенные: класс, суббота, аллея']
      ];
      return `${T('div',0,'kicker','13 · Алгоритм')}
        ${T('h2',1,'','Четыре шага — и согласная на месте')}
        <div class="steps">${ST.map((x,i)=>
          T('div',2+i,'st'+(n===i+1?' on':(n&&i+1!==n?' dim':'')),
            `<span class="n">${x[0]}</span><span class="t">${x[1]}<small>${x[2]}</small></span>`)).join('')}</div>
        ${BTN(6,'cta', n>=4?'Сначала':'Следующий шаг',`r604Alg()`)}
        ${T('p',7,'cap', n?('Шаг '+n+' из 4. Не пиши «как слышится». На конце как раз слышится не то.'):'Нажимай: шаги загорятся по одному.')}`;
    }
    if(f===14){
      const pick=s.vs;
      return `${T('div',0,'kicker','14 · Два слова рядом')}
        ${T('h2',1,'','«Местный» и «вкусный»')}
        ${T('p',2,'lead','Оба на «-сный». В одном т есть, в другом нет. Решает только проверка.')}
        <div class="pair">
          ${T('div',3,'card'+(pick==='мест'?' on':''),'<div class="nm">местный</div><div class="ex">место — т звучит<br>пишем т</div>')}
          ${T('div',4,'card'+(pick==='вкус'?' on':''),'<div class="nm">вкусный</div><div class="ex">вкусен — т нет<br>не пишем т</div>')}
        </div>
        <div class="row">${BTN(5,pick==='мест'?'chip ok':'chip','в «местный» т нужна',`r604Vs('мест')`)}${BTN(6,pick==='вкус'?'chip ok':'chip','в «вкусный» т не нужна',`r604Vs('вкус')`)}</div>
        ${T('p',7,'cap', pick?'Оба ответа верны — если у каждого своя проверка. Не копируй одно слово на другое.':'Нажми. Проверка разводит слова, которые слух склеивает.')}`;
    }
    if(f===15){
      const i=s.pkt||0;
      const C=[
        ['а  Гласный после согласной','зуб — зубы, снег — снега'],
        ['∅  Непроизносимую заставь зазвучать','солнце — солнышко; вкусный — вкусен'],
        ['сс  Удвоенные — словарь','класс, суббота, аллея, хоккей']
      ];
      return `${T('div',0,'kicker','15 · В карман')}
        ${T('h2',1,'','Три фразы, которые стоит унести')}
        <div class="pkt">${C.map((x,k)=>BTN(2+k,i===k+1?'chip on':'chip',
          `<b>${x[0]}</b>${i===k+1?`<i>${x[1]}</i>`:''}`,`r604Pkt(${k+1})`)).join('')}</div>
        ${T('p',5,'cap', i?'Держи эти три. Дальше сам: вставь букву. Она прилетит в слово.':'Нажми карточку — фраза перевернётся.')}`;
    }
    return '';
  }
  function render(el){
    ...
    if(f===16 && window.RULETTER&&RULETTER.draw){
      const box=document.createElement('div');
      try{ RULETTER.draw(box,604); }catch(e){ box.innerHTML=''; }
      el.innerHTML=`<div class="s6 l604" data-frame="16">${T('div',0,'kicker','16 · Тренажёр')}${box.innerHTML}</div>`;
      return;
    }
    el.innerHTML=`<div class="s6 l604" data-frame="${f}">${pred(f, body(s,f))}</div>`;
  }
  window.r604Pred=(f,k)=>{ S()['q'+f]=k; chRender(0); };
  window.r604Hear=(n)=>{ S().hear=n?1:0; chRender(0); };
  window.r604Pr=(p)=>{ S().pr=p; chRender(0); };
  window.r604Pos=(n)=>{ S().pos=n; chRender(0); };
  window.r604Pull=()=>{ const s=S(); s.pull=s.pull?0:1; chRender(0); };
  window.r604Son=(x)=>{ S().son=x; chRender(0); };
  window.r604Ex=(i)=>{ S().ex=i; S().exOn=1; chRender(0); };
  window.r604ExOn=()=>{ const s=S(); s.exOn=s.exOn?0:1; chRender(0); };
  window.r604Sun=(n)=>{ S().sun=n?1:0; chRender(0); };
  window.r604Cmb=(i)=>{ S().cmb=i; chRender(0); };
  window.r604Un=(i)=>{ S().un=i; S().unOn=1; chRender(0); };
  window.r604UnOn=()=>{ const s=S(); s.unOn=s.unOn?0:1; chRender(0); };
  window.r604Extra=(i)=>{ S().extra=i; chRender(0); };
  window.r604Dbl=(w)=>{ S().dbl=w; chRender(0); };
  window.r604Vs=(w)=>{ S().vs=w; chRender(0); };
  window.r604Alg=()=>{ const s=S(); s.alg=s.alg>=4?0:(s.alg||0)+1; chRender(0); };
  window.r604Pkt=(i)=>{ S().pkt=i; chRender(0); };
  if(window.WAVE_B){
