  ...
  const L602 = {
    ...
    tasks: [
      { q: 'Найди корень в слове «пришкольный»', kind: 'choice', choices: ['школь', 'при', 'ный'], ans: 0,
        hints: ['Убери приставку при- и окончание -ый.', 'Останется школь- — родственник слова школа.'], sol: 'школь' },
...
(function(){
  ...
  const L603 = {
    ...
    explain: [
      ...
      'Разберём на примерах. «Молоко»: проверяем — молОчный, значит пишем две буквы о. «Земля»: зЕмли — пишем е. «Пятак»: пЯть — пишем я. Каждый раз мы ищем слово, где гласная станет ударной.',
  ...
  function scene(step, st){
    ...
    if(step===6){
      return SV(`
        ...
        ${T2(72,62,'молоко',PALE,{fs:13})}${T2(200,62,'← молОчный: о',GREEN,{fs:11})}
...
(function(){
  ...
  const L605 = {
    ...
    tasks: [
      ...
      { q: 'В каком слове нужен твёрдый знак: «с_ел», «за_ехал»?', kind: 'choice', choices: ['съел', 'заехал оба без ъ'], ans: 0,
  ...
  function scene(step, st){
    ...
    if(step===2){
      return SV(`
        ...
        ${BOX(172,44,148,96,GOLD,'женский',['-а, -я:','парта, земля, дядя'])}
...
(function(){
  ...
  const GAME607=[['читаю книгу','в','винительный'],['нет книги','р','родительный'],
    ['дать другу','д','дательный'],['рисую карандашом','т','творительный'],
    ['думаю о книге','п','предложный'],['ученик читает','и','именительный']];
...
(function(){
  ...
  const GAME609=[['Он учит?ся','тся','что делает? — без ь'],['Надо учит?ся','ться','что делать? — с ь'],
    ['Он не хоч?т','нет','не хочется — не с глаголом раздельно'],['Она улыбает?ся','тся','что делает? — без ь'],
...
window.RUKEXAM.build({
  ...
  items: [
    ...
    {word:'п_анино', hint:"Итальянское слово «тихо». Гласная во втором слоге.", ans:'и', opts:['и','е'], icon:'piano', spell:'пианино', ask:'проверяем гласную во втором слоге', rule:'Словарное слово: пишем «пианино».', note:'Пианино — от итальянского piano, «тихо».'},
...
window.RUKEXAM.build({
  ...
  items: [
    ...
    {word:'пр_рода', hint:'Словарное слово: здесь приставка при-.', ans:'и', opts:['и','е'], icon:'tree', spell:'природа', ask:'проверяем гласную в приставке', rule:'В слове «природа» пишется при-.', note:'Природа — с приставкой при-. Запомни: природа, привет, пример.'}
  ],
  tail: [
...
window.RUKEXAM.buildMcko({
  ...
  items: [
    ...
    { kind:'radio', points:1, theme:'Ударение', icon:'rank',
      ...
      sol:'Верно выделено ударение в слове «краси́вее». Нормы: звони́шь, то́рты, катало́г. В демотесте МЦКО в этом задании было слово «досуг», где норма спорная, поэтому здесь взяты слова с однозначным ударением.' },
    ...
    { kind:'radio', points:1, theme:'Морфемный анализ слова', icon:'frame',
      ...
      material:'поверите · полосатый · поучение · построили',
      opts:['поверите','полосатый','поучение','построили'], correct:2, shown:'поучение',
      ask:'Разбери каждое слово по составу.', hint:'Найди приставку, потом окончание: по-уч-ени-е.',
      sol:'Поучение: по- (приставка) + -уч- (корень) + -ени- (суффикс) + -е (окончание). В «поверите» окончание -ите и суффикса нет, в «полосатом» нет приставки, в «построили» два суффикса.' },
    ...
    { kind:'multi', points:2, theme:'ё/о после шипящих', icon:'candle',
      ...
      opts:['переш..птываться','ш..фёр','реш..тчатый','ш..рох','теч..т'],
      correct:[1,3], shown:'шофёр, шорох',
      ask:'Отметь все подходящие слова.',
      hint:'Ё — если есть проверка (шептать, решето, течёт); О — в словах-исключениях.',
      sol:'Под ударением в корне после шипящих пишем Ё, если есть проверочное слово: перешёптываться, решётчатый, течёт. О — в исключениях: шофёр, шорох, шов, крыжовник.' },
    ...
    { kind:'radio', points:1, theme:'Типы речи', icon:'mountain',
      ...
      material:'(4) Листок этой травы внутри пушист. (5) Капли весело катаются по нежному листку и не мочат его.',
    ...
    { kind:'radio', points:1, theme:'Тема и главная мысль', icon:'orange',
      ...
      material:'Текст о каплях росы, которые кажутся алмазами, и о росинке, которая вкуснее всякого напитка.',
    ...
    { kind:'text', points:1, theme:'Синонимы и антонимы', icon:'basket',
      ...
      material:'(1) Когда в солнечное утро летом войдёшь в лес, то в мокрой траве видны алмазы…',
    ...
    { kind:'multi', points:2, theme:'Прямое и переносное значения слов', icon:'ornament',
      ...
      hint:'Эпитет — образное, необычное определение; он рисует предмет или действие.',
      sol:'В этой работе МЦКО эпитетами признаны «весело катаются» (образное наречие) и «по нежному листку». «В мокрой траве» и «в треугольных листах» — обычные определения, они называют признак прямо.' },
