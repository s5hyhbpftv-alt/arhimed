...
(function(){
  ...
  function парусник(x,y,м,опц){
    ...
    return `<g transform="translate(${x} ${y}) scale(${м})">${о.качка!==false?качать('0 0;0 3;0 0','3.6s'):''}
      ...
      <path d="${флаг1}" fill="#e05a3a" stroke="${ОБВОД}" stroke-width=".9">${ДВИЖ?`<animate attributeName="d" values="${флаг1};${флаг2};${флаг1}" dur="1.4s" repeatCount="indefinite"/>`:''}</path>
  ...
  function капитан(x,y,м,опц){
    ...
    return `<g transform="translate(${x} ${y}) scale(${м})">
      ...
      <path d="M-24 -54 h8" stroke="url(#рм-латунь)" stroke-width="2"/>
