...
(function(){
  ...
  function visW44(el){
    const step=LV.step||0;
    const lk=lidKey(LV.id); if(!CHS[lk]) CHS[lk]={}; const st=CHS[lk];
    if(st._at!==step){ st._at=step; if(step===0||step===3||step===4||step===5||step===8){ st.sh=0; } if(step===2||step===3){ st.n=0; } if(step===10){ if(st.p==null) st.p=10; } if(step===12){ if(st.tr==null) st.tr=0; st.s1=0; st.s2=0; } if(step===13){ st.sel=null; st.q=0; } }
    let h='';
    if(step===0){
      const show=st.sh===1;
