/* ====== МАТЕМАТИКА · УРОК 991 · «ЗАДАЧИ НА КУПЛЮ-ПРОДАЖУ» ==================
   ...
   Три лавки — три ключа темы, по одному на каждую неизвестную величину:
     канатчик  — ищем СТОИМОСТЬ   (цена × количество);
     парусник  — ищем ЦЕНУ        (стоимость : количество), полотно режется
                                   на равные куски — деление на части;
     бочар     — ищем КОЛИЧЕСТВО  (стоимость : цена), монеты раскладываются
                                   кучками по 12 — деление по содержанию.

   СЮЖЕТ ПОД МИР 4 КЛАССА В «ПУТИ» («Открытое море»): гавань, где судно
   снаряжают в последний раз перед тем, как берег уйдёт за корму.

   ВСЕ ЧИСЛА ПРОВЕРЕНЫ РАСЧЁТОМ:
     кошелёк 300
     канат   14 × 4 = 56          → остаток 244
     полотно 90 : 5 = 18          → остаток 154
     бочки   84 : 12 = 7          → остаток  70
     амфоры  55 : 5 = 11 (дешевле, чем 36 : 3 = 12) → остаток 15
     сдача у бочара: 7 × 12 = 84, 100 − 84 = 16
   ...
   улетают из кошеля к прилавку, полотно делится на равные куски, монеты
...
(function(){
  ...
  const НАЧАЛО = 300;
  const ПОКУПКИ = [
    {ключ:'канат',   имя:'Канат',   сколько:'4 мотка',  сумма:56},
    {ключ:'полотно', имя:'Полотно', сколько:'5 локтей', сумма:90},
    {ключ:'бочки',   имя:'Бочки',   сколько:'7 бочек',  сумма:84},
    {ключ:'амфоры',  имя:'Амфоры',  сколько:'5 амфор',  сумма:55}
  ];
  ...
  /* ---------------- ТОВАРЫ ---------------- */
  /* моток каната: кольцо со свивкой — видно, что это верёвка, а не бублик */
  const моток = (x,y,r) => {
  ...
  /* 1. Поручение: гавань, лавки, кошель */
  function F1(s){
    ...
    return ОПИСЬ(s) +
      ЗАДАЧА('Архимед даёт тебе <b>300 драхм</b> и опись: канат, полотно, бочки и амфоры. Судно уходит в открытое море — это последняя гавань, где можно что-то купить.') +
      `<div class="pic">${свг(`
        ...
        ${подпись(168,22,'Последняя гавань',GOLD,16)}
        ${т(42,238,'кошель',14,МУТ)}
        ${т(118,238,'канат',14,МУТ)}
        ${т(204,238,'полотно',14,МУТ)}
  ...
  function F2(s){
    ...
    return ОПИСЬ(s) +
      ЗАДАЧА('Моток каната стоит <b>14 драхм</b> — это <b>цена</b>. Бери мотки с полки и смотри, что делается со стоимостью.') +
      `<div class="pic">${свг(`
        ${фон(240)}
  ...
  /* 3. Канатчик: ищем СТОИМОСТЬ */
  function F3(s){
    const в = s.о1, верно = в==='ok', есть = куплено(s,'канат');
    const монеты = есть ? `<g>${анТРаз('0 0;196 -18','760ms')}
      ${монета(58,192,7)}${монета(72,188,7)}${монета(65,202,7)}</g>` : '';
    return ОПИСЬ(s) +
      ЗАДАЧА('Канатчик: <b>14 драхм за моток</b>. Нужно <b>4 мотка</b>. Сколько отдать за всё?') +
      `<div class="pic">${свг(`
        ${фон(252)}
        ${подпись(168,24,'Лавка канатчика',GOLD,16)}
        ...
        ${подпись(206,244, верно?'14 × 4 = 56':'14 × 4 = ?', верно?GREEN:GOLD, 20)}
      `,252)}</div>` +
      `<div class="ask">
      ...
      ДЕЛО(есть ? '✓ Канат куплен, отдал 56 драхм' : 'Заплатить 56 драхм', "r991Купить('канат')", есть, верно) +
      ПРАВИЛО('<b>Стоимость = цена × количество.</b>');
  }

  /* 4. Парусник: ищем ЦЕНУ */
  function F4(s){
    const в = s.о2, верно = в==='ok', есть = куплено(s,'полотно');
    const монеты = есть ? `<g>${анТРаз('0 0;0 -24','760ms')}
      ${монета(296,196,7)}${монета(282,200,7)}</g>` : '';
    return ОПИСЬ(s) +
      ЗАДАЧА('Парусник отмерил <b>5 локтей</b> полотна и просит за всё <b>90 драхм</b>. Сколько стоит <b>один локоть</b>?') +
      `<div class="pic">${свг(`
        ${фон(250)}
        ${подпись(168,24,'Лавка парусника',GOLD,16)}
        ${подпись(168,72,'за всё полотно — 90 др.',GOLD,16)}
        ...
        ${т(56,196,'1 локоть',14,МУТ)}
        ${монеты}
        ${верно?рамкаВсп(30,98,84,160,GREEN,'6s',0.12):''}
        ${подпись(212,216, верно?'90 : 5 = 18':'90 : 5 = ?', верно?GREEN:GOLD, 20)}
      `,250)}</div>` +
      `<div class="ask">
        ${BTN(4, верно?'hit':(в?'miss':''), '18 драхм за локоть', "r991О2('ok')")}
        ${BTN(5, в==='no'?'miss':'', '450 драхм за локоть', "r991О2('no')")}
      </div>` +
      (в!=null ? РАЗБОР(верно,
        верно ? 'Верно: полотно разрезали на 5 равных кусков, и все 90 драхм разделились между ними поровну. 90 : 5 = 18.'
              : 'Нет: один локоть не может стоить дороже, чем всё полотно. Стоимость делят на количество: 90 : 5 = 18.')
        : СКАЗ('Ответ','Выбери вариант выше.')) +
      ДЕЛО(есть ? '✓ Полотно куплено, отдал 90 драхм' : 'Заплатить 90 драхм', "r991Купить('полотно')", есть, верно) +
      ПРАВИЛО('<b>Цена = стоимость : количество.</b>');
  }

  /* 5. Бочар: ищем КОЛИЧЕСТВО */
  function F5(s){
    const в = s.о3, верно = в==='ok', есть = куплено(s,'бочки');
    const X = (i)=> 30+i*46;
    return ОПИСЬ(s) +
      ЗАДАЧА('У тебя отложено <b>84 драхмы</b> на бочки. Бочка стоит <b>12 драхм</b>. Сколько бочек получится?') +
      `<div class="pic">${свг(`
        ${фон(256)}
        ${подпись(168,24,'Лавка бочара',GOLD,16)}
  ...
  function F6(s){
    ...
    return ОПИСЬ(s) +
      ЗАДАЧА('Три величины связаны намертво. Закрой печатью ту, которую ищешь, — и оставшиеся две сами покажут действие. Числа взяты от каната: 14 драхм, 4 мотка, 56 драхм.') +
      `<div class="pic">${свг(`
        ${фон(250)}
  ...
  function F7(s){
    ...
    return ОПИСЬ(s) +
      ЗАДАЧА('За семь бочек ты протянул бочару <b>монету в 100 драхм</b>. Он отсчитал сдачу. Сколько он должен вернуть?') +
      `<div class="pic">${свг(`
        ${фон(244)}
        ${подпись(168,24,'Расчёт у бочара',GOLD,16)}
  ...
  function F8(s){
    const в = s.о5, верно = в==='ok', есть = куплено(s,'амфоры');
    return ОПИСЬ(s) +
      ЗАДАЧА('Осталось купить амфоры для масла. Слева: <b>3 амфоры за 36 драхм</b>. Справа: <b>5 амфор за 55 драхм</b>. Где амфора дешевле?') +
      `<div class="pic">${свг(`
        ${фон(254)}
        ...
        ${верно?подпись(250,232,'55 : 5 = 11',GREEN,16):подпись(168,232,'сравни цену одной амфоры',GOLD,14)}
      `,254)}</div>` +
      `<div class="ask">
        ${BTN(4, верно?'hit':(в?'miss':''), 'Справа: там амфора стоит 11 драхм', "r991О5('ok')")}
        ${BTN(5, в==='no'?'miss':'', 'Слева: там вся покупка дешевле', "r991О5('no')")}
      </div>` +
      (в!=null ? РАЗБОР(верно,
        верно ? 'Верно. Считать надо не «сколько отдал», а «сколько за одну»: 36 : 3 = 12, а 55 : 5 = 11. Справа дешевле на драхму с амфоры.'
              : 'Нет: 36 меньше 55, но и амфор там меньше. Сравнивают <b>цену одной</b>: 36 : 3 = 12 против 55 : 5 = 11. Дешевле справа.')
        : СКАЗ('Ответ','Выбери вариант выше.')) +
      ДЕЛО(есть ? '✓ Амфоры куплены, отдал 55 драхм' : 'Заплатить 55 драхм', "r991Купить('амфоры')", есть, верно) +
  ...
  function F9(s){
    ...
    return ОПИСЬ(s) +
      ЗАДАЧА('Последняя проверка перед отходом. <b>За 3 амфоры отдали 45 драхм.</b> Сколько стоит одна?') +
      `<div class="pic">${свг(`
        ${фон(248)}
        ${подпись(168,24,'Ловушка',GOLD,16)}
        ${[0,1,2].map(i=>`<g>${очередь(i,3,'6s')}${амфора(136+i*32,76,0.7)}</g>`).join('')}
        ${подпись(168,118,'за 3 амфоры отдали 45',GOLD,14)}
      ...
      (в!=null ? РАЗБОР(верно,
        верно ? 'Верно: 45 : 3 = 15. И проверка сходится — 15 × 3 = 45.'
              : 'Нет: 135 больше, чем все отданные 45 драхмы. Одна амфора не может стоить дороже трёх. Делим: 45 : 3 = 15.')
        : СКАЗ('Ответ','Выбери вариант выше.')) +
  ...
  function F10(s){
    ...
    return ОПИСЬ(s) +
      ЗАДАЧА(всё
        ? 'Опись закрыта: канат, полотно, бочки и амфоры на борту. Посчитаем, сколько ушло и сколько осталось.'
        : 'В описи ещё не всё отмечено — вернись на прошлые кадры и расплатись. А пока посмотрим, как сходится счёт, если купить всё.') +
      `<div class="pic">${свг(`
  ...
  const УРОВНИ = [
    ...
    { вопрос:'3 амфоры за 33 драхмы или 5 амфор за 60. Где амфора дешевле?',
      варианты:[{т:'Где 3 за 33: там 11 за штуку',ок:true},{т:'Где 5 за 60',ок:false}],
      разбор:'33 : 3 = 11, а 60 : 5 = 12. Сравнивают цену одной, а не всю покупку.' },
  ...
  const Т1 = { имя:'Стоимость', задания:[
    ...
    {q:'7 канатов по 9 драхм. Стоимость?', в:0, варианты:['63 драхмы','16 драхм'], раз:'9 × 7 = 63.'},
  ...
  const Т2 = { имя:'Цена и количество', задания:[
    ...
    {q:'Есть 100 драхм, амфора стоит 20. Сколько амфор?', в:1, варианты:['80 амфор','5 амфор'], раз:'100 : 20 = 5.'}
  ]};
  ...
  const L991 = {
    ...
    subj: 'jun',
    explain: [
      'Архимед даёт 300 драхм и опись поручения. Последняя гавань перед открытым морем: цена — сколько стоит один, количество — сколько взяли, стоимость — сколько отдали за всё.',
      'Моток каната стоит 14 драхм. Бери мотки с полки: цена не меняется, а стоимость растёт — это цена, повторённая столько раз, сколько мотков.',
      'Лавка канатчика: 14 драхм за моток, нужно 4 мотка. Стоимость = цена × количество, 14 × 4 = 56.',
      'Лавка парусника: 5 локтей полотна за 90 драхм. Полотно режут на равные куски, и 90 делится между ними поровну: цена = стоимость : количество, 90 : 5 = 18.',
      'Лавка бочара: 84 драхмы, бочка стоит 12. Раскладываем монеты кучками по 12 — сколько кучек, столько бочек: количество = стоимость : цена, 84 : 12 = 7.',
      ...
      'Торг: 3 амфоры за 36 или 5 за 55. Сравнивают не всю покупку, а цену одной: 12 против 11.',
      'Ловушка: за 3 амфоры отдали 45 — цена одной 45 : 3 = 15, а не 135. Цена одного всегда меньше стоимости всех.',
    ...
    check: {
      q: 'За 6 мотков каната отдали 84 драхмы. Сколько стоит один моток?',
      choices: ['14 драхм','504 драхмы','90 драхм'],
      ans: 0,
      exp: 'Ищем цену: стоимость делим на количество, 84 : 6 = 14.'
    },
    tasks: [
      { q:'Моток каната стоит 14 драхм. Сколько отдашь за 4 мотка?', kind:'choice',
        choices:['56 драхм','18 драхм'], ans:0,
        hints:['Ищут стоимость.','Цену повторяют столько раз, сколько мотков: 14 × 4.'],
        sol:'14 × 4 = 56 драхм.' },
      { q:'5 локтей полотна стоили 90 драхм. Сколько стоит один локоть?', kind:'choice',
