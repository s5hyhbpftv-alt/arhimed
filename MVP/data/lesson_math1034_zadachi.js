...
(function(){
  ...
  #lvis .s6.l1034 .ask button{flex:1 1 44%;min-height:56px;padding:12px 14px;font-size:16px;font-weight:600;line-height:1.3;text-align:left;
  ...
  /* УЧЕНИК: реалистичная фигурка — кожа с тёплым градиентом, волосы с блеском,
     ткань с мягким светом, обувь с бликом, мягкая тень на полу. */
  const ученик = (x,y,цвет,нач,форма,м) => {
    ...
    return `<g transform="translate(${x} ${y}) scale(${м||1}) translate(${-x} ${-y})">${нач!=null?проявить('12s',нач,нач+0.04):''}
      ...
      <path d="M${x-7} ${y-47.5} q7 -5.4 14 0 q-3 -2 -7 -2 q-4 0 -7 2 Z" fill="${волосы}" stroke="${ОБВОД}" stroke-width="0.9"/>
      <ellipse cx="${x-2.6}" cy="${y-50.4}" rx="3.2" ry="1.5" fill="rgba(255,255,255,.28)"/>
      <circle cx="${x-2.4}" cy="${y-46.4}" r="1.15" fill="#2a1d12"/>
      <circle cx="${x+2.4}" cy="${y-46.4}" r="1.15" fill="#2a1d12"/>
      <circle cx="${x-2.8}" cy="${y-46.8}" r="0.4" fill="#fff"/>
      <circle cx="${x+2}" cy="${y-46.8}" r="0.4" fill="#fff"/>
      ${форма==='улыбка'?`<path d="M${x-2.4} ${y-42.6} q2.4 2 4.8 0" fill="none" stroke="#8a4a34" stroke-width="1"/>`:''}
    </g>`;
  };
  ...
  /* 1. Класс из 30 учеников: реалистичные фигурки, свет сверху-слева */
  function F1(s){
    const в = s.класс;
    return ВОПРОС('В классе 30 учеников, две трети — девочки. Сколько девочек?') +
      `<div class="pic">${свг(`
        ...
        <rect x="18" y="40" width="300" height="140" rx="10" fill="rgba(255,255,255,.03)" stroke="${ЛИНИЯ}" stroke-width="1.2"/>
        ${Array.from({length:30},(_,i)=>{
          const кл=i%10, ряд=Math.floor(i/10);
          return ученик(38+кл*29,118+ряд*30,i%3===0?'ткань3':'ткань1',i*0.02,null,0.52);
        }).join('')}
        <g>${проявить('12s',0.62,0.72)}
          ${т(168,208,'две трети класса — девочки',16,ИНК,true)}
          ${т(168,236,'сколько это человек?',16,GOLD,true)}</g>
      `,252)}</div>` +
      `<div class="ask">
  ...
  /* 2. Класс делится на три части: ряды разъезжаются плавно (кривая проекта) */
  function F2(s){
    const в = s.делим;
    return ВОПРОС('Сколько учеников в одной трети класса?') +
      `<div class="pic">${свг(`
        ${фон(256)}
        ${т(168,24,'Делим класс на три части',14,GOLD,true)}
        <rect x="18" y="40" width="300" height="146" rx="10" fill="rgba(255,255,255,.03)" stroke="${ЛИНИЯ}" stroke-width="1.2"/>
        ${Array.from({length:30},(_,i)=>{
          const кл=i%10, ряд=Math.floor(i/10);
          const цвет = ряд===0 ? 'ткань3' : 'ткань1';
          const сдвиг = 10+ряд*3;
          return `<g>${анТК('0 0;0 0;0 '+сдвиг+';0 '+сдвиг+';0 0','12s',
              '0;'+кт(0.12+ряд*0.06)+';'+кт(0.36+ряд*0.06)+';0.9;1')}
            ${ученик(38+кл*29,114+ряд*32,цвет,0,null,0.52)}</g>`;
        }).join('')}
        <g>${проявить('12s',0.46,0.56)}
          ${т(168,206,'одна треть: 30 : 3 = 10',16,GREEN,true)}
          ${т(168,232,'две трети: 10 · 2 = 20',16,GOLD,true)}</g>
      `,256)}</div>` +
      `<div class="ask">
