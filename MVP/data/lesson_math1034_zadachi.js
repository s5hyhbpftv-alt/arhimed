...
(function(){
  ...
  const ОПРЕДЕЛЕНИЯ = `
    <defs>
      ...
      <linearGradient id="c1034-целое" x1="0" y1="0" x2="0" y2="1" color-interpolation="linearRGB">
        <stop offset="0" stop-color="#cfe9fa"/><stop offset="0.55" stop-color="#9fd0ee"/>
        <stop offset="1" stop-color="#5f9ec9"/>
      </linearGradient>
      <linearGradient id="c1034-часть" x1="0" y1="0" x2="0" y2="1" color-interpolation="linearRGB">
        <stop offset="0" stop-color="#ffe9a8"/><stop offset="0.55" stop-color="#f3cf6a"/>
        <stop offset="1" stop-color="#dcae43"/>
      </linearGradient>
      <linearGradient id="c1034-часть2" x1="0" y1="0" x2="0" y2="1" color-interpolation="linearRGB">
        <stop offset="0" stop-color="#b6e8cd"/><stop offset="0.55" stop-color="#8fd1a8"/>
        <stop offset="1" stop-color="#4f9b76"/>
      </linearGradient>
      <filter id="c1034-тень" x="-60%" y="-60%" width="220%" height="220%" color-interpolation-filters="linearRGB">
  ...
  /* ЧЕЛОВЕЧЕК-точка: маленькая фигурка ученика в ряду класса */
  const ученик = (x,y,цвет,нач) => `<g>${нач!=null?проявить('12s',нач,нач+0.04):''}
    <circle cx="${x}" cy="${y}" r="6" fill="url(#c1034-${цвет})" stroke="${ОБВОД}" stroke-width="1.4"/>
    <path d="M${x-6} ${y+11} a6 6 0 0 1 12 0 Z" fill="url(#c1034-${цвет})" stroke="${ОБВОД}" stroke-width="1.2"/>
  </g>`;

  /* ================= КАДРЫ ================= */

  /* 1. Класс из 30 учеников */
  function F1(s){
    const в = s.класс;
    return ВОПРОС('В классе 30 учеников, две трети — девочки. Сколько девочек?') +
      `<div class="pic">${свг(`
        ${фон(240)}
        ${т(168,22,'Класс: 30 учеников',14,GOLD,true)}
        ${Array.from({length:30},(_,i)=>{
          const кл=i%10, ряд=Math.floor(i/10);
          return ученик(38+кл*29,58+ряд*36,'целое',i*0.03);
        }).join('')}
        <g>${проявить('11s',0.6,0.7)}
          ${т(168,196,'две трети класса — девочки',14,ИНК,true)}
          ${т(168,222,'сколько это человек?',14,GOLD,true)}</g>
      `,240)}</div>` +
      `<div class="ask">
  ...
  /* 2. Класс делится на три части */
  function F2(s){
    const в = s.делим;
    return ВОПРОС('Сколько учеников в одной трети класса?') +
      `<div class="pic">${свг(`
        ${фон(244)}
        ${т(168,22,'Делим класс на три части',14,GOLD,true)}
        ${Array.from({length:30},(_,i)=>{
          const кл=i%10, ряд=Math.floor(i/10);
          return `<g>${анТ('0 0;0 0;0 '+(ряд*4)+';0 '+(ряд*4)+';0 0','11s',
              'keyTimes="0;'+кт(0.1+ряд*0.08)+';'+кт(0.3+ряд*0.08)+';0.9;1"')}
            ${ученик(38+кл*29,58+ряд*36,ряд===0?'часть2':'целое',0)}</g>`;
        }).join('')}
        <g>${проявить('11s',0.44,0.52)}
          ${т(168,192,'одна треть: 30 : 3 = 10',16,GREEN,true)}
          ${т(168,218,'две трети: 10 · 2 = 20',16,GOLD,true)}
          ${т(168,240,'мальчиков осталось 10',12,МУТ)}</g>
      `,244)}</div>` +
      `<div class="ask">
  ...
  /* 3. Полоса-модель: полоса делится на ТРИ части, как велит знаменатель */
  function F3(s){
    const в = s.полоса;
    const x0=24, ш=288, часть=ш/3;
    return ВОПРОС('Полоса изображает 30. Сколько составят две трети полосы?') +
      `<div class="pic">${свг(`
        ${фон(250)}
        ${т(168,22,'Делим полосу на три равные части',14,GOLD,true)}
        <rect x="${x0}" y="56" width="${ш}" height="34" rx="6" fill="rgba(255,255,255,.05)" stroke="${ЛИНИЯ}" stroke-width="1.5"/>
        ${[0,1,2].map(i=>`<rect x="${(x0+i*часть+2).toFixed(1)}" y="58" width="${(часть-4).toFixed(1)}" height="30" rx="4"
          fill="${i<2?'url(#c1034-часть)':'url(#c1034-часть2)'}">
          ${i<2?проявить('12s',0.34+i*0.08,0.42+i*0.08):проявить('12s',0.62,0.7)}</rect>
          ${т((x0+(i+0.5)*часть).toFixed(1),108,'10',16,i<2?GOLD:МУТ,true)}`).join('')}
        ${делится(x0+часть,50,96,0.08,GOLD)}${делится(x0+2*часть,50,96,0.16,GOLD)}
        ${т(x0+часть,134,'2/3',14,GOLD,true)}
        ${т(x0+2.5*часть,134,'1/3',14,МУТ,true)}
        <g>${проявить('12s',0.5,0.6)}
          ${т(168,168,'30 : 3 = 10 — одна часть',14,ИНК,true)}
          ${т(168,196,'10 · 2 = 20 — две части',16,GOLD,true)}</g>
        <g>${проявить('12s',0.7,0.8)}
          ${карта(24,212,288,30,'30 : 3 · 2 = 20',16,GREEN,0,'rgba(143,209,168,.12)')}</g>
      `,250)}</div>` +
      `<div class="ask">
  ...
  /* 4. Обратная задача: число по его части */
  function F4(s){
    const в = s.обратно;
    return ВОПРОС('Две трети класса — это 20 человек. Сколько учеников во всём классе?') +
      `<div class="pic">${свг(`
        ${фон(252)}
        ${т(168,22,'Известна часть — ищем целое',14,GOLD,true)}
        ${[0,1,2].map(i=>{
          const x=32+i*94;
          const известна=i<2;
          return `<g>
            <rect x="${x}" y="56" width="86" height="52" rx="8"
              fill="${известна?'url(#c1034-часть)':'rgba(255,255,255,.05)'}"
              stroke="${известна?GOLD:ЛИНИЯ}" stroke-width="1.8">
              ${известна?проявить('12s',0.06+i*0.05,0.12+i*0.05):ан('opacity','0.3;0.3;1;1','12s','keyTimes="0;0.62;0.72;1"')}</rect>
            ${т(x+43,90,известна?'20':'?',20,известна?'#3a2a08':МУТ,true)}
            ${т(x+43,126,'треть '+(i+1),12,МУТ)}</g>`;
        }).join('')}
        <g>${проявить('12s',0.3,0.4)}
          ${т(168,158,'две трети — 20 человек',14,ИНК,true)}
          ${т(168,184,'20 : 2 = 10 — одна треть',14,GREEN,true)}
          ${т(168,210,'10 · 3 = 30 — весь класс',16,GOLD,true)}</g>
      `,252)}</div>` +
      `<div class="ask">
  ...
  function F5(s){
    ...
    return ВОПРОС('Что делать, если известна часть, а нужно целое?') +
      `<div class="pic">${свг(`
        ${фон(232)}
        ${т(168,22,'Два действия — два случая',14,GOLD,true)}
        ${карта(20,44,296,44,'дано целое → целое : зн · ч = часть',14,GREEN,0.05,'rgba(143,209,168,.12)')}
        ${карта(20,104,296,44,'дана часть → часть : ч · зн = целое',14,GOLD,0.24,'rgba(255,215,106,.12)')}
        <g>${проявить('11s',0.46,0.56)}
          ${карта(20,164,140,44,'2/3 от 30 = 20',14,ИНК,0,'rgba(255,255,255,.05)')}
          ${карта(176,164,140,44,'20 это 2/3 → 30',14,ИНК,0,'rgba(255,255,255,.05)')}
          ${т(168,226,'в обеих задачах участвуют одни и те же числа',12,МУТ)}</g>
      `,232)}</div>` +
      `<div class="ask">
  ...
  function F6(s){
    ...
    return ВОПРОС('В задаче сказано: «четыре пятых класса — это 20 человек». Что дано?') +
      `<div class="pic">${свг(`
        ${фон(240)}
        ${т(168,22,'Смотрим, что дано',14,GOLD,true)}
        ${карта(120,40,96,32,'задача',14,ИНК,0.04,'rgba(255,255,255,.05)')}
        <path d="M168 74 L168 92" stroke="${ЛИНИЯ}" stroke-width="2">
          ${проявить('11s',0.2,0.28)}</path>
        <path d="M168 92 C168 108 84 100 84 118" fill="none" stroke="${ЛИНИЯ}" stroke-width="2">
          ${проявить('11s',0.3,0.38)}</path>
        <path d="M168 92 C168 108 252 100 252 118" fill="none" stroke="${ЛИНИЯ}" stroke-width="2">
          ${проявить('11s',0.3,0.38)}</path>
        ${карта(20,118,128,40,'дано целое → умножаем',12,GREEN,0.4,'rgba(143,209,168,.12)')}
        ${карта(188,118,128,40,'дана часть → делим',12,GOLD,0.4,'rgba(255,215,106,.12)')}
        <g>${анД('M0 0 C0 20 84 12 84 54','9s')}
          <circle cx="168" cy="84" r="7" fill="${GOLD}" stroke="${ОБВОД}" stroke-width="1.4"/>
        </g>
        <g>${проявить('11s',0.6,0.7)}
          ${карта(52,178,232,34,'дана часть 20 — идём вправо',14,GOLD,0,'rgba(255,215,106,.12)')}
          ${т(168,228,'значит ищем целое: 20 : 4 · 5 = 25',14,GREEN,true)}</g>
      `,240)}</div>` +
      `<div class="ask">
  ...
  function F7(s){
    const в = s.верёвка;
    const x0=28, ш=280, часть=ш/4;
    return ВОПРОС('Верёвка 12 м. Сколько составят три четверти?') +
      `<div class="pic">${свг(`
        ${фон(252)}
        ${т(168,22,'Верёвка 12 м на четыре части',14,GOLD,true)}
        <rect x="${x0}" y="56" width="${ш}" height="26" rx="6" fill="rgba(255,255,255,.05)" stroke="${ЛИНИЯ}" stroke-width="1.5"/>
        ${[0,1,2,3].map(i=>`<rect x="${(x0+i*часть+1).toFixed(1)}" y="57" width="${(часть-2).toFixed(1)}" height="24" rx="4"
          fill="${i<3?'url(#c1034-часть)':'url(#c1034-часть2)'}">
          ${проявить('12s',0.1+i*0.06,0.18+i*0.06)}</rect>`).join('')}
        ${[1,2,3].map(i=>делится(x0+i*часть,50,88,0.06+i*0.05,GOLD)).join('')}
        ${т(x0+1.5*часть,104,'3/4',14,GOLD,true)}
        ${т(x0+3.5*часть,104,'1/4',14,МУТ,true)}
        <g>${проявить('11s',0.44,0.54)}
          ${т(168,140,'12 : 4 = 3 — одна четверть',14,ИНК,true)}
          ${т(168,170,'3 · 3 = 9 — три четверти',16,GOLD,true)}
          ${т(168,200,'осталось 3 м',12,МУТ)}
          ${карта(28,214,280,28,'обратно: 9 : 3 · 4 = 12',14,GREEN,0.6,'rgba(143,209,168,.12)')}</g>
      `,252)}</div>` +
      `<div class="ask">
  ...
  /* 8. Часовая шкала: 5/6 часа */
  function F8(s){
    const в = s.часы;
    const цx=112, цy=118, r=66;
    return ВОПРОС('Сколько минут в пяти шестых часа?') +
      `<div class="pic">${свг(`
        ${фон(250)}
        ${т(168,22,'Час — это шесть по десять минут',14,GOLD,true)}
        ${Array.from({length:6},(_,i)=>{
          ...
          return `<path d="M${цx} ${цy} L${x1.toFixed(1)} ${y1.toFixed(1)} A${r} ${r} 0 0 1 ${x2.toFixed(1)} ${y2.toFixed(1)} Z"
            fill="${i<5?'rgba(255,215,106,.28)':'rgba(255,255,255,.05)'}" stroke="${ОБВОД}" stroke-width="1.4">
            ${проявить('12s',0.12+i*0.06,0.2+i*0.06)}</path>`;
        }).join('')}
        ${т(цx,цy+5,'60',20,ИНК,true)}
        <g>${проявить('12s',0.6,0.7)}
          ${т(252,96,'одна часть',12,МУТ)}
          ${т(252,120,'10 минут',14,GREEN,true)}
          ${т(252,150,'пять частей',12,МУТ)}
          ${т(252,174,'50 минут',16,GOLD,true)}</g>
        ${т(168,222,'60 : 6 · 5 = 50 минут; остаётся 10 минут',14,ИНК,true)}
      `,250)}</div>` +
      `<div class="ask">
  ...
  /* 9. Задача про покупку: 7/10 от 200 */
  function F9(s){
    const в = s.покупка;
    return ВОПРОС('В задаче 7/10 от 200 рублей. Что нужно сделать первым?') +
      `<div class="pic">${свг(`
        ${фон(228)}
        ${т(168,22,'7/10 от 200 рублей',14,GOLD,true)}
        ${карта(24,44,288,40,'200 : 10 = 20 — одна десятая',14,ИНК,0.05,'rgba(255,255,255,.05)')}
        <g>${проявить('11s',0.28,0.38)}
          ${карта(24,100,288,40,'20 · 7 = 140 — семь десятых',16,GOLD,0.28,'rgba(255,215,106,.12)')}</g>
        ${Array.from({length:10},(_,i)=>`<rect x="${(26+i*28).toFixed(1)}" y="158" width="26" height="26" rx="4"
          fill="${i<7?'url(#c1034-часть)':'rgba(255,255,255,.05)'}" stroke="${i<7?GOLD:ЛИНИЯ}" stroke-width="1.4">
          ${проявить('12s',0.4+i*0.03,0.46+i*0.03)}</rect>`).join('')}
        <g>${проявить('11s',0.76,0.86)}
          ${т(168,212,'семь клеток по 20 — это 140',12,МУТ)}</g>
      `,228)}</div>` +
      `<div class="ask">
  ...
  /* 10. Сводка двух типов задач */
  function F10(s){
    const в = s.сводка;
    const строки=[['2/3 от 30','30 : 3 · 2 = 20','ищем часть'],
                  ['20 — это 2/3','20 : 2 · 3 = 30','ищем целое'],
                  ['3/4 от 12','12 : 4 · 3 = 9','ищем часть'],
                  ['9 — это 3/4','9 : 3 · 4 = 12','ищем целое']];
    return ВОПРОС('В задаче «12 : 4 · 3» что мы ищем?') +
      `<div class="pic">${свг(`
        ${фон(252)}
        ${т(168,22,'Два типа задач',14,GOLD,true)}
        ${строки.map(([дано,действие,что],i)=>{
          const y=38+i*48;
          return `<g>
            <rect x="16" y="${y}" width="304" height="40" rx="9" fill="rgba(255,255,255,.04)" stroke="${ЛИНИЯ}" stroke-width="1.3">
              ${проявить('11s',0.05*i,0.05*i+0.04)}</rect>
            ${т(74,y+26,дано,14,ИНК,true)}
            ${т(178,y+26,действие,14,GOLD,true)}
            <g>${проявить('11s',0.05*i+0.1,0.05*i+0.18)}
              ${т(290,y+26,что,12,что.indexOf('часть')>0&&i%2===0?GREEN:GOLD,true)}</g>
          </g>`;
        }).join('')}
        <g>${проявить('11s',0.62,0.72)}
          ${т(168,238,'действие выбирают по тому, что дано',12,МУТ)}</g>
      `,252)}</div>` +
      `<div class="ask">
