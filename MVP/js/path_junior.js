...
(function(){
  ...
  const ЭТАПЫ = {
    ...
    3: [
      ...
      {имя:'Формулы плавания', цель:'s = v · t, цена и стоимость',
       ...
       задачи:[]},
      {имя:'Ровный киль', цель:'сумма и разность, уравнивание',
       сюжет:'Груз по бортам кладут не на глаз: если справа тяжелее, шлюпка кренится. Сколько на каждом борту — считают, а не примеряют.',
       ид:[], сл:['Сумма и разность','уравнивание'],
       задачи:[]},
      ...
      {имя:'Судовой журнал', цель:'пишем без ошибок',
       ...
       ид:[620,935,861,862,863,864,868], сл:[],
       задачи:['Словарные слова']},
    ],
    4: [
      {имя:'Сняться с якоря', цель:'то, что умеешь с берега',
       ...
       ид:[], сл:['3–4 класс'],
       задачи:[]},
      {имя:'Последняя гавань', цель:'цена, количество и единица',
       сюжет:'Перед открытым морем — последний рынок. Дальше купить будет негде, и считать придётся до того, как отдашь деньги.',
       ид:[], сл:['Цена, количество, стоимость','куплю-продажу','Приведение к единице'],
       задачи:[]},
      ...
      {имя:'Компас и угол', цель:'углы, площадь, координаты',
       сюжет:'Курс — это угол. Ошибся на один градус — за сутки уйдёшь от острова на много миль.',
       ид:[], сл:['Площадь','Углы','Измерение углов','Координатный угол'],
       задачи:[]},
      {имя:'Склянки и календарь', цель:'время и сутки',
       ...
       ид:[], сл:['Олимп-4: время','Время и сутки','расписания и пояса'],
       задачи:['Время']},
      ...
      {имя:'Судовой журнал', цель:'пишем без ошибок',
       ...
       ид:[620,869,870,872,873,875,876], сл:[],
       задачи:['Словарные слова']},
  ...
  function шагHTML(ш, i, форма, следующий){
    ...
    const дело = "jpОткрыть('" + ш.вид + "'," + (ш.вид==='задача' ? "'"+ш.id+"'" : ш.id) + ")";
  ...
  /* ---------- ЦЕПОЧКА ШАГОВ --------------------------------------------
     Ребёнок, пришедший по маршруту, должен после пройденного шага получить
     следующий шаг МАРШРУТА, а не уходить в «Книгу знаний» и не получать
     случайную задачу из общего банка. Для этого маршрут помечает, что шаг
     открыт из «Пути», и умеет назвать следующий незакрытый шаг. */
  window.jpОткрыть = function(вид, id){
    window.JP.путь = true;
    window.JP.текущий = {вид:вид, id:id};
    if(typeof go === 'function') go((вид==='урок'?'lesson-':'task-')+id);
  };
  /* Идём ли мы сейчас по маршруту: только для 1–4 класса и только если шаг
     открыт из «Пути» (из каталога поведение прежнее). */
  window.jpВПути = function(){
    try{ return !!(window.JP && window.JP.путь) && младший(); }catch(e){ return false; }
  };
  window.jpВышли = function(){ try{ window.JP.путь=false; window.JP.текущий=null; }catch(e){} };
  /* Следующий шаг: первый непройденный после текущего, а если такого нет —
     первый непройденный вообще. Пройденность считается по свежему DB. */
  window.jpСледующийШаг = function(){
    try{
      const все = etapyШаги(маршрут().этапы);
      const т = (window.JP && window.JP.текущий) || null;
      const место = т ? все.findIndex(ш=>ш.вид===т.вид && String(ш.id)===String(т.id)) : -1;
      const свежий = ш => ш.вид==='урок' ? урокГотов({id:ш.id}) : задачаГотова({id:ш.id});
      for(let i=место+1;i<все.length;i++) if(!свежий(все[i])) return все[i];
      for(let i=0;i<все.length;i++) if(!свежий(все[i])) return все[i];
      return null;
    }catch(e){ return null; }
  };
  ...
  function renderPathJunior(){
    jpCss();
    /* Пришли на сам экран маршрута — значит цепочка окончена: пометка снимается,
       иначе урок, открытый потом из каталога, вёл бы себя как шаг маршрута. */
    try{ if(window.JP) window.JP.путь = false; }catch(e){}
    const k = +классУченика();
    ...
    const сейчас = следующий
      ? `<button type="button" class="jp-now" onclick="jpОткрыть('${следующий.вид}',${следующий.вид==='задача'?("'"+следующий.id+"'"):следующий.id})">
